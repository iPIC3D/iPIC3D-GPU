...
void dfprintf_fileLine(FILE* fptr, const char* func, const char* file,
                       int line_number, const char* format, ...) {
  fflush(fptr);
  va_list args;
  va_start(args, format);
  fprintf(fptr, "(%d) DEBUG %s(), %s:%d: ", get_rank(), func,
...
void assert_error(const char* file, int line, const char* func, const char* op,
                  ...
                  double rhs) {
  fprintf(stderr,
          "ERROR in file %s, line %d, function %s"
...
#define implement_assert_errmsg(t1, t2)                                        \
  void assert_error(const char* file, int line, const char* func,              \
    ...
    std::cerr << "ERROR in file " << file << ", line " << line                 \
              << ", function " << func << "\n\tassertion failed: " << lhs_str  \
              << op << rhs_str << ", i.e., " << lhs << op << rhs << endl;      \
    abort();                                                                   \
  }
...
void TimeTasks::print_cycle_times() {
  if (!get_rank()) {
    ...
    fprintf(stdout, TIMING_PREFIX "moms flds pcls Bfld cycl\n");
    fprintf(stdout,
            TIMING_PREFIX "%4.2f "
                          "%4.2f "
                          "%4.2f "
                          "%4.2f "
                          "%4.2f (total time)\n",
            get_time(TimeTasks::MOMENTS), get_time(TimeTasks::FIELDS),
            get_time(TimeTasks::PARTICLES), get_time(TimeTasks::BFIELD),
            get_time());
    fprintf(stdout,
            TIMING_PREFIX "%4.2f "
                          "%4.2f "
            ...
            get_communicate(TimeTasks::BFIELD), get_communicate());
    fprintf(stdout,
            TIMING_PREFIX "%4.2f "
                          "%4.2f "
                          "%4.2f "
                          "%4.2f "
                          "%4.2f (computation)\n",
            get_compute(TimeTasks::MOMENTS), get_compute(TimeTasks::FIELDS),
            get_compute(TimeTasks::PARTICLES), get_compute(TimeTasks::BFIELD),
            get_compute());
    fprintf(stdout,
            TIMING_PREFIX "MOMS comm  FLDS comm  PCLS comm  CYCL comm\n");
    fprintf(stdout,
            TIMING_PREFIX "%4.2f "
                          "%4.2f  "
                          "%4.2f "
                          ...
                          "%4.2f "
                          "%4.2f\n",
            get_time(TimeTasks::MOMENTS), get_communicate(TimeTasks::MOMENTS),
            get_time(TimeTasks::FIELDS), get_communicate(TimeTasks::FIELDS),
            get_time(TimeTasks::PARTICLES),
            get_communicate(TimeTasks::PARTICLES), get_time(),
            get_communicate());
    fflush(stdout);
  }
}
