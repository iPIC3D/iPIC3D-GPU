 ...
 */
__device__ __forceinline__ int compute_cell_idx(cudaPclType_X xp,
                                                cudaPclType_Y yp,
                                                cudaPclType_Z zp,
                                                const grid3DCUDA* g) {
  int cx, cy, cz;
...
/**
 * @brief Aggregate equal-cell contributions within a warp before issuing
 * atomics.
 *
 ...
 */
__device__ __forceinline__ void warp_aggregated_atomic_inc(int* histogram,
                                                           int cell) {
...
__global__ void cell_sort_histogram_small_kernel(
    const cudaPclType_X* __restrict__ x, const cudaPclType_Y* __restrict__ y,
    const cudaPclType_Z* __restrict__ z, int* __restrict__ cell_counts,
    const grid3DCUDA* __restrict__ grid, int num_cells, uint32_t num_to_sort) {
  ...
  for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < num_to_sort;
       tid += stride) {
...
__global__ void cell_sort_histogram_warp_agg_kernel(
    const cudaPclType_X* __restrict__ x, const cudaPclType_Y* __restrict__ y,
    const cudaPclType_Z* __restrict__ z, int* __restrict__ cell_counts,
    const grid3DCUDA* __restrict__ grid, uint32_t num_to_sort) {
  const size_t stride = size_t(gridDim.x) * blockDim.x;
...
launch_cell_sort_histogram(const cudaPclType_X* x, const cudaPclType_Y* y,
                           const cudaPclType_Z* z, int* cell_counts,
                           const grid3DCUDA* deviceGrid, int num_cells,
                           uint32_t num_to_sort, cudaStream_t stream) {
  if (num_to_sort == 0)
    return;
  ...
  if (num_cells <= SORT_HISTOGRAM_SHMEM_LIMIT) {
    ...
    cell_sort_histogram_small_kernel<<<blocks, SORT_BLOCK_SIZE, shmem,
                                       stream>>>(
 ...
 */
__global__ void
cell_sort_prefix_sum_single_kernel(int* __restrict__ cell_offsets,
...
__global__ void cell_sort_prefix_sum_phase1_kernel(
    int* __restrict__ output, int* __restrict__ block_sums,
    const int* __restrict__ input, int num_elements) {
  extern __shared__ int sdata[];
  const int tid = threadIdx.x;
 ...
 */
__global__ void
cell_sort_prefix_sum_phase2_kernel(int* __restrict__ block_sums, int num_blocks,
 ...
 */
__global__ void
cell_sort_prefix_sum_phase3_kernel(int* __restrict__ data,
...
__host__ inline void launch_cell_sort_prefix_sum(int* cell_offsets,
  ...
  if (num_cells <= SORT_SCAN_ELEMENTS_PER_BLOCK) {
    ...
    cell_sort_prefix_sum_single_kernel<<<1, SORT_SCAN_BLOCK_SIZE, shmem,
                                         stream>>>(cell_offsets, cell_counts,
                                                   num_cells, padded);
    return;
  }

  // ======= Phase 1: Scan each tile and record tile totals =======
  int nblocks = (num_cells + SORT_SCAN_ELEMENTS_PER_BLOCK - 1) /
  ...
  cell_sort_prefix_sum_phase2_kernel<<<1, SORT_SCAN_BLOCK_SIZE, phase2_shmem,
                                       stream>>>(block_sums, nblocks,
                                                 phase2_padded);

  // ======= Phase 3: Add the scanned tile prefixes back to each tile =======
  cell_sort_prefix_sum_phase3_kernel<<<nblocks, SORT_SCAN_BLOCK_SIZE, 0,
                                       stream>>>(cell_offsets, block_sums,
                                                 num_cells);
}
...
__global__ void cell_sort_sorted_indices_kernel(
    ...
    const cudaPclType_X* __restrict__ x, const cudaPclType_Y* __restrict__ y,
    const cudaPclType_Z* __restrict__ z, const grid3DCUDA* __restrict__ grid,
...
__host__ inline void launch_cell_sort_sorted_indices(
    unsigned int* sorted_indices, int* cell_offsets, const cudaPclType_X* x,
    const cudaPclType_Y* y, const cudaPclType_Z* z,
    const grid3DCUDA* deviceGrid, uint32_t num_to_sort, cudaStream_t stream) {
  if (num_to_sort == 0)
    return;
  const int blocks = getGridSize(num_to_sort, (uint32_t)SORT_BLOCK_SIZE);
...
__global__ void cell_sort_scatter_partial_kernel(
    ...
    const unsigned int* __restrict__ sorted_indices, uint32_t num_to_sort,
    uint32_t nop) {
  const size_t stride = size_t(gridDim.x) * blockDim.x;

  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < nop; i += stride) {
    if (i < num_to_sort)
...
__host__ inline void launch_cell_sort_scatter_partial(
    T* dst, const T* src, const unsigned int* sorted_indices,
    uint32_t num_to_sort, uint32_t nop, cudaStream_t stream) {
  if (nop == 0)
    return;
 ...
 * launch any kernels.
 */
__host__ void CellSorter::prepareBuffers(particleArrayCUDA* hostPtr,
                                         cudaStream_t s) {
  if (!initialized)
    return;
 ...
 */
__host__ void CellSorter::enqueueSortAsync(particleArrayCUDA* hostPtr,
  ...
  launch_cell_sort_histogram(soa->x, soa->y, soa->z, buffers.cell_counts,
                             deviceGrid, num_cells, num_to_sort, s);
  ...
  cudaErrChk(cudaMemcpyAsync(buffers.cell_start_offsets, buffers.cell_offsets,
                             num_cells * sizeof(int), cudaMemcpyDeviceToDevice,
                             s));

  // ======= Stage 3: Sorted indices =======
  launch_cell_sort_sorted_indices(buffers.sorted_indices, buffers.cell_offsets,
 ...
 */
__host__ void CellSorter::sort(particleArrayCUDA* hostPtr,
                               const grid3DCUDA* deviceGrid,
