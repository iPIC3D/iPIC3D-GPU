...
static int runTest() {
  ...
  std::normal_distribution<cudaCommonType> normalDist2(center + right * 0.4,
                                                       right * 0.1);
  ...
  cudaErrChk(cudaMemcpy(uPtr, uCPU, nop * sizeof(cudaCommonType),
                        cudaMemcpyHostToDevice));
  cudaErrChk(cudaMemcpy(vPtr, vCPU, nop * sizeof(cudaCommonType),
                        cudaMemcpyHostToDevice));
  cudaErrChk(cudaMemcpy(wPtr, wCPU, nop * sizeof(cudaCommonType),
                        cudaMemcpyHostToDevice));
  cudaErrChk(cudaMemcpy(qPtr, qCPU, nop * sizeof(cudaCommonType),
                        cudaMemcpyHostToDevice));
  ...
  for (int i = 0; i < nop; i++) {
    ...
    if (uVal >= minVal && uVal <= maxVal && vVal >= minVal && vVal <= maxVal &&
        wVal >= minVal && wVal <= maxVal) {
      int bin1 = static_cast<int>((uVal - minVal) / resolution1);
      if (bin1 >= VELOCITY_HISTOGRAM3D_RES_1)
        bin1 = VELOCITY_HISTOGRAM3D_RES_1 - 1;
      int bin2 = static_cast<int>((vVal - minVal) / resolution2);
      if (bin2 >= VELOCITY_HISTOGRAM3D_RES_2)
        bin2 = VELOCITY_HISTOGRAM3D_RES_2 - 1;
      int bin3 = static_cast<int>((wVal - minVal) / resolution3);
      if (bin3 >= VELOCITY_HISTOGRAM3D_RES_3)
        bin3 = VELOCITY_HISTOGRAM3D_RES_3 - 1;

      cpuHist[bin1 + bin2 * VELOCITY_HISTOGRAM3D_RES_1 +
              bin3 * VELOCITY_HISTOGRAM3D_RES_1 * VELOCITY_HISTOGRAM3D_RES_2] +=
  ...
  for (int i = 0; i < histogramSize; i++) {
    if (std::fabs(histogramHostPtr[i] - cpuHist[i]) > tolerance) {
      std::cout << "Mismatch in UV histogram at bin " << i
                << ": GPU = " << histogramHostPtr[i] << ", CPU = " << cpuHist[i]
                << "\n";
      pass = false;
      break;
    }
