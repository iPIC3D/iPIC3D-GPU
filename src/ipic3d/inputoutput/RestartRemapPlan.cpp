...
RestartInterval intersectInterval(RestartInterval a, RestartInterval b) {
  return makeInterval(std::max(a.begin, b.begin), std::min(a.end, b.end));
}
...
RestartInterval localInterval(RestartInterval global, RestartInterval owner) {
  return makeInterval(global.begin - owner.begin, global.end - owner.begin);
}
...
void validateMesh(const RestartMeshMetadata& mesh, const char* label) {
  if (!mesh.valid) {
    eprintf("ERROR: restart %s mesh metadata is missing or invalid", label);
  }
  if (mesh.nranks != mesh.xlen * mesh.ylen * mesh.zlen) {
    eprintf("ERROR: restart %s mesh topology is inconsistent", label);
  }
  if (mesh.nxc < mesh.xlen || mesh.nyc < mesh.ylen || mesh.nzc < mesh.zlen) {
...
void validateCompatibleMeshes(const RestartMeshMetadata& source,
  ...
  if (source.nxc != destination.nxc || source.nyc != destination.nyc ||
...
RestartInterval rankCellInterval(int globalCells, int ranks, int coord) {
  ...
  const int count =
      (coord == ranks - 1) ? globalCells - regular * (ranks - 1) : regular;
...
RestartRankBox restartRankBoxFor(const RestartMeshMetadata& mesh, int rank) {
  ...
  if (box.coord[0] != mesh.xlen - 1)
    box.ownedActiveNodes.x.end -= 1;
  if (box.coord[1] != mesh.ylen - 1)
    box.ownedActiveNodes.y.end -= 1;
  if (box.coord[2] != mesh.zlen - 1)
    box.ownedActiveNodes.z.end -= 1;
...
RestartRemapPlan::RestartRemapPlan(const RestartMeshMetadata& source,
  ...
  for (int sourceRank = 0; sourceRank < source_.nranks; ++sourceRank) {
    const RestartRankBox sourceBox = restartRankBoxFor(source_, sourceRank);

    const RestartBox3D nodeGlobal =
        intersectBox(sourceBox.ownedActiveNodes, destinationBox_.activeNodes);
    if (!nodeGlobal.empty()) {
      ...
      copy.destinationLocal = localBox(nodeGlobal, destinationBox_.activeNodes);
      nodeCopies_.push_back(copy);
    }

    const RestartBox3D cellGlobal =
        intersectBox(sourceBox.activeCells, destinationBox_.activeCells);
    if (!cellGlobal.empty()) {
      RestartCellCopy copy;
      copy.sourceRank = sourceRank;
      copy.global = cellGlobal;
      copy.sourceLocal = localBox(cellGlobal, sourceBox.activeCells);
      copy.destinationLocal = localBox(cellGlobal, destinationBox_.activeCells);
