...
template <class type>
type**** newArray4_Amaya(int sz1, int sz2, int sz3, int sz4) {
...
template <class type>
void delArr4_Amaya(type**** arr, int dummyx, int dummyy, int dummyz) {
  delete[] arr[0][0][0];
  delete[] arr[0][0];
  delete[] arr[0];
  delete[] arr;
...
#define newArr4_Amaya(type, sz1, sz2, sz3, sz4)                                \
  newArray4_Amaya<type>((sz1), (sz2), (sz3), (sz4))
#define newArr3_Amaya(type, sz1, sz2, sz3)                                     \
  newArray3_Amaya<type>((sz1), (sz2), (sz3))
...
template <class type> class Rank1 {
  const size_t S1;
...
template <class type> class Rank2 {
  const size_t S1, S2;
  type* __restrict__ const arr;

public:
  Rank2(size_t s1, size_t s2)
      : S1(s1), S2(s2), arr(AlignedAlloc(type, s1 * s2)) {}

  // Rank2( const Rank2& other ) : S1( other.S1 ), S2( other.S2 ), arr(
  // other.arr ) {}
...
template <class type> class Rank3 {
  const size_t S1, S2, S3;
  type* const __restrict__ arr;

public:
  Rank3(size_t s1, size_t s2, size_t s3)
      : S1(s1), S2(s2), S3(s3), arr(AlignedAlloc(type, s1 * s2 * s3)) {}
...
template <class type> class Rank4 {
  const size_t S1, S2, S3, S4;
  type* __restrict__ const arr;

public:
  Rank4(size_t s1, size_t s2, size_t s3, size_t s4)
      : S1(s1), S2(s2), S3(s3), S4(s4),
        arr(AlignedAlloc(type, s1 * s2 * s3 * s4)) {}

  // Rank4( const Rank4& other ) : S1( other.S1 ), S2( other.S2 ), S3( other.S3
  // ), S4( other.S4 ), arr( other.arr ) {}
...
template <class type> class BracketRank1 {
  const size_t S1;
  type* __restrict__ const arr;

public:
  BracketRank1(size_t s1, void* __restrict__ const storage)
      : S1(s1), arr(reinterpret_cast<type* __restrict__ const>(storage)) {}

  BracketRank1(size_t s1) : S1(s1), arr(new type[s1]) {}

  type& operator[](size_t i) const { return arr[i]; }
};

template <class type> class BracketRank2 {
  const size_t S1, S2;
  type* __restrict__ const arr;

public:
  void free() { delete[] arr; }
  BracketRank2(size_t s1, size_t s2, void* storage)
      : S1(s1), S2(s2),
        arr(reinterpret_cast<type* __restrict__ const>(storage)) {}

  BracketRank2(size_t s1, size_t s2) : S1(s1), S2(s2), arr(new type[s1 * s2]) {}
...
template <class type> void testArr2_diagonal() {
...
template <class type> void testArr2() {
  const int ITERS = 10000;
  const size_t dim1 = 64;
  const size_t dim2 = 64;
...
template <class type>
void set_prod3(array_ref3<type> Aarr, const_array_ref3<type> Barr,
               array_ref3<type> Carr, int ITERS, size_t dim1, size_t dim2,
               size_t dim3) {
  for (int t = 0; t < ITERS; t++)
    for (size_t i = 0; i < dim1; i++)
      for (size_t j = 0; j < dim2; j++)
...
template <class type> void testArr3() {
  const int ITERS = 100;
  const size_t dim1 = 64;
  const size_t dim2 = 64;
...
template <class type> void testArr4() {
      ...
      for (size_t j = 0; j < dim2; j++)
        for (size_t k = 0; k < dim3; k++)
          for (size_t l = 0; l < dim4; l++) {
            ...
            // Afix.arr[i][j][k][l] = Bfix.arr[i][j][k][l] *
            // Cfix.arr[i][j][k][l]; Afix.fetch(i,j,k,l) = Bfix.fetch(i,j,k,l) *
            // Cfix.fetch(i,j,k,l);
          }
    ...
    for (size_t i = 0; i < dim1; i++)
      for (size_t j = 0; j < dim2; j++)
        for (size_t k = 0; k < dim3; k++)
          for (size_t l = 0; l < dim4; l++) {
            Apar.fetch(i, j, k, l) =
                Bpar.get(i, j, k, l) * Cpar.get(i, j, k, l);
          }
  printf("%d us = Total time (i,j,k,l) access of array_ref4\n",
         tv_to_us(stopwatch(LAP)));
