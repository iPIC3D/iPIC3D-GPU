...
/** method to calculate the sum of two vectors vector1 = vector1 + vector2*/
void sum(arr3_double vect1, const arr4_double vect2, int nx, int ny, int nz,
         int ns) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
...
void sum4(arr3_double vect1, double alfa, const arr3_double vect2, double beta,
          const arr3_double vect3, double gamma, const arr3_double vect4,
          double delta, const arr3_double vect5, int nx, int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
        vect1.fetch(i, j, k) =
            alfa * (vect2.get(i, j, k) + beta * vect3.get(i, j, k) +
                    gamma * vect4.get(i, j, k) + delta * vect5.get(i, j, k));
}
...
/** method to calculate the scalar-vector product */
void scale(arr3_double vect1, const arr3_double vect2, double alfa, int nx,
           int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
...
void addscale(double alfa, arr3_double vect1, arr3_double vect2,
              const arr3_double vect3, int nx, int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
...
void addscale(double alfa, arr3_double vect1, const arr3_double vect2, int nx,
              int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
...
void addscale(double alfa, double beta, double* vect1, const double* vect2,
              int n) {
  for (int i = 0; i < n; i++)
    vect1[i] = vect1[i] * beta + alfa * vect2[i];
}
/** method to calculate vector1 = beta*vector1 + alfa*vector2 */
void addscale(double alfa, double beta, arr3_double vect1,
              const arr3_double vect2, int nx, int ny, int nz) {
...
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++) {
        vect1.fetch(i, j, k) =
            beta * vect1.get(i, j, k) + alfa * vect2.get(i, j, k);
      }
}
/** method to calculate vector1 = beta*vector1 + alfa*vector2 */
void addscale(double alfa, double beta, arr3_double vect1,
              const arr3_double vect2, int nx, int ny) {
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      vect1.fetch(i, j, 0) =
          beta * vect1.get(i, j, 0) + alfa * vect2.get(i, j, 0);
}

/** method to calculate vector1 = alfa*vector2 + beta*vector3 */
void scaleandsum(arr3_double vect1, double alfa, double beta,
                 const arr3_double vect2, const arr3_double vect3, int nx,
                 int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
...
void scaleandsum(arr3_double vect1, double alfa, double beta,
                 const arr4_double vect2, const arr3_double vect3, int ns,
...
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
        vect1.fetch(i, j, k) =
            alfa * vect2.get(ns, i, j, k) + beta * vect3.get(i, j, k);
}
/** method to calculate vector1 = alfa*vector2*vector3 with vector2 depending on
 * species*/
void prod(arr3_double vect1, double alfa, const arr4_double vect2, int ns,
          const arr3_double vect3, int nx, int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
        vect1.fetch(i, j, k) =
            alfa * vect2.get(ns, i, j, k) * vect3.get(i, j, k);
}
/** method to calculate vect1 = vect2/alfa */
void div(arr3_double vect1, double alfa, const arr3_double vect2, int nx,
         int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
...
void prod6(arr3_double vect1, const arr3_double vect2, const arr3_double vect3,
           const arr3_double vect4, const arr3_double vect5,
           const arr3_double vect6, const arr3_double vect7, int nx, int ny,
           int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
        vect1.fetch(i, j, k) = vect2.get(i, j, k) * vect3.get(i, j, k) +
                               vect4.get(i, j, k) * vect5.get(i, j, k) +
                               vect6.get(i, j, k) * vect7.get(i, j, k);
}
/** method used for calculating PI */
void proddiv(arr3_double vect1, const arr3_double vect2, double alfa,
             const arr3_double vect3, const arr3_double vect4,
             const arr3_double vect5, const arr3_double vect6, double beta,
             const arr3_double vect7, const arr3_double vect8, double gamma,
             const arr3_double vect9, int nx, int ny, int nz) {
#pragma omp parallel for collapse(2)
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
        vect1.fetch(i, j, k) =
            (vect2.get(i, j, k) +
             alfa * (vect3.get(i, j, k) * vect4.get(i, j, k) -
                     vect5.get(i, j, k) * vect6.get(i, j, k)) +
             beta * vect7.get(i, j, k) * vect8.get(i, j, k)) /
            (1 + gamma * vect9.get(i, j, k));
...
/** method to set equal two vectors */
void eq(arr4_double vect1, const arr3_double vect2, int nx, int ny, int nz,
        int is) {
  for (int i = 0; i < nx; i++)
    for (int j = 0; j < ny; j++)
      for (int k = 0; k < nz; k++)
...
void loopX(double* b, double z, double x, double y, double a, double zc,
  ...
  double Bz =
      B0 *
      (EllipticE(k, err) * (1 - Alpha * Alpha - Beta * Beta) / (Q - 4 * Alpha) +
       EllipticF(k, err)) /
      (M_PI * sqrt(Q));
  double BRho =
      B0 * Gamma *
  ...
  // for debugging
  /*cout << "\n\nAt (" << x << "," << y << "," << z << "), the field is :" <<
    endl; cout << "Bx: " << Bx << " T" << endl; cout << "By: " << By << " T" <<
    endl; cout << "Bz: " << Bz << " T" << endl; cout << "BRho: " << BRho << " T"
    << endl;*/
...
void loopY(double* b, double y, double z, double x, double a, double yc,
           double zc, double xc, double m) {
...
void loopZ(double* b, double x, double y, double z, double a, double xc,
           double yc, double zc, double m) {
