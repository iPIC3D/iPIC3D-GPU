...
static const char *taskNames[] = // order must agree with Tasks in TimeTasks.h
    {"none = 0",
     //
     "fields", "particles", "moments", "AFTER_EXCLUSIVE",
     //
     "communicating", "BEFORE_COMMUNICATION", "flds_comm", "flds_mpi_allreduce",
     "flds_mpi_sendrecv", "pcls_comm", "pcls_mpi_allreduce",
     "pcls_mpi_sendrecv", "moms_comm", "moms_mpi_allreduce",
     "moms_mpi_sendrecv",
     //
     "BEFORE_REPORT_LIST", "reduce_fields", "bfield", "moment_pcl_sorting",
     "moment_accumulation", "moment_reduction", "mover_pcl_sorting",
     "mover_pcl_moving", "transpose_pcls_to_aos", "transpose_pcls_to_soa",
     "write_fields", "write_particles", "PCLS_MPI_Isend", "PCLS_MPI_Irecv",
     "PCLS_MPI_Wait", "PCLS_MPI_Cancel", "PCLS_MPI_Request_free",
     "PCLS_MPI_Test", "PCLS_MPI_Waitany",
     //
     "number_of_tasks"};
...
void TimeTasks::print_cycle_times(int cycle, double *tskdur,
  ...
  FILE *file = stdout;
  {
    ...
    sndrcv[PARTICLES] += tskdur[PCLS_MPI_Isend] + tskdur[PCLS_MPI_Irecv] +
                         tskdur[PCLS_MPI_Wait] + tskdur[PCLS_MPI_Cancel] +
                         tskdur[PCLS_MPI_Request_free] + tskdur[PCLS_MPI_Test] +
...
static void reduce_doubles(int len, MPI_Op mpi_op, void *inbuff,
                           void *outbuff) {

  MPI_Allreduce(inbuff, outbuff, len, MPI_DOUBLE, mpi_op,
...
void TimeTasks::print_cycle_times(int cycle, const char *reduce_mode) {
  ...
  reduce_doubles(NUMBER_OF_TASKS, mpi_op, task_duration,
                 reported_task_duration);
...
void TimeTasks::print_cycle_times(int cycle) {
  ...
  // cycle); print_cycle_times(cycle, task_duration, "max");
  ...
  // cycle); print_cycle_times(cycle, task_duration, "min");
  if (!MPIdata::get_rank())
    fflush(stdout);
}
...
TimeTasks_caller_to_set_task_for_scope::TimeTasks_caller_to_set_task_for_scope(
    TimeTasks::Tasks task_) {
  assert(!omp_get_thread_num()); // if(omp_get_thread_num()) return;
  task = task_;
