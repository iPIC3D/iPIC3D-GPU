...
#ifndef PSK_EXCEPTION_H
#define PSK_EXCEPTION_H
...
public:
  ...
  Exception(const std::string& err_str, const std::string fn_str = "",
            int sys_errno = 0)
      : _can_recover(true), _err_str(err_str), _fn_str(fn_str),
        _type_str("PSK::Exception"), _sys_errno(sys_errno) {
    if (sys_errno != 0)
      _sys_err_str = strerror(sys_errno);
    errno = 0;
  }
  Exception(const char* c_err_str, const char* c_fn_str = "", int sys_errno = 0)
      : _can_recover(true), _err_str(c_err_str), _fn_str(c_fn_str),
        _type_str("PSK::Exception"), _sys_errno(sys_errno) {
    if (sys_errno != 0)
      _sys_err_str = strerror(sys_errno);
    errno = 0;
  ...
  Exception(const Exception& e)
      : _can_recover(e._can_recover), _err_str(e._err_str), _fn_str(e._fn_str),
        _type_str(e._type_str), _err_msgs(e._err_msgs),
        _sys_errno(e._sys_errno), _sys_err_str(e._sys_err_str) {
    ;
  }

  void append_err_str(const std::string& str) { _err_str += str; }

  void prepend_err_str(const std::string& str) { _err_str = str + _err_str; }

  void push(const std::string& err_msg) { _err_msgs.push_back(err_msg); }

  void diag_cout() {
    printf("%s %s in %s\n", _type_str.c_str(), _err_str.c_str(),
           _fn_str.c_str());
    // std::cout << _type_str << " " << _err_str << " in " << _fn_str <<
    // std::endl;
    if (_sys_errno != 0) {
      printf("sys errno: %d [%s]\n", _sys_errno, _sys_err_str.c_str());
      // std::cout << "sys errno: " << _sys_errno << " [" << _sys_err_str << "]"
      // << std::endl;
    }
