...
/**
 * @brief Simulation case identifiers resolved once during solver
 * initialization.
 */
enum class CaseType {
  GEMnoPert,
...
public:
  ...
  /**
   * @brief Wait for mover completion, exchange particles, and handle
   * injections.
   *
   * @param cycle Current simulation cycle.
...
private:
   ...
   * (host→device, GPU_SOLVER=OFF). */
  void copyHeatFluxBulkH2D(int species, cudaStream_t stream);
  /** @brief Copy one species' rho/J bulk moments device→device from GPU solver
   * arrays (GPU_SOLVER=ON). */
  void copyHeatFluxBulkD2D(int species, cudaStream_t stream);
   ...
   */
  void copyRestartParticleCellMetadataFromDevice(int species,
                                                 cudaStream_t stream);
  /** @brief Convert guarded sorter cell metadata to active-cell restart
   * metadata. */
...
private:
  ...
  ParticleCommInjection**
      particlesCommInj; // MPI exchange + injection engine (per species)
  ParticleSoAHost**
      particlesHost; // lightweight SoA host mirror (no communicator)
  ParticleSoAHost** testpart;
  ExosphereIonization*
      exosphereIonization; // exosphere photoionization source (CPU sampling)
  int numSolarWindSpecies; // cached: col->getNumSolarWindSpecies()
  int numPlanetarySpecies; // cached: ns - numSolarWindSpecies
  std::vector<std::future<void>>
      exosphereTaskFutures; // persistent future buffer (avoids
                            // per-call allocation)
  double* Ke; // kinetic energy of each species, the normal one, added up
  double* BulkEnergy; // bulk kinetic energy of each species, consider the bulk
                      // motion
  double*
      momentum; // an array of doubles, total momentum of all particle species
  double* Qtot; // total charge per species (sum of particle weights)
  double* Qremoved; // array of double, with species length, removed charges
                    // from the depopulation area
  Timing* my_clock;
  ...
  particleArrayCUDA**
      pclsArrayHostPtr; // array of pointers to host-resident metadata objects
  ...
  arrayCUDA<SpeciesParticle>**
      incomingStagingHostPtr;              // per-species AoS staging for H→D
                                           // incoming particles
  ...
  injectionParameter**
      injectionParamHostPtr; // for every species (GPU injection)
  ...
  particleArrayCUDA** pclsArrayCUDAPtr; // array of pointers to device-resident
                                        // particle metadata
  ...
  arrayCUDA<SpeciesParticle>**
      incomingStagingCUDAPtr; // per-species device copy of staging metadata
  ...
  injectionParameter**
      injectionParamCUDAPtr; // for every species (GPU injection)
  ...
  cudaEvent_t solverDoneEvent; ///< recorded on solverStream_ after field solve;
                               ///< GPU_SOLVER only
  cudaEvent_t momentsPipelineDoneEvt; ///< recorded on solverStream_ at end of
                                      ///< MomentsAwait() GPU pipeline (after
                                      ///< gpuCalculateHatFunctions); guards
                                      ///< heatFluxStream D2D reads of
                                      ///< d_rhons/d_Jxs/d_Jys/d_Jzs. GPU_SOLVER
                                      ///< + heatFlux only.
  ...
  cudaEvent_t* moverHashedReadyEvt; // [ns] : recorded on streams[i]   after
                                    // mover writes hashedSums
  cudaEvent_t* auxHashedReadyEvt;   // [ns] : recorded on streams[i+ns] after
  ...
  cudaParticleType* planetIonChargeDevice; // single-element device buffer for
                                           // reduction output
  int* planetCutoffDevice; // single-element device buffer for cutoff index
                           // output
  planetArray** planetArrayCUDAPtrDevice; // device array of device pointers
                                          // (for chargeCutoffKernel)
  ...
  // Persistent host buffers for processPlanetParticles (avoid per-call
  // allocation)
  int* planetElecOffsets;      // [planetElecSpeciesCount] species offsets into
                               // merged buffers
  planetArray** planetTmpPtrs; // [planetElecSpeciesCount] temp pointer array
  int* planetSurvivorCount;    // [planetElecSpeciesCount] survivor counts per
                               // electron species (host)
  int* planetSurvivorCountDevice; // [planetElecSpeciesCount] per-species atomic
                                  // counters (device)
  SpeciesParticle*
      planetReflectedBuf; // device buffer for compact reflected particles
  int planetReflectedBufCapacity; // capacity (in particles) of
                                  // planetReflectedBuf
  uint32_t planetRngCycleCounter; // incremented each cycle for diffuse-scatter
                                  // RNG seed
};
