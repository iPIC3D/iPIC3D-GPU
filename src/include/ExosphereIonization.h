#ifndef EXOSPHERE_IONIZATION_H
#define EXOSPHERE_IONIZATION_H
 ...
 * The caller is responsible for transferring the resulting particles to the
 * GPU.
 *
 ...
 *
 * Thread safety:
 *   - Each planetary species has its own std::mt19937_64 RNG and particle
 * buffer.
 *   - sampleIonizedParticles() is safe to call concurrently for DIFFERENT
 * speciesIndex values (each accesses only its own RNG, buffer, and charge
 * accumulator).
 *   - Concurrent calls with the SAME speciesIndex are NOT safe.
 *
 * Performance notes:
 *   - Per-species particle buffers are allocated once and reused across
 * timesteps (grow-only policy: capacity only increases, never shrinks).
 *   - The injection rate constant is pre-computed once per call
 * (loop-invariant).
 *   - The radial boundary check uses squared distances to avoid sqrt when
 * possible.
...
public:
  /**
   * @brief Construct with references to simulation configuration, grid, and
   * topology. Allocates persistent per-species particle buffers and RNGs for
   * all planetary species.
   *
   ...
   */
  ExosphereIonization(const Collective* collective, const Grid3DCU* grid,
                      const VCtopology3D* topology);

  /**
   * @brief Sample new ionized exosphere particles for one species on the CPU.
   *
   * Thread-safe for concurrent calls with different speciesIndex values. Each
   * species has its own RNG, particle buffer, and charge accumulator — no
   * shared mutable state is accessed. Uses std::mt19937_64 for reproducible,
   * thread-safe random sampling.
   *
   * Particles are written into a persistent internal buffer (no heap allocation
   * unless the buffer must grow). The returned reference is valid until the
   * next call to sampleIonizedParticles for the *same* speciesIndex.
   *
   ...
   *                      is reached, sampling stops early. This allows the
   * caller to enforce memory budgets based on GPU/CPU availability.
   * @return Read-only reference to the internal particle buffer for this
   * species. The caller must consume or copy the data before the next call for
   *         the same species.
   */
  const std::vector<SpeciesParticle>&
  sampleIonizedParticles(int speciesIndex, int maxParticles = 0);

  /**
   * @brief Get the total injected charge across all planetary species.
   *        This is a local (per-MPI-rank) value; use MPI_Allreduce for global
   * diagnostics.
   */
  double getTotalInjectedCharge() const;

  /**
   * @brief Get the injected charge for a specific planetary species from the
   * last call.
   *
   * @param speciesIndex Global species index for the requested planetary
   * species.
   */
  double getSpeciesInjectedCharge(int speciesIndex) const;

  /**
   * @brief Get the number of particles produced by the last call for one
   * species.
   *
   * @param speciesIndex Global species index for the requested planetary
   * species.
...
private:
   ...
   *
   * Returns Nexo * exp(-(distance - planetRadius) / scaleHeight) for distance >
   * planetRadius, and 0 otherwise (inside the planet).
   *
   * @param surfaceDensity  Neutral density at the planet surface (in n_sw
   * units).
   ...
   */
  static void
  sampleMaxwellianThreadSafe(double& u, double& v, double& w, double ut,
                             double vt, double wt, double u0, double v0,
                             double w0, std::mt19937_64& rng,
  ...
  int numPlanetarySpecies;        ///< number of planetary species (cached from
                                  ///< Collective)
  int firstPlanetarySpeciesIndex; ///< global index of first planetary species
                                  ///< (= numSolarWindSpecies)
  ...
  /// Seeded in constructor with (MPI_rank * numPlanetarySpecies +
  /// neutralIndex).
