...
void test_particle_communication(BlockCommunicator<Particle>& lowXrecv,
  ...
  const int incount = 2;
  MPI_Request recv_requests[incount] = {lowXrecv.get_curr_request(),
  ...
  while (!(lowXrecv.comm_finished() && hghXrecv.comm_finished())) {
    ...
    // MPI_Waitsome(incount, recv_requests, &outcount, recv_indices,
    // recv_statuses); dprintf("recv.comm_finished() = [%d, %d]",
    ...
    case 0: // lowXrecv
    {
      ...
      // dout << ": received lowXrecv." << recv_block.get_id() << recv_block <<
      // endl;
      lowXrecv.release_received_block();
    ...
    case 1: // hghXrecv
    {
      ...
      // dout << ": received hghXrecv." << recv_block.get_id() << recv_block <<
      // endl;
      hghXrecv.release_received_block();
...
void test_particle_communication() {
  ...
  if (0) {
    ...
    MPI_Irecv(up_recvbuff, count, MPI_CHAR, up_src, 0, up_comm,
              &up_recv_request);
    MPI_Irecv(dn_recvbuff, count, MPI_CHAR, dn_src, 0, dn_comm,
              &dn_recv_request);
    // send a message to upper
    MPI_Request up_send_request;
    MPI_Request dn_send_request;
    MPI_Isend(up_sendbuff, up_sendcount, MPI_CHAR, up_dst, 0, up_comm,
              &up_send_request);
    MPI_Isend(dn_sendbuff, dn_sendcount, MPI_CHAR, dn_dst, 0, dn_comm,
              &dn_send_request);
    // wait for message to arrive
