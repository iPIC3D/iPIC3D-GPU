...
struct OutputTagConfig {
  ...
  bool needsAnyMoments() const {
    return !rhoSpecies.empty() || !PXXSpecies.empty() || !PXYSpecies.empty() ||
           !PXZSpecies.empty() || !PYYSpecies.empty() || !PYZSpecies.empty() ||
           !PZZSpecies.empty() || writeRhoTot || writeJTot || writePXXTot ||
           writePXYTot || writePXZTot || writePYYTot || writePYZTot ||
           writePZZTot || needsAnyHeatFlux();
  }

  bool needsJTotComputation() const { return writeJTot; }

  bool needsAnyPTot() const {
    return writePXXTot || writePXYTot || writePXZTot || writePYYTot ||
           writePYZTot || writePZZTot;
  }

  bool needsAnyHeatFlux() const {
    for (int c = 0; c < HeatFlux::ComponentCount; ++c) {
      if (!heatFluxSpecies[c].empty() || writeHeatFluxTot[c])
        return true;
    }
    return false;
  }
  ...
  /** Number of scalar moment writes (rho + pressure; excludes J which is
   * vector). */
...
inline int heatFluxComponentFromToken(const std::string& token) {
  for (int c = 0; c < HeatFlux::ComponentCount; ++c)
    if (token == HeatFlux::ComponentNames[c])
      return c;
  return -1;
}
...
inline bool parseMomentToken(const std::string& tok, int ns,
  ...
  if (tok == "rho") {
    addAllSpecies(cfg.rhoSpecies, ns);
    return true;
  }
  if (tok == "J") {
    addAllSpecies(cfg.JSpecies, ns);
    return true;
  }
  if (tok == "P") {
    addAllPressureSpecies(cfg, ns);
    return true;
  }
  if (tok == "PXX") {
    addAllSpecies(cfg.PXXSpecies, ns);
  ...
  static const std::regex re("^(rho|J|PXX|PXY|PXZ|PYY|PYZ|PZZ|P|Qxxx|Qxxy|Qxxz|"
                             "Qxyy|Qxyz|Qxzz|Qyyy|Qyyz|Qyzz|Qzzz|Q)(\\d+)$");
  std::smatch m;
  if (std::regex_match(tok, m, re)) {
    int si = std::stoi(m[2].str());
    if (si < 0 || si >= ns) {
      fprintf(stderr,
              "WARNING: MomentsOutputTag token '%s' references "
    ...
    if (base == "rho")
      cfg.rhoSpecies.insert(si);
    else if (base == "J")
      cfg.JSpecies.insert(si);
    else if (base == "P")
      addPressureForSpecies(cfg, si);
    else if (base == "PXX")
      cfg.PXXSpecies.insert(si);
    else if (base == "PXY")
      cfg.PXYSpecies.insert(si);
...
inline bool parseFieldToken(const std::string& tok, int ns,
  ...
  if (tok == "Je") {
    cfg.JSpecies.insert(0);
    return true;
  }
  if (tok == "Ji") {
    cfg.JSpecies.insert(1);
    return true;
  }
  if (tok == "Je2") {
    cfg.JSpecies.insert(2);
    return true;
  }
  if (tok == "Ji3") {
    cfg.JSpecies.insert(3);
    return true;
  }
  if (tok == "rho") {
    cfg.writeRhoTot = true;
...
inline OutputTagConfig parseOutputTags(const std::string& fieldTag,
                                       const std::string& momentsTag, int ns) {
  OutputTagConfig cfg;

  for (const auto& tok : splitTags(fieldTag)) {
    if (!parseFieldToken(tok, ns, cfg)) {
      fprintf(stderr,
              "WARNING: unrecognised FieldOutputTag token '%s' — ignored.\n",
              tok.c_str());
    }
  }
  ...
  for (const auto& tok : splitTags(momentsTag)) {
    if (!parseMomentToken(tok, ns, cfg)) {
      fprintf(stderr,
              "WARNING: unrecognised MomentsOutputTag token '%s' — ignored.\n",
              tok.c_str());
    }
  }
