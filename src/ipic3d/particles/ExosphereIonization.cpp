...
ExosphereIonization::ExosphereIonization(
      ...
      particlesPerCellSW(collective->getNpcel(0))
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        std::seed_seq seed{mpiRank, pairIndex, 0x50494333};  // "PIC3" as extra entropy
...
double ExosphereIonization::neutralDensity(
...
ExosphereIonization::sampleIonizedParticles(int speciesIndex)
{
    ...
    // ── Get reference to this species' persistent buffer and reset particle count ──
    ...
    const double weightFactor       = collective->getMacroParticleWeightRatio(neutralIndex);
    ...
    const double chargePerParticle = chargeSign * cellVolume
                                     / (particlesPerCellSW * fourPi * weightFactor);

    // Injection rate constant (everything in N_inject that doesn't depend on position):
    ...
    const double injectionRateConstant = particlesPerCellSW * weightFactor
                                       * timeStep * ionizationFreq;

    // Squared radii for fast boundary checks (avoids sqrt)
    const double planetRadiusSquared       = planetRadius * planetRadius;
    const double maxInjectionRadiusSquared = maxInjectionRadius * maxInjectionRadius;
    ...
    for (int ix = 1; ix < numCellsX - 1; ix++) {
        for (int iy = 1; iy < numCellsY - 1; iy++) {
            for (int iz = 1; iz < numCellsZ - 1; iz++) {
                ...
                const double neutralDens = neutralDensity(surfaceDensity, distanceFromPlanet,
                                                          planetRadius, exoScaleHeight);

                // Expected number of newly ionized macro-particles in this cell:
                //   N_inject = injectionRateConstant * neutralDensity
                const double expectedParticleCount = injectionRateConstant * neutralDens;
                if (expectedParticleCount <= 0.0) continue;
                ...
                for (int ip = 0; ip < numParticlesToInject; ip++) {
                    ...
                    sampleMaxwellianThreadSafe(velocityX, velocityY, velocityZ,
                                              thermalVelocityX, thermalVelocityY, thermalVelocityZ,
                                              driftVelocityX, driftVelocityY, driftVelocityZ,
                                              rng, uniformDist);
