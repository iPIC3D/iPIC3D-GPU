...
public:
  ...
  void set(long long _ID, double _x, double _y, double _z, double _u, double _v,
           double _w, double _q) {
    ID = _ID;
    x[0] = _x;
    x[1] = _y;
...
public:
  void set(double Bx, double By, double Bz, double Ex, double Ey, double Ez) {
    BE[0] = Bx;
    BE[1] = By;
    BE[2] = Bz;
    BE[3] = Ex;
  ...
  double getB(int j) { return BE[j]; }
  double getE(int j) { return BE[j + 3]; }
};
...
void test_mover() {
  ...
  for (int i = 0; i < NUMPCLS; i++) {
    pcls[i].set(i, usample(), usample(), usample(), usample(), usample(),
                usample(), usample());
    flds[i].set(usample(), usample(), usample(), usample(), usample(),
                usample());
  }
  ...
  for (int pidx = 0; pidx < NUMPCLS; pidx += NUM_PCLS_MOVED_AT_A_TIME) {
    ...
    ALIGNED(pcl[0]);
    double xorig[NUM_PCLS_MOVED_AT_A_TIME][D]
        __attribute__((aligned(ALIGNMENT)));
    double uorig[NUM_PCLS_MOVED_AT_A_TIME][D]
        __attribute__((aligned(ALIGNMENT)));
    double xavg[NUM_PCLS_MOVED_AT_A_TIME][D]
        __attribute__((aligned(ALIGNMENT)));
    double uavg[NUM_PCLS_MOVED_AT_A_TIME][D]
        __attribute__((aligned(ALIGNMENT)));
    ...
    for (int innter = 0; innter < NiterMover; innter++) {
      ...
      // __attribute__((aligned(ALIGNMENT))); int
      // cx[NUM_PCLS_MOVED_AT_A_TIME][D] __attribute__((aligned(ALIGNMENT)));
      // for(int i=0;i<NUM_PCLS_MOVED_AT_A_TIME;i++)
      // {
      ...
      // __attribute__((aligned(ALIGNMENT))); for(int
      // i=0;i<NUM_PCLS_MOVED_AT_A_TIME;i++)
      // {
      ...
      double Om[NUM_PCLS_MOVED_AT_A_TIME][D]
          __attribute__((aligned(ALIGNMENT)));
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++)
        for (int j = 0; j < 3; j++) {
      ...
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++) {
        omsq[i] =
            Om[i][0] * Om[i][0] + Om[i][1] * Om[i][1] + Om[i][2] * Om[i][2];
        denom[i] = 1.0 / (1.0 + omsq[i]);
      }
      // solve the position equation
      double ut[NUM_PCLS_MOVED_AT_A_TIME][D]
          __attribute__((aligned(ALIGNMENT)));
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++)
        for (int j = 0; j < 3; j++) {
      ...
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++) {
        udotOm[i] =
            ut[i][0] * Om[i][0] + ut[i][1] * Om[i][1] + ut[i][2] * Om[i][2];
      }
      // solve the velocity equation
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++)
        for (int j = 0; j < 3; j++) {
          // With D=4, the cross product can be handled with 64-bit
          // cross-product swizzles. With D=4, the scalar-vector product can be
          // handled with 64-bit broadcast with 4-element granularity.  See
          // pages 27, 29, and 30 in the "Intel Xeon Phi Coprocessor Instruction
          // Set Reference Manual".
          uavg[i][0] = (ut[i][0] + (ut[i][1] * Om[i][2] - ut[i][2] * Om[i][1] +
                                    udotOm[i] * Om[i][0])) *
                       denom[i];
          uavg[i][1] = (ut[i][1] + (ut[i][2] * Om[i][0] - ut[i][0] * Om[i][2] +
                                    udotOm[i] * Om[i][1])) *
                       denom[i];
          uavg[i][2] = (ut[i][2] + (ut[i][0] * Om[i][1] - ut[i][1] * Om[i][0] +
                                    udotOm[i] * Om[i][2])) *
                       denom[i];
        }
...
void test_mover_AoS_vec() {
  ...
  for (int pidx = 0; pidx < NUMPCLS; pidx += NUM_PCLS_MOVED_AT_A_TIME) {
    ...
    for (int innter = 0; innter < NiterMover; innter++) {
      ...
      double E[NUM_PCLS_MOVED_AT_A_TIME * D]
          __attribute__((aligned(ALIGNMENT)));
      double B[NUM_PCLS_MOVED_AT_A_TIME * D]
          __attribute__((aligned(ALIGNMENT)));
      //// could do this with memset
      // for(int i=0;i<NUM_PCLS_MOVED_AT_A_TIME;i++)
      // for(int j=0;j<D;j++)
      ...
      double uxOm_A[NUM_PCLS_MOVED_AT_A_TIME * D]
          __attribute__((aligned(ALIGNMENT)));
      double uxOm_B[NUM_PCLS_MOVED_AT_A_TIME * D]
          __attribute__((aligned(ALIGNMENT)));
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++) {
      ...
      double denomv[NUM_PCLS_MOVED_AT_A_TIME * D]
          __attribute__((aligned(ALIGNMENT)));
      for (int i = 0; i < NUM_PCLS_MOVED_AT_A_TIME; i++) {
      ...
      for (int k = 0; k < NUM_PCLS_MOVED_AT_A_TIME * D; k++) {
        int i = k / D; // k=i*D+j;
        uavg[k] =
            ut[k] + (uxOm_A[k] - uxOm_B[k] + udotOm[i] * Om[k]) * denom[i];
      }
      // update average position
      for (int k = 0; k < NUM_PCLS_MOVED_AT_A_TIME * D; k++) {
