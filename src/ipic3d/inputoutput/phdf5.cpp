...
PHDF5fileClass::PHDF5fileClass(string filestr, int nd, const int* coord,
                               MPI_Comm mpicomm) {
...
void PHDF5fileClass::CreatePHDF5file(double* L, int* dglob, int* dlocl,
                                     bool bp) {
  ...
  if (!MPIdata::get_rank())
    dprint(str_stripe_count);
  if (!MPIdata::get_rank())
    dprint(str_stripe_size);
  if (!MPIdata::get_rank())
    dprint(str_cb_buffer_size);
  ...
  for (int i = 0; i < ngrp; i++) {
    if (!(grpnames[i].c_str() == "Particles" && !bparticles)) {
      hid_t grp;
      grp = H5Gcreate2(file_id, grpnames[i].c_str(), H5P_DEFAULT, H5P_DEFAULT,
                       H5P_DEFAULT);
      H5Gclose(grp);
    }
  }
  ...
  status = H5LTmake_dataset(file_id, "/Parameters/LxLyLz", 1, d,
                            H5T_NATIVE_DOUBLE, LxLyLz);
  status = H5LTmake_dataset(file_id, "/Parameters/ncell", 1, d, H5T_NATIVE_INT,
                            dglob);
}

void PHDF5fileClass::ClosePHDF5file() { H5Fclose(file_id); }

int PHDF5fileClass::WritePHDF5dataset(string grpname, string datasetname,
                                      const_arr3_double data, int nx, int ny,
                                      int nz, double scale) {
  ...
  for (int i = 0; i < ndim; i++) {
    if (dim[i] % chdim[i] != 0) {
      eprintf(
          "Grid size is not a multiple of the chunk size in the %d dimension,"
          "\tGlob: %d %d %d\n"
          "\tLocl: %d %d %d\n",
          i, dim[0], dim[1], dim[2], chdim[0], chdim[1], chdim[2]);
      return 1;
    }
  }
  ...
  dataset = H5Dcreate2(file_id, dname.c_str(), h5type, glob_dspace, H5P_DEFAULT,
                       dataset_prop, H5P_DEFAULT);
...
int PHDF5fileClass::WritePHDF5dataset(string grpname, string datasetname,
                                      const_arr3_double data, int i0, int j0,
                                      int k0, const hsize_t* gdim,
                                      const hsize_t* ldim, const hsize_t* foff,
                                      double scale) {
  hid_t const h5type = H5T_NATIVE_DOUBLE;
...
void PHDF5fileClass::ReadPHDF5param() {
  ...
  ndim = 3;
  if (datadims[0] <= 1 || datadims[1] <= 1 || datadims[2] <= 1)
    ndim = 2;
...
void PHDF5fileClass::ReadPHDF5dataset_double(string datasetname,
                                             arr3_double data) {
...
int PHDF5fileClass::getPHDF5ncx() { return (int)dim[0]; }
...
int PHDF5fileClass::getPHDF5ndim() { return ndim; }
