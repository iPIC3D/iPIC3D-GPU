...
//
template <class type> class Larray {
protected: // members
...
public: // memory
  ~Larray() { AlignedFree(list); }
  Larray() : list(0), _size(0), _capacity(0) {}
  Larray(int requested_size) : list(0), _size(0), _capacity(0) {
    if (requested_size > 0)
  ...
  virtual void reserve(int newcapacity) {
    ...
    newcapacity =
        ((newcapacity - 1) / num_elem_in_block + 1) * num_elem_in_block;
...
template <class type> class LarrayRegistered : public Larray<type> {
...
public:
  ...
  void reserve(int newcapacity) override {
    ...
    newcapacity = ((newcapacity - 1) / this->num_elem_in_block + 1) *
                  this->num_elem_in_block;
