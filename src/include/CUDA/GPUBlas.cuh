...
void gpuEqValue(cudaSolverType* d, cudaSolverType val, size_t n, cudaStream_t stream = 0);

/** d[i] *= alfa */
void gpuScale(cudaSolverType* d, cudaSolverType alfa, size_t n, cudaStream_t stream = 0);
...
/** dst[i] += src[i] */
void gpuSum(cudaSolverType* dst, const cudaSolverType* src,
            size_t n, cudaStream_t stream = 0);

/** dst[i] -= src[i] */
void gpuSub(cudaSolverType* dst, const cudaSolverType* src,
            size_t n, cudaStream_t stream = 0);
...
void gpuAddscale2(cudaSolverType alfa, cudaSolverType beta,
                  cudaSolverType* dst, const cudaSolverType* src,
                  size_t n, cudaStream_t stream = 0);
...
void gpuEq(cudaSolverType* dst, const cudaSolverType* src, size_t n, cudaStream_t stream = 0);
...
void gpuSetAll0_N(cudaSolverType** d_ptrs, int nFields, size_t n, cudaStream_t stream = 0);
 ...
 *   d_out[k+1] = norm2(w, n)
 *
 * d_out is a device array of >= (k+2) cudaSolverType elements (zeroed internally).
...
void gpuBatchedDotNorm(const cudaSolverType* w, const cudaSolverType* V_base,
                       ...
                       cudaSolverType* d_out, cudaStream_t stream = 0);
