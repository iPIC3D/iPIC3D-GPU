...
void VCtopology3D::setup_vctopology(MPI_Comm old_comm) {
  ...
  if (CART_COMM != MPI_COMM_NULL) {
    ...
    // previous check that nprocs = XLEN*YLEN*ZLEN should prevent reaching this
    // line.
  ...
  if (CART_COMM_P != MPI_COMM_NULL) {
    ...
    MPI_Cart_shift(CART_COMM_P, YDIR, RIGHT, &yleft_neighbor_P,
                   &yright_neighbor_P);
    MPI_Cart_shift(CART_COMM_P, ZDIR, RIGHT, &zleft_neighbor_P,
                   &zright_neighbor_P);
  }
  ...
  _isBoundaryProcess_P = _noXleftNeighbor_P || _noXrghtNeighbor_P ||
                         _noYleftNeighbor_P || _noYrghtNeighbor_P ||
...
void VCtopology3D::PrintMapping() {
  ...
  cout << "----------------------" << endl;
  cout << "Coordinates: X = " << coordinates[0] << "; Y = " << coordinates[1]
       << "; Z = " << coordinates[2] << endl;
  cout << "Neighbors: xLeft = " << xleft_neighbor
       << "; xRight = " << xright_neighbor << "; yLeft = " << yleft_neighbor
       << "; yRight = " << yright_neighbor << "; zLeft = " << zleft_neighbor
       << "; zRight = " << zright_neighbor << endl;
  cout << endl;
}
