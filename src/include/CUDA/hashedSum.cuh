#ifndef HASHED_SUM_CUH
#define HASHED_SUM_CUH
...
private:
  __host__ __device__ int hash(int key) { return key % bucketNum; }

public:
  __host__ hashedSum(int bucketNum) : bucketNum(bucketNum) {
    // if(bucketNum > bucketSize)throw std::out_of_range("[!]hashedSum out of
    // range");
    resetBucket();
  }
  ...
  __host__ void resetBucket() { memset(bucket, 0, bucketNum * sizeof(int)); }
