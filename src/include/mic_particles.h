...
inline F64vec8 sample_field_mic(F64vec8 weights, F64vec8 field_components[8]) {
...
inline F64vec8 compute_uvg_for_2pcls(F64vec8 uorig, F64vec8 B, F64vec8 E,
                                     F64vec8 qdto2mc) {
  ...
  const F64vec8 udotOm = ut.aaaa() * Omx + ut.bbbb() * Omy + ut.cccc() * Omz;
...
inline void get_field_components_for_cell(F64vec8 field_components0[8],
  ...
  //
  {
    ...
    //
    // broadcast coordinates of first and second particle to low and high end of
    // I32vec16
    const __mmask16 rmask_00ff =
        _mm512_int2mask(0xff00); // mask=1111111100000000
    ...
    const __mmask16 rmask_00001111x2 =
        _mm512_int2mask(0xf0f0); // mask=11110000x2
    const __mmask16 rmask_00110011x2 =
        _mm512_int2mask(0xcccc); // mask=11001100x2
    const __mmask16 rmask_01010101x2 =
        _mm512_int2mask(0xaaaa); // mask=10101010x2
