...
inline bool signal_hack() { return true; }
...
public:
  Block(int capacity_, int id_)
      : capacity(capacity_), listID(id_), request(MPI_REQUEST_NULL), signal(0) {
...
public: // sending
  ...
  bool is_active() { return request != MPI_REQUEST_NULL; }
  bool is_inactive() { return request == MPI_REQUEST_NULL; }
  bool isfull() {
  ...
  void push_back(const type& in) { block.push_back(in); }
  ...
  void set_request(MPI_Request request_) { request = request_; }
  void clear() {
    block.clear();
...
//
template <class type> class BlockCommunicator {
  enum CommState { NONE = 0, INITIAL, ACTIVE, FINISHED };
...
public: // construction
  BlockCommunicator()
      : connection(), blocksize(0), nextListID(0), nextCommID(0),
        commState(NONE) {}
...
public: // access
  ...
  bool comm_finished() const { return commState == FINISHED; }
  bool comm_active() { return get_curr_request() != MPI_REQUEST_NULL; }

  // operations
  //
private:
  void increment_curr_block() { increment_block(curr_block); }
  // void rewind()
  //{
...
void BlockCommunicator<type>::init(Connection connection_, int blocksize_,
                                   int numblocks) {
...
//
// inline std::ostream& operator<<(std::ostream& os, const Block<Particle>&
// block_)
//{
