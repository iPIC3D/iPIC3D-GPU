...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot strong-scaling efficiency from iPIC3D stdout files."
    )
    parser.add_argument(
        "results_folder",
        type=Path,
...
def main():
    ...
    baseline_accelerator_count = accelerator_counts[0]
    efficiencies = [
        100.0
        * baseline_time
        * baseline_accelerator_count
        / (cycle_time * accelerator_count)
        for cycle_time, accelerator_count in zip(
            cycle_loop_seconds, accelerator_counts
        )
    ]
    ...
    ax.set_title(
        f"iPIC3D Strong Scaling Efficiency ({cycle_count} cycles)"
    )
    ax.grid(True, which="major", linestyle=":", alpha=0.6)
    ax.legend()

    output_path = results_folder / "strong_scaling_efficiency.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)

    for node_count, cycle_time, efficiency in zip(
        nodes, cycle_loop_seconds, efficiencies
    ):
        print(
            f"{node_count:3d} nodes "
            f"({node_count * GPUS_PER_NODE:4d} GPUs): "
