...
void read_virtual_sat_block(float Bx0, float dt, char output_filename[]) {
  ...
  for (int isatloop = 0; isatloop < number_of_satellites_to_extract;
    ...
    // write out - each block of data corresponds to a satellite
    // 'VirtualSatelliteFilenameFound'

    float omega_ci_x_t; // this will be the X-axis in the output file

    // write out notation at the beginning of each block
    fprintf(output_file, "%s %i %s %s", "Data Block: satellite No",
            satellite_number_best_match, "from file",
            VirtualSatelliteFilenameFound);
    fprintf(output_file, "\n", "");
    fprintf(output_file, "%i %s", number_of_data_rows,
            "data rows in this block");
    fprintf(output_file, "\n", "");
  ...
  fclose(satellite_info_file); // close file
                               // 'BestVirtualSatelliteInfoFoundBlock.txt'
}
