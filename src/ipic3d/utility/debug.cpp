...
#define implement_dprintvar_fileLine(code, type)                               \
  void printvar_fileLine(const char* func, const char* file, int line,         \
    ...
    fprintf_fileLine(stdout, "DEBUG", func, file, line, code " == %s", val,    \
                     name);                                                    \
  }
...
// void dfprintf_fileLine(FILE * fptr, const char *func, const char *file, int
// line_number, const char *format, ...)
// {
...
//   chars_so_far += vsnprintf(sptr+chars_so_far, maxchars-chars_so_far, format,
//   args); va_end(args); sprintf(sptr+chars_so_far, "\n");
//
//   // print the message
//   fflush(fptr);
...
void fprintf_fileLine(FILE* fptr, const char* type, const char* func,
...
#ifndef NO_MPI
#ifdef _OPENMP
  snprintf(process_thread_str, 20, "(%d.%d) ", MPIdata::get_rank(),
           omp_get_thread_num());
#else
  snprintf(process_thread_str, 20, "(%d)", MPIdata::get_rank());
#endif
#else
#ifdef _OPENMP
  snprintf(process_thread_str, 20, "(.%d) ", omp_get_thread_num());
#else
  snprintf(process_thread_str, 20, "");
#endif
#endif
  ...
  va_start(args, format);
  chars_so_far = snprintf(sptr, maxchars,
                          "%s%s %s(), %s:%d: ", process_thread_str, type, func,
                          /*file,*/ my_basename(file), line_number);
  /* print out remainder of message */
  chars_so_far +=
      vsnprintf(sptr + chars_so_far, maxchars - chars_so_far, format, args);
  va_end(args);
