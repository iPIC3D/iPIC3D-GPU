...
void Parameters::init_parameters() {
  RESORTING_PARTICLES =
      get_MOVER_TYPE() == SoA_vec_resort || get_MOVER_TYPE() == AoS_vec_resort;
  SORTING_PARTICLES =
      get_VECTORIZE_MOMENTS() || get_MOVER_TYPE() == SoA_vec_onesort ||
  ...
  USING_AOS = get_MOMENTS_TYPE() == AoS || get_MOVER_TYPE() == AoS ||
