...
/*! Collective properties. Input physical parameters for the simulation.  Use
 * ConfigFile to parse the input file @date Wed Jun 8 2011 @par Copyright: (C)
 * 2011 K.U. LEUVEN @author Pierre Henri, Stefano Markidis @version 1.0 */
...
public:
  ...
  Collective(const ConfigFile& config,
             const std::string& input_name = "in-memory", bool restart = false);
  /*! read input file */
  ...
  int getMaxExosphereParticlesPerSpecies() const {
    return (maxExosphereParticlesPerSpecies);
  }
  double getNeutralSurfaceDensity(int neutralSpecies) const {
    return (neutralSurfaceDensity[neutralSpecies]);
  }
  double getExosphericScaleHeight(int neutralSpecies) const {
    return (exosphericScaleHeight[neutralSpecies]);
  }
  double getPhotoionizationFrequency(int neutralSpecies) const {
    return (photoionizationFrequency[neutralSpecies]);
  }
  double getMacroParticleWeightRatio(int neutralSpecies) const {
    return (macroParticleWeightRatio[neutralSpecies]);
  }
  ...
  int getdivBCorrectionCycle() const { return (divBCorrectionCycle); }
  ...
  double getPoissonChebRescaleEigMin() const {
    return (PoissonChebRescaleEigMin);
  }
  double getPoissonChebRescaleEigMax() const {
    return (PoissonChebRescaleEigMax);
  }
...
private:
  ...
  int MacrocellNy;
  int MacrocellNz;
  /*! Top-level on/off switch for the spectra pipeline (VelocitySpectra in
   * input). */
  int velocitySpectra;
  ...
  /*! weight factor (planet-to-SW macro-particle weight ratio) per neutral
   * species */
  ...
  /*! restart_status 0 --> no restart; 1--> restart, create new; 2--> restart,
   * append; */
  int restart_status;
  /*! last cycle */
  int last_cycle;
  ...
  /*! Field Boundary Condition 0 = Dirichlet Boundary Condition: specifies the
   * valueto take pn the boundary of the domain 1 = Neumann Boundary Condition:
   * specifies the value of derivative to take on the boundary of the domain 2 =
   * Periodic Condition */
  ...
  int currentFromAmpere; ///< 1 = J from curl(B) with u0 weights, 0 = J from u0
                         ///< drift
  int spatiallyVaryingThermal; ///< 1 = use reference-state pressure for local
                               ///< vth, 0 = global uth
  ...
  /*! Poisson Chebyshev preconditioner: rescale factor for min eigenvalue
   * (default 1.0) */
  double PoissonChebRescaleEigMin;
  /*! Poisson Chebyshev preconditioner: rescale factor for max eigenvalue
   * (default 1.0) */
