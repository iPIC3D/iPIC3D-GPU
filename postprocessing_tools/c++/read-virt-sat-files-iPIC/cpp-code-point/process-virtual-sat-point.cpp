/***************************************************************************
    ...
    process-virtual-sat.cpp  -  postprocessing program to extract data from
 VirtualSatellites* files from iPIC3D
    ...
    If using FFTW3, compile with "c++ process-virtual-sat-point.cpp
 -I/usr/local/include/ -L/usr/lib/ -lfftw3 -lm" or with "c++
 process-virtual-sat-point.cpp -lfftw3 -lm" of if you install FFTW3 from source
 adjust the 'include' and 'lib' locations accordingly.
...
////////////////////////////////////////////////////////////////////////
// Function 'find_virtual_sat' finds the best satellite match with closest
// position to user given (x,y,z) spatial coordinates. The result is used by
// funciton 'read_virtual_sat'.
////////////////////////////////////////////////////////////////////////

void find_virtual_sat(float x_satellite_user, float y_satellite_user,
                      float z_satellite_user, int const nproc) {
  ...
  int satellite_number_best_match; // later used in read-virtual-sat.cpp to
                                   // extract the corresponding data
  string VirtualSatelliteFilenameFoundString; // this file contains the best
                                              // satellite coordinates match

  vector<float> desired_sat_coordinates(
      number_of_coords,
      0); // define 3D vector with zeros - the user desired satellite position
  vector<float> current_sat_coordinates(
      number_of_coords,
      0); // define 3D vector with zeros - reading current coords. from the file
  vector<float> closest_sat_coordinates(
      number_of_coords,
      0); // define 3D vector with zeros - closest
          // satellite position to what we want
  vector<float> current_vector_difference(number_of_coords, 0);
  vector<float> closest_vector_difference(
      number_of_coords,
      0); // this will end up being closest to the desired satellite position
  ...
  desired_sat_coordinates[2] = z_satellite_user;

  // set up initially the closest satelite position to something much bigger
  ...
  closest_vector_difference[2] = desired_sat_coordinates[2] * 1.0E30;
  ...
  for (int iproc = 0; iproc < nproc; iproc++) { // loop over all files
    ...
    if (virt_sat_file) { // if the file exists
      for (i = 0; i < number_of_satellites;
           i++) { // loop over rows (number of satellites in file)
        for (j = 0; j < number_of_coords;
             j++) { // loop over columns/coordinates in space
          fscanf(virt_sat_file, "%E", &temp_number); // read row element
          current_sat_coordinates[j] = temp_number;
          if ((k = fgetc(virt_sat_file)) ==
              '\n') { // if new line then goto next row (set
                      // of spatial coordinates)
            break;
          }
        }
        // vector difference
        current_vector_difference[0] =
            desired_sat_coordinates[0] - current_sat_coordinates[0];
        current_vector_difference[1] =
            desired_sat_coordinates[1] - current_sat_coordinates[1];
        current_vector_difference[2] =
            desired_sat_coordinates[2] - current_sat_coordinates[2];
        // modulus of vector difference
        mod_current_vector_difference =
            sqrt(current_vector_difference[0] * current_vector_difference[0] +
                 current_vector_difference[1] * current_vector_difference[1] +
                 current_vector_difference[2] * current_vector_difference[2]);
        mod_closest_vector_difference =
            sqrt(closest_vector_difference[0] * closest_vector_difference[0] +
                 closest_vector_difference[1] * closest_vector_difference[1] +
                 closest_vector_difference[2] * closest_vector_difference[2]);
        ...
        if (mod_current_vector_difference <= mod_closest_vector_difference) {
          ...
          closest_vector_difference[2] = current_vector_difference[2];
          closest_sat_coordinates[0] = current_sat_coordinates[0];
          closest_sat_coordinates[1] = current_sat_coordinates[1];
          closest_sat_coordinates[2] = current_sat_coordinates[2];
          satellite_number_best_match =
              i + 1; // +1 because counting starts from zero
  ...
  // count the numeber of lines in 'VirtualSatelliteFilenameFoundString' and
  // take only the number of data rows in it
  FILE* file_to_count_lines =
      fopen(VirtualSatelliteFilenameFoundString.c_str(), "r");
  int line_count = 0;
  while (!fscanf(file_to_count_lines, "%*[^\n]%*c"))
    line_count++;
  fseek(file_to_count_lines, 0, SEEK_SET);
  fclose(file_to_count_lines);
  line_count = line_count - number_of_satellites;

  // print some messages on screen
  cout << "---> Found satellite No " << satellite_number_best_match
       << " in file " << "'" << VirtualSatelliteFilenameFoundString << "'"
       << endl;
  cout << "---> Desired Sat. Coordinates " << desired_sat_coordinates[0] << " "
       << desired_sat_coordinates[1] << " " << desired_sat_coordinates[2]
       << endl;
  cout << "---> Closest Sat. Coordinates " << closest_sat_coordinates[0] << " "
       << closest_sat_coordinates[1] << " " << closest_sat_coordinates[2]
       << endl;
  cout << "---> Closest Vector Difference " << closest_vector_difference[0]
       << " " << closest_vector_difference[1] << " "
       << closest_vector_difference[2] << endl;
  cout << "---> Number of data lines in the file " << line_count << endl;

  // write out best satellite info to be used by read-virt-sat.cpp
  FILE* best_satellite_match_info_found =
      fopen("BestVirtualSatelliteInfoFoundPoint.txt", "w");
  fprintf(best_satellite_match_info_found, "%s %i %i",
          VirtualSatelliteFilenameFoundString.c_str(),
          satellite_number_best_match, line_count);
  fclose(best_satellite_match_info_found);
}

////////////////////////////////////////////////////////////////////////
// Function 'read_virtual_sat' uses output from function 'find_virtual_sat' to
// extract the corresponding satellite data
////////////////////////////////////////////////////////////////////////

void read_virtual_sat(float Bx0, float dt, char output_filename[]) {

  const int number_of_coords = 3;      // dimension
  const int number_of_satellites = 27; // satellites per file
  const int number_of_input_variables =
      14; // these are written in the virt-sat files

  // read the name of the VirtSat filename containing the best satellite match
  // and the respective satellite number
  ...
  cout << "---> Extracting data for satellite No "
       << satellite_number_best_match << " from file "
       << "'" << VirtualSatelliteFilenameFound << "'" << " containing "
       << line_count << " lines with data" << endl;

  // the input file form which we will read data for satellite number
  // 'satellite_number_best_match'
  const int satellite_number_in_file =
      satellite_number_best_match; // satellite number for which we
                                   // extract data from input file
  const int number_of_data_rows =
      line_count; // define the number of rows (time records)

  // misc definitions
  const int number_of_elements_per_row =
      number_of_satellites * number_of_input_variables;
  ...
  float* temp_array = (float*)malloc(
      number_of_data_rows * number_of_elements_per_row * sizeof(float));
  ...
  // skip the first 'number_of_satellites' rows containing the (x,y,z)
  ...
  for (i = 0; i < number_of_data_rows; i++) {           // loop row
    for (j = 0; j < number_of_elements_per_row; j++) {  // loop column
      ...
      //            temp_array[i][j]=temp_array_element;                     //
      //            store it in temp_array
  ...
  for (i = 0; i < number_of_data_rows; i++) { // loop time records
    ...
    // select from each row the 14 input vars corresponding to the selected
    // satellite
    ...
    V_tot_e[i] = sqrt(Vx_tot_e[i] * Vx_tot_e[i] + Vy_tot_e[i] * Vy_tot_e[i] +
                      Vz_tot_e[i] * Vz_tot_e[i]);
    V_tot_i[i] = sqrt(Vx_tot_i[i] * Vx_tot_i[i] + Vy_tot_i[i] * Vy_tot_i[i] +
                      Vz_tot_i[i] * Vz_tot_i[i]);
    // accelerations
    if (i > 0)
      i1 = i - 1;
    ...
    a_tot_e[i] = sqrt(ax_tot_e[i] * ax_tot_e[i] + ay_tot_e[i] * ay_tot_e[i] +
                      az_tot_e[i] * az_tot_e[i]);
    a_tot_i[i] = sqrt(ax_tot_i[i] * ax_tot_i[i] + ay_tot_i[i] * ay_tot_i[i] +
                      az_tot_i[i] * az_tot_i[i]);
  ...
  fprintf(output_file, "%s",
          "ax_tot_e     ay_tot_e     az_tot_e     ax_tot_i     ay_tot_i     "
          "az_tot_i");
  ...
  for (i = 0; i < number_of_data_rows; i++) { // loop time records
    ...
    fprintf(output_file, "%E %E %E %E %E %E %E %E %E %E %E %E %E %E ", Bx[i],
            By[i], Bz[i], Ex[i], Ey[i], Ez[i], Jx_tot_e[i], Jy_tot_e[i],
            Jz_tot_e[i], Jx_tot_i[i], Jy_tot_i[i], Jz_tot_i[i], rho_tot_e[i],
            rho_tot_i[i]);
    fprintf(output_file, "%E %E %E %E %E %E ", Vx_tot_e[i], Vy_tot_e[i],
            Vz_tot_e[i], Vx_tot_i[i], Vy_tot_i[i], Vz_tot_i[i]);
    fprintf(output_file, "%E %E ", B[i], E[i]);
    fprintf(output_file, "%E %E ", V_tot_e[i], V_tot_i[i]);
    fprintf(output_file, "%E %E %E %E %E %E ", ax_tot_e[i], ay_tot_e[i],
            az_tot_e[i], ax_tot_i[i], ay_tot_i[i], az_tot_i[i]);
    fprintf(output_file, "%E %E ", a_tot_e[i], a_tot_i[i]);
    fprintf(output_file, "\n", "");
  }
  ...
  FILE* output_file_Jx_tot_e_minmax =
      fopen("output_filename_Jx_tot_e_minmax.txt", "w");
  FILE* output_file_Jy_tot_e_minmax =
      fopen("output_filename_Jy_tot_e_minmax.txt", "w");
  FILE* output_file_Jz_tot_e_minmax =
      fopen("output_filename_Jz_tot_e_minmax.txt", "w");
  FILE* output_file_Jx_tot_i_minmax =
      fopen("output_filename_Jx_tot_i_minmax.txt", "w");
  FILE* output_file_Jy_tot_i_minmax =
      fopen("output_filename_Jy_tot_i_minmax.txt", "w");
  FILE* output_file_Jz_tot_i_minmax =
      fopen("output_filename_Jz_tot_i_minmax.txt", "w");
  FILE* output_file_rho_tot_e_minmax =
      fopen("output_filename_rho_tot_e_minmax.txt", "w");
  FILE* output_file_rho_tot_i_minmax =
      fopen("output_filename_rho_tot_i_minmax.txt", "w");
  FILE* output_file_Vx_tot_e_minmax =
      fopen("output_filename_Vx_tot_e_minmax.txt", "w");
  FILE* output_file_Vy_tot_e_minmax =
      fopen("output_filename_Vy_tot_e_minmax.txt", "w");
  FILE* output_file_Vz_tot_e_minmax =
      fopen("output_filename_Vz_tot_e_minmax.txt", "w");
  FILE* output_file_Vx_tot_i_minmax =
      fopen("output_filename_Vx_tot_i_minmax.txt", "w");
  FILE* output_file_Vy_tot_i_minmax =
      fopen("output_filename_Vy_tot_i_minmax.txt", "w");
  FILE* output_file_Vz_tot_i_minmax =
      fopen("output_filename_Vz_tot_i_minmax.txt", "w");
  ...
  FILE* output_file_V_tot_e_minmax =
      fopen("output_filename_V_tot_e_minmax.txt", "w");
  FILE* output_file_V_tot_i_minmax =
      fopen("output_filename_V_tot_i_minmax.txt", "w");
  FILE* output_file_ax_tot_e_minmax =
      fopen("output_filename_ax_tot_e_minmax.txt", "w");
  FILE* output_file_ay_tot_e_minmax =
      fopen("output_filename_ay_tot_e_minmax.txt", "w");
  FILE* output_file_az_tot_e_minmax =
      fopen("output_filename_az_tot_e_minmax.txt", "w");
  FILE* output_file_ax_tot_i_minmax =
      fopen("output_filename_ax_tot_i_minmax.txt", "w");
  FILE* output_file_ay_tot_i_minmax =
      fopen("output_filename_ay_tot_i_minmax.txt", "w");
  FILE* output_file_az_tot_i_minmax =
      fopen("output_filename_az_tot_i_minmax.txt", "w");
  FILE* output_file_a_tot_e_minmax =
      fopen("output_filename_a_tot_e_minmax.txt", "w");
  FILE* output_file_a_tot_i_minmax =
      fopen("output_filename_a_tot_i_minmax.txt", "w");
  // write
...
// user inputs
#include "user_inputs_point.h"
  // find the virtual satellite which is closest to our specified (x,y,z) point
  find_virtual_sat(x_satellite_user, y_satellite_user, z_satellite_user, nproc);
