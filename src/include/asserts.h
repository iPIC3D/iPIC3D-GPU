...
//   ((void) ((e) ? (void)0 : dassert_printf_fileLine(#e, __FILE__, __LINE__,
//   __func__,##args)))
#else // ifdef USE_GCC_OPTIMIZATION
// optimized version of preceding
// #define assert(e) \
// (__builtin_expect(!(e), 0) ? assert_fileLine (#e, __FILE__, __LINE__) :
// (void)0)
#define dassert_(e)                                                            \
  (__builtin_expect(!(e), 0)                                                   \
       ? dassert_fileLine(#e, __FILE__, __LINE__, __func__)                    \
       : (void)0)
// #define dassert_printf(e, args...)  \
//   (__builtin_expect(!(e), 0) ? dassert_printf_fileLine (#e, __FILE__,
//   __LINE__, __func__,##args) : (void)0)
...
#define assert_not_almost_eq(lhs, rhs, tol)                                    \
  (fcmp(lhs, rhs, tol) ? (void)0                                               \
                       : assert_error(__FILE__, __LINE__, __func__,            \
                                      " !=~= ", #lhs, #rhs, lhs, rhs))
#define assert_almost_eq(lhs, rhs, tol)                                        \
  (builtin_expect(fcmp(lhs, rhs, tol), 0)                                      \
       ? assert_error(__FILE__, __LINE__, __func__, " =~= ", #lhs, #rhs, lhs,  \
                      rhs)                                                     \
       : (void)0)
...
#define assert_divides(lhs, rhs)                                               \
  (builtin_expect(rhs % lhs, 0)                                                \
       ? assert_error(__FILE__, __LINE__, __func__, "(divides)", #lhs, #rhs,   \
                      lhs, rhs)                                                \
       : (void)0)
...
#define assert_op(op, lhs, rhs)                                                \
  (builtin_expect(!(lhs op rhs), 0)                                            \
       ...
       : (void)0)
// these implementations are much faster than if using // std::isnan(a) and
// std::isinf(a)// #include <float.h> // need for DBL_MAX if invoking
// assert_isfinite// check that the number is between -inf and inf// (so that
// this will work even in case -ffast-math is set)
#define assert_isfinite(a)                                                     \
  (builtin_expect(!(a >= -DBL_MAX && a <= DBL_MAX), 0)                         \
       ? (void)(printf(                                                        \
                ...
                abort())                                                       \
       : (void)0)
// // if -funsafe-math-optimizations is set this will fail to detect// nan for
// old gcc compilers. We could work around this by// implementing assert_isnum
// in an object module compiled without// -funsafe-math-optimizations, but this
// would incur unacceptable// function call overhead. So the user will only get
// checks// against nan for modules not compiled with this option.//
#define report_nan(a)                                                          \
