...
#ifndef INTERFACE_FLUID_H
#define INTERFACE_FLUID_H
...
private:
  ...
  // Eventually we will interpolate from FLUID grid to iPIC3D grid based on
  // coordinates
  ...
  void InitDataStorage() {
    for (int i = 0; i < 3; i++) {
      ...
      cout << "XYZ_D[i] = " << XYZ_D[i] << ",  nIJK_D[i] = " << nIJK_D[i]
           << endl;
      if (nIJK_D[i] == 0) {
        nIJK_D[i] = 1;
      ...
      if (myReadFile.is_open()) {
        while (!myReadFile.eof()) {
          ...
          for (int iDim = 0; iDim < 3; iDim++) {
            ijk_D[iDim] = 0;
            if (nIJK_D[iDim] > 1)
              ijk_D[iDim] = floor((xyz_D[iDim] - INxRange_I[iDim * 2] -
                                   0.5 * INgridDx_D[iDim]) /
                                      INgridDx_D[iDim] +
                                  0.5);
          }
...
public:
  // constructor
  InterfaceFluid() { InitFluid(); };

  void InitFluid() {
    cout << "Constructor::InterfaceFluid" << endl;
    cout.flush();
  ...
  void setGlobalStartIndex(VCtopology3D* vct) {
    StartIdx_D[0] =
        vct->getCoordinates(0) * getFluidNxc() / (double)vct->getXLEN();
    StartIdx_D[1] =
        vct->getCoordinates(1) * getFluidNyc() / (double)vct->getYLEN();
    StartIdx_D[2] =
        vct->getCoordinates(2) * getFluidNzc() / (double)vct->getZLEN();

    cout << " setGlobalStartIndex :: " << StartIdx_D[0] << ", " << StartIdx_D[1]
         << ", " << StartIdx_D[2] << endl;
    cout << " setGlobalStartIndex :: " << StartIdx_D[0] << ", " << StartIdx_D[1]
         << ", " << StartIdx_D[2] << endl;
  };
  ...
  inline double getFluidNxc() { return (nIJK_D[0] - 2 * NG); };
  ...
  inline double getFluidUx(const int i, const int j, const int k,
                           const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
    return (State_GV[ig][jg][kg][iUx]);
  };
  inline double getFluidUy(const int i, const int j, const int k,
                           const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
    return (State_GV[ig][jg][kg][iUy]);
  };
  inline double getFluidUz(const int i, const int j, const int k,
                           const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
  ...
  inline double getFluidUthx(const int i, const int j, const int k,
                             const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
  ...
  inline double getFluidUthy(const int i, const int j, const int k,
                             const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
    return (sqrt(State_GV[ig][jg][kg][iP] / State_GV[ig][jg][kg][iRho]));
  ...
  inline double getFluidUthz(const int i, const int j, const int k,
                             const int is) {
    int ig, jg, kg;
    getGlobalIndex(i, j, k, &ig, &jg, &kg);
    // return(sqrt(State_GV[ig][jg][kg][iP]/State_GV[ig][jg][kg][iRho]));
  ...
  inline double getFluidRhoCenter(const int ii, const int jj, const int kk,
                                  const int is) {
    // is : index specis
    int i, j, k;
  ...
  inline void setFluidFieldsCenter(double* Ex, double* Ey, double* Ez,
    ...
    if (FluidModel == "mhd") {
      // cout<<"mhd: E = - UxB"<<endl;
      (*Ex) = State_GV[i][j][k][iUz] * State_GV[i][j][k][iBy] -
              State_GV[i][j][k][iUy] * State_GV[i][j][k][iBz];
      (*Ey) = State_GV[i][j][k][iUx] * State_GV[i][j][k][iBz] -
              State_GV[i][j][k][iUz] * State_GV[i][j][k][iBx];
      (*Ez) = State_GV[i][j][k][iUy] * State_GV[i][j][k][iBx] -
              State_GV[i][j][k][iUx] * State_GV[i][j][k][iBy];
  ...
  void ReadIdlAscii() {
    ...
    // Helping variable to set a max waiting time when looking for the data
    // header file.
    ...
    if (myReadFile.is_open()) {
      ...
      for (int i = 0; i < 6; i++) {
        myReadFile >> INxRange_I[i];
      }
      myReadFile >> TempString;
      for (int i = 0; i < 3; i++) {
        myReadFile >> INplotDx_D[i];
      }
      for (int i = 0; i < 3; i++) {
        myReadFile >> INgridDx_D[i];
      }
      for (int i = 0; i < 4; i++) {
        myReadFile >> TempString;
      }
      ...
      for (int i = 0; i < (INnPlotvar + INnEqpar); i++) {
        myReadFile >> INVarName_V[i];
      }
      ...
      for (int i = 0; i < 3; i++) {
        myReadFile >> INnRoot_D[i];
      }
      myReadFile >> TempString;
      for (int i = 0; i < 3; i++) {
        myReadFile >> INnIJK_D[i];
      }
      /*
        myReadFile >> TempString;
        while (!myReadFile.eof()) {
