...
int main(int argc, char** argv) {
  ...
  for (int si = 0; si < ns; si++) {
    ...
    dataset_id = H5Dopen2(file_id, ss.str().c_str(), H5P_DEFAULT); // HDF 1.8.8
    status = H5Dread(dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     &pclz); // cout << "pclz=" << pclz<<endl;
    status = H5Dclose(dataset_id);

    if ((pclx * pcly * pclz * nxc * nyc * nzc) > maxPcl)
      maxPcl = pclx * pcly * pclz * nxc * nyc * nzc;
  }
  ...
  const int len = sizeof(procID) / sizeof(int);
  cout << "process number = " << len << endl;
  int i_cycle = 120000;
  stringstream cc;

  cc.str("");
  cc << "upstream_" << upstreamXmin << "_" << upstreamXmax << "species" << si
     << "_cyc" << i_cycle << ".vtk";
  ofstream upstreamfile(cc.str().c_str());

  cc.str("");
  cc << "shock_" << shockXmin << "_" << shockXmax << "species" << si << "_cyc"
     << i_cycle << ".vtk";
  ofstream shockfile(cc.str().c_str());

  cc.str("");
  cc << "downstream_" << downstreamXmin << "_" << downstreamXmax << "species"
     << si << "_cyc" << i_cycle << ".vtk";
  ...
  */
  ...
  for (int procid = 0; procid < len; procid++) {
    ...
    for (int i = 0; i < nop; i++) {
      if (X[i] >= upstreamXmin && X[i] <= upstreamXmax) {
        ...
        downstreamfile << X[i] << " " << Y[i] << " " << Z[i] << " " << U[i]
                       << " " << V[i] << " " << W[i] << " " << Q[i] << "\n";
  ...
  cout << "upstreamnop =  " << upstreamnop
       << ", upstreamVxmin =  " << upstreamVxmin
       << ", upstreamVxmax =  " << upstreamVxmax
       << ", upstreamVymin =  " << upstreamVymin
       << ", upstreamVymax =  " << upstreamVymax
       << ", upstreamVzmin =  " << upstreamVzmin
       << ", upstreamVzmax =  " << upstreamVzmax << endl;

  cout << "shocknop =  " << shocknop << ", shockVxmin =  " << shockVxmin
       << ", shockVxmax =  " << shockVxmax << ", shockVymin =  " << shockVymin
       << ", shockVymax =  " << shockVymax << ", shockVzmin =  " << shockVzmin
       << ", shockVzmax =  " << shockVzmax << endl;

  cout << "downstreamnop =  " << downstreamnop
       << ", downstreamVxmin =  " << downstreamVxmin
       << ", downstreamVxmax =  " << downstreamVxmax
       << ", downstreamVymin =  " << downstreamVymin
       << ", downstreamVymax =  " << downstreamVymax
       << ", downstreamVzmin =  " << downstreamVzmin
       << ", downstreamVzmax =  " << downstreamVzmax << endl;
