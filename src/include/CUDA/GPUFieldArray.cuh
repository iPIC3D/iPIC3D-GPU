...
// ---------------------------------------------------------------------------
template<typename T>
class GPUFieldArray3Impl
{
public:

    // ---- construction / destruction ----

    GPUFieldArray3Impl() : d_ptr_(nullptr), S3_(0), S2_(0), S1_(0), size_(0), owning_(true) {}
    ...
    static GPUFieldArray3Impl wrapDevice(T* devPtr, size_t s3, size_t s2, size_t s1)
    {
    ...
    {
        cudaErrChk(cudaMemcpy(d_ptr_, h_ptr, size_ * sizeof(T), cudaMemcpyHostToDevice));
    }

    /** Copy entire array from device to host (arr3_double flat buffer). */
    void copyToHost(T* h_ptr) const
    {
        cudaErrChk(cudaMemcpy(h_ptr, d_ptr_, size_ * sizeof(T), cudaMemcpyDeviceToHost));
    }
    ...
    {
        cudaErrChk(cudaMemcpyAsync(d_ptr_, h_ptr, size_ * sizeof(T), cudaMemcpyHostToDevice, stream));
    }

    void copyToHostAsync(T* h_ptr, cudaStream_t stream) const
    {
        cudaErrChk(cudaMemcpyAsync(h_ptr, d_ptr_, size_ * sizeof(T), cudaMemcpyDeviceToHost, stream));
    }
    ...
    void setAll(T val, cudaStream_t stream = 0)
    {
        ...
        {
            // For non-zero, we do a small kernel launch (defined in GPUFieldArray.cu).
...
// ---------------------------------------------------------------------------
template<typename T>
class GPUFieldArray4Impl
{
public:

    GPUFieldArray4Impl() : d_ptr_(nullptr), S4_(0), S3_(0), S2_(0), S1_(0), size_(0) {}
...
public:
    ...
    GPUKrylovVectorImpl(GPUKrylovVectorImpl&& o) noexcept : d_ptr_(o.d_ptr_), size_(o.size_)
    { o.d_ptr_ = nullptr; o.size_ = 0; }
    ...
    { cudaErrChk(cudaMemcpyAsync(d_ptr_, h, size_ * sizeof(T), cudaMemcpyHostToDevice, s)); }

    void copyToHost(T* h, cudaStream_t s = 0) const
    { cudaErrChk(cudaMemcpyAsync(h, d_ptr_, size_ * sizeof(T), cudaMemcpyDeviceToHost, s)); }
