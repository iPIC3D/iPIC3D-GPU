...
void eprintf_fileLine(FILE* fptr, const char* type, const char* func,
...
#else
  snprintf(process_thread_str, 50, ", process %d", MPIdata::get_rank());
#endif
#else
#ifdef _OPENMP
  snprintf(process_thread_str, 50, ", thread %d", omp_get_thread_num());
#else
  ...
  va_list args;
  va_start(args, format);
  chars_so_far = snprintf(
      sptr, maxchars, "%s in method %s(), file %s, line %d%s:\n\t", type, func,
