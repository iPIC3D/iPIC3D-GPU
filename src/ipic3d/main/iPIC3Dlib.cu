...
int c_Solver::Init(int argc, char **argv) {
  ...
  // exchange array, small
  exchangePart = (Particles3D*) malloc(sizeof(Particles3D)*ns);
  for (int i = 0; i < ns; i++)
  {
    new(&exchangePart[i]) Particles3D(i,col,vct,grid); // a big memory reserve no longer happens here
    // release the memory of most of the AoS and all of the SoA

    if (exchangePart[i].getNOP() > 0) {
      throw std::runtime_error("Error: exchangePart[i].getNOP() > 0");
    }

    const int nop = grid->get_num_cells_rr() * col->getNpcel(i);

    exchangePart[i].get_pcl_array().clear();
    exchangePart[i].get_pcl_array().reserve(nop * 0.1); // now only the AoS array is reserved
  }

  // we don't initialize the particles here, we will do it in the initCUDA
  // and the remaining ID is not reserved


  // part = (Particles3D*) malloc(sizeof(Particles3D)*ns);
  // for (int i = 0; i < ns; i++)
  // {
  //   new(&part[i]) Particles3D(i,col,vct,grid); // memory reserve happens here, but size is 0
  // }
...
int c_Solver::initCUDA(){
  ...
  for(int i=0; i<ns; i++){ cudaErrChk(cudaStreamCreate(streams+i)); cudaErrChk(cudaStreamCreate(streams+i+ns)); stayedParticle[i] = 0; }
    ...
    for(int i=0; i<ns; i++){
      const int nop = col->getNpcel(i) * grid->get_num_cells_rr();

      // the constructor will copy particles from host to device
      // maybe we don't have enough memory for next resize, unless we use driver API
      pclsArrayHostPtr[i] = newHostPinnedObject<particleArrayCUDA>(nop * 1.2, cudaDeviceOnNode, streams[i]); // It manages the memory of the AoS array
      
      // initialize the particles, maxwellian
      exchangePart[i].maxwellianOnBuffer(EMf, pclsArrayHostPtr[i]->getpcls(), nop);
      pclsArrayHostPtr[i]->setNOE(nop);
      pclsArrayCUDAPtr[i] = pclsArrayHostPtr[i]->copyToDevice();

      
      cudaErrChk(cudaHostRegister(exchangePart[i].get_pcl_array().getList(), exchangePart[i].get_pcl_array().capacity() * sizeof(SpeciesParticle), cudaHostRegisterDefault));
  ...
  for(int i=0; i<ns; i++){
    moverParamHostPtr[i] = newHostPinnedObject<moverParameter>(exchangePart+i, pclsArrayCUDAPtr[i], departureArrayCUDAPtr[i], hashedSumArrayCUDAPtr[i]);
...
int c_Solver::deInitCUDA(){
  ...
  { // unregister the pinned mem
    for (int i = 0; i < ns; i++) {
      cudaErrChk(cudaHostUnregister(exchangePart[i].get_pcl_array().getList()));
...
int c_Solver::cudaLauncherAsync(const int species){
  ...
  if(x > exchangePart[species].get_pcl_list().capacity()){
    // unregister the list
    cudaErrChk(cudaHostUnregister(exchangePart[species].get_pcl_array().getList()));
    // expand the host array
    auto pclArray = exchangePart[species].get_pcl_arrayPtr();
  ...
  cudaErrChk(cudaMemcpyAsync(exchangePart[species].get_pcl_array().getList(), exitingArrayHostPtr[species]->getArray(), 
                              x*sizeof(SpeciesParticle), cudaMemcpyDefault, streams[species+ns]));
  exchangePart[species].get_pcl_array().setSize(x);
...
bool c_Solver::ParticlesMover()
{
  ...
  for(int i=0; i<ns; i++){
    ...
    auto incomingParticleNum = exchangePart[i].getNOP();

    // now the host array contains the entering particles
    if((incomingParticleNum + stayedParticle[i]) >= pclsArrayHostPtr[i]->getSize()){ // not enough size, expand the device array size
      pclsArrayHostPtr[i]->expand((incomingParticleNum + stayedParticle[i]) * 1.5, streams[i]);
    ...
    cudaErrChk(cudaMemcpyAsync(pclsArrayHostPtr[i]->getpcls() + stayedParticle[i], 
              (void*)&(exchangePart[i].get_pcl_list()[0]), 
              incomingParticleNum*sizeof(SpeciesParticle),
              cudaMemcpyDefault, streams[i]));
    // update counter
    pclsArrayHostPtr[i]->setNOE(stayedParticle[i] + incomingParticleNum); 
    ...
    momentKernelNew<<<getGridSize(incomingParticleNum, 128), 128, 0, streams[i] >>>
