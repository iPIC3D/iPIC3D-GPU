...
/**
 * @brief Scatter an external device-side AoS staging buffer into the solver SoA
 * layout.
 *
...
template <>
__host__ void particleArraySoAView<cudaParticleType, 4>::borrowFrom(
    particleArrayCUDA* pclArray) {
  if (owning) {
    freeMemory();
    owning = false;
  }
