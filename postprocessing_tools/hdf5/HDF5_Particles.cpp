...
int main(int argc, char** argv) {
  ...
  // read NsTestPart
  //    int NsTestPart;
  //    dataset_id = H5Dopen2(file_id, "/collective/NsTestPart", H5P_DEFAULT);
  //    // HDF 1.8.8 status = H5Dread(dataset_id, H5T_NATIVE_INT, H5S_ALL,
  //    H5S_ALL, H5P_DEFAULT,&NsTestPart); status = H5Dclose(dataset_id);
  ...
  for (int i_cycle = start_cycle; i_cycle < (end_cycle + 1);
    ...
    for (int si = 0; si < ns; si++) {
      ...
      cc.str("");
      cc << "/particles/species_" << (si) << "/x/cycle_"
      ...
      cc.str("");
      cc << "species" << (si) << "_cycle" << i_cycle << "_proc" << procID
         << ".vtk";
