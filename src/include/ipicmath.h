...
inline int pow2rounddown(int x) {
  assert(x >= 0);
  // if (x < 0)
  //     return 0;
...
inline void sample_maxwellian(double& u, double& v, double& w, double ut,
                              double vt, double wt) {
...
inline void sample_maxwellian(double& u, double& v, double& w, double ut,
                              double vt, double wt, std::mt19937_64& rng) {
...
template <typename T> inline T modulo(T x, T L, T Linv) {
  return x - floor(x * Linv) * L;
}

inline cudaParticleType modulo(cudaParticleType x, cudaCommonType L,
                               cudaCommonType Linv) {
  return x - floor(x * Linv) * L;
}
