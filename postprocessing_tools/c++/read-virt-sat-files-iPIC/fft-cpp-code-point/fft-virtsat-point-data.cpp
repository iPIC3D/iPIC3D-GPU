/***************************************************************************
 ...
 and the x-axis is frequency in radians/wci which are the frequencies
 correspoinding to the FFT elements. Then this frequency is scaled - see the
 Note below.
 ...
 fft-virtsat-point.cpp -lfftw3 -lm" or if you install FFTW3 from source adjust
 ...
 http://opencv-code.com/Using_the_Fast_Fourier_Transform_Library_FFTW
    ...
    Note                 : The following quantities are needed for scaling the
 FFT output x-axis
                           ...
                           The FFT contains information between 0 and fs, where
 fs is the sample frequency. The sampling frequency is the frequency rate
 between the adjacent samples. It is calculated as fs=1/delta_T. In the case of
 iPIC3D delta_T=wci*dt So if the frequency axis is plotted as (0:(M-1))/M it is
 actually f/fs what is plotted over X-axis. If we multiply by 2Pi/delta_T, e.g.
 [(0:(M-1))/M]*[2Pi/delta_T] we get it in radians/wci. If we multiply by 2Pi/dt,
 e.g. [(0:(M-1))/M]*[2Pi/dt] we get it in radians.
...
int main(int argc, char* argv[]) {
  ...
  int NFFT = istop - istart;   // number of data elements (data window) on which
                               // we perform the FFTs
  ...
  fftw_plan plan_Bx =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_Bx, FFTout_Bx, FFTW_ESTIMATE);
  fftw_plan plan_By =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_By, FFTout_By, FFTW_ESTIMATE);
  fftw_plan plan_Bz =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_Bz, FFTout_Bz, FFTW_ESTIMATE);
  fftw_plan plan_Ex =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_Ex, FFTout_Ex, FFTW_ESTIMATE);
  fftw_plan plan_Ey =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_Ey, FFTout_Ey, FFTW_ESTIMATE);
  fftw_plan plan_Ez =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_Ez, FFTout_Ez, FFTW_ESTIMATE);
  fftw_plan plan_Jx_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jx_tot_e, FFTout_Jx_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Jy_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jy_tot_e, FFTout_Jy_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Jz_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jz_tot_e, FFTout_Jz_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Jx_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jx_tot_i, FFTout_Jx_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_Jy_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jy_tot_i, FFTout_Jy_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_Jz_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Jz_tot_i, FFTout_Jz_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_rho_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_rho_tot_e, FFTout_rho_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_rho_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_rho_tot_i, FFTout_rho_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_Vx_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vx_tot_e, FFTout_Vx_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Vy_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vy_tot_e, FFTout_Vy_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Vz_tot_e = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vz_tot_e, FFTout_Vz_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_Vx_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vx_tot_i, FFTout_Vx_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_Vy_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vy_tot_i, FFTout_Vy_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_Vz_tot_i = fftw_plan_dft_r2c_1d(
      NFFT, FFTin_Vz_tot_i, FFTout_Vz_tot_i, FFTW_ESTIMATE);
  fftw_plan plan_B =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_B, FFTout_B, FFTW_ESTIMATE);
  fftw_plan plan_E =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_E, FFTout_E, FFTW_ESTIMATE);
  fftw_plan plan_V_tot_e =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_V_tot_e, FFTout_V_tot_e, FFTW_ESTIMATE);
  fftw_plan plan_V_tot_i =
      fftw_plan_dft_r2c_1d(NFFT, FFTin_V_tot_i, FFTout_V_tot_i, FFTW_ESTIMATE);
  ...
  for (int i = istart; i < istop + 1; i++) { // loop time records
    ...
    n0 = n0 + rho_tot_i[i] -
         rho_tot_e[i]; // '-' for rho_electron because of their negative charge
  ...
  /* compute the reference frequencies
     ...
     wci - ion cyclotron frequency     : wci=charge*b0/m_ion. In iPIC3D
     charge/m_ion=1 --> wci=b0 wce - electron cyclotron frequency:
     wce=wci*mass_ratio wlh - lower hybrid frequency      : wsm - square mass
     ratio frequency :
     ...
     Seems like taking the first record only fits best the results from the
     matlab FFT code, although taking averaged vaules is the logical way to it.
   */

  /*    // compute reference frequencies
      n0 = n0*4.0*3.1415927/2.0/(istop-istart);                             //
  compute average and *4Pi in order to convert to real charge density ***
  something is wrong - gives negative close to zero: - matlab code is
  n0=mean(rhoi-rhoe)/2;
  //    n0 = ( rho_tot_i[istart] - rho_tot_e[istart] )*4.0*3.1415927/2.0;     //
  for first time record only b0 = b0/(istop-istart); // compute the average
  //    b0=B0x;
      wci = b0;                                                             //
  check what frequency is equal to below - matlab code is
  wci=b0=sqrt(mean(bx.^2+by.^2+bz.^2)); wpi = sqrt(fabs(n0)); // use with the
  averaged n0 above
  //    wpi = sqrt(n0);
      wce = wci*mratio;
  ...
  n0 = (rho_tot_i[istart] - rho_tot_e[istart]) * 4.0 * 3.1415927 /
       2.0; // '-' for rho_electron because of their negative charge
  b0 = sqrt(Bx[istart] * Bx[istart] + By[istart] * By[istart] +
            Bz[istart] * Bz[istart]);
  wci = B0x; // B0x;
  wpi = sqrt(n0);
  wce = wci * mratio;
  ...
  // write out the data
  for (int i = 0; i < M;
       i++) { // loop time records - time units are wci*t (cyclotron periods)
    frequency_over_Fs = 1.0 * i / M;
    frequency_radians =
        frequency_over_Fs; // the x-axis is f/Fs - see the note in the beginning
    frequency_radians =
        2.0 * 3.14159 * frequency_over_Fs /
        dt; // the x-axis is [rad] - see the note in the beginning
