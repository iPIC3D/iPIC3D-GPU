...
int main(void) {
  ...
  for (i = 0; i < NUM_MESH_CELLS; i++) {
    ...
    ALIGNED(mesh_cells[i].x);
    ALIGNED(mesh_cells[i].y);
    ALIGNED(mesh_cells[i].z);
    ALIGNED(mesh_cells[i].u);
    ALIGNED(mesh_cells[i].v);
    ALIGNED(mesh_cells[i].w);
...
#pragma omp for schedule(static, 1) nowait
    for (i = 0; i < (NUM_THREADS_PER_MESH_CELL * NUM_MESH_CELLS); i++) {
      ...
      // cidx, my_start_pidx, my_num_pcls); fflush(stdout);

      // time[1] = time_sec();

#if 1
      /*
       * Divide particles into blocks of particles and do everything for one
       * block completely
       *
       * Assumption: We always process complete blocks, i.e., arrays have to be
       * padded
       */
...
#pragma ivdep
        ...
        for (j = 0, pidx = cpidx; j < BLOCKSIZE; j++, pidx++) {
          ...
          avg[0] =
              (t[0] + (t[1] * Om[2] - t[2] * Om[1] + udotOm * Om[0])) * denom;
          avg[1] =
              (t[1] + (t[2] * Om[0] - t[0] * Om[2] + udotOm * Om[1])) * denom;
          avg[2] =
              (t[2] + (t[0] * Om[1] - t[1] * Om[0] + udotOm * Om[2])) * denom;
...
#pragma omp parallel
  {
    ...
    for (i = 0; i < 8; i++) {
      weights[i] = ptr =
          (double*)_mm_malloc(sizeof(double) * num_pcls, ALIGNMENT);
      if (!ptr) {
        printf("%d: Malloc failed.\n", __LINE__);
        exit(1);
      }
    ...
    field_components_0 = field_components[0];
    ALIGNED(field_components_0);
    field_components_1 = field_components[1];
    ALIGNED(field_components_1);
    field_components_2 = field_components[2];
    ALIGNED(field_components_2);
    field_components_3 = field_components[3];
    ALIGNED(field_components_3);
    field_components_4 = field_components[4];
    ALIGNED(field_components_4);
    field_components_5 = field_components[5];
    ALIGNED(field_components_5);
    field_components_6 = field_components[6];
    ALIGNED(field_components_6);
    field_components_7 = field_components[7];
    ALIGNED(field_components_7);
...
// For all particles
// Note: Might be better to do this in batches of particles for better cache
// re-use
#pragma ivdep
    ...
    time[4] = time_sec();
    printf("tid: %d  malloc: %f  weights: %f BE: %f rest: %f\n", tid,
           time[1] - time[0], time[2] - time[1], time[3] - time[2],
           time[4] - time[3]);
  }
  time[1] = time_sec();
