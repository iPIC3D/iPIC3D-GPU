...
RestartMeshMetadata makeDestinationMeshForRestart(
    const RestartMeshMetadata& source,
...
void validateDestinationGrid(const RestartRemapPlan& plan,
                             const Grid* grid)
{
...
{
    if (backend == "adios2") return "ADIOS2";
    if (backend == "hdf5") return "HDF5";
    return backend;
}

void printRestartReadSignature(const char* objectName,
                               const std::string& backend,
                               const char* remapKind,
                               int cycle)
{
    std::cout << "[*] " << objectName << " Restarting ("
              << restartBackendLabel(backend) << ") with "
              << remapKind << " remap from cycle label: "
              << cycle << std::endl;
}
...
void appendParticleSpansForCopy(
    ...
    std::vector<ParticleReadSpan>& spans)
{
    ...
    for (int iz = copy.sourceLocal.z.begin;
         iz < copy.sourceLocal.z.end; ++iz) {
        for (int iy = copy.sourceLocal.y.begin;
             iy < copy.sourceLocal.y.end; ++iy) {
            const int firstCell =
                activeCellLinearIndex(copy.sourceLocal.x.begin, iy, iz,
                                      activeCellDims);
            const int lastCell =
                activeCellLinearIndex(copy.sourceLocal.x.end - 1, iy, iz,
                                      activeCellDims);

            const long long begin = cellOffsets[firstCell];
            const long long end = static_cast<long long>(cellOffsets[lastCell])
                                + cellCounts[lastCell];
...
{
    long long total = 0;
    for (const ParticleReadSpan& span : spans) total += span.count;
    return total;
}

void resizeParticleVectors(
    long long totalParticles,
...
void copyChunkToParticleVectors(
    const RestartParticleChunkBuffers& buffers,
    size_t count,
...
public:
                      ...
                      RestartParticleCellLayout& layout)
    {
        ...
        auto offsetsVar =
            io.InquireVariable<int>("part" + spec + "CellOffsets");
        auto countsVar =
            io.InquireVariable<int>("part" + spec + "CellCounts");
...
public:
                  ...
                  arr3_double dst)
    {
        ...
        buffer_.resize(boxValueCount(copy.sourceLocal));
        const herr_t status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE,
...
public:
    void readMetadata(hid_t file_id,
                      int species,
                      int last_cycle,
                      const RestartCellCopy& copy,
                      const RestartMeshMetadata& sourceMesh,
                   ...
                   RestartParticleChunkBuffers& buffers)
    {
        ...
        readDoubleSelection(file_id, speciesPath + "/u/" + cycle,
                            offset, count, buffers.u.data());
        readDoubleSelection(file_id, speciesPath + "/v/" + cycle,
                            offset, count, buffers.v.data());
        readDoubleSelection(file_id, speciesPath + "/w/" + cycle,
                            offset, count, buffers.w.data());
        readDoubleSelection(file_id, speciesPath + "/q/" + cycle,
                            offset, count, buffers.q.data());
        readDoubleSelection(file_id, speciesPath + "/x/" + cycle,
                            offset, count, buffers.x.data());
        readDoubleSelection(file_id, speciesPath + "/y/" + cycle,
...
private:
    void readIntDataset(hid_t file_id,
                        const std::string& datasetPath,
                        std::vector<int>& dst)
    {
        hid_t dataset_id = H5Dopen2(file_id, datasetPath.c_str(), H5P_DEFAULT);
        ...
        const herr_t status = H5Dread(dataset_id, H5T_NATIVE_INT,
    ...
    template <typename T>
    void readSelection(hid_t file_id,
                       const std::string& datasetPath,
                       hid_t nativeType,
                       long long offset,
                       size_t count,
                       T* dst)
    {
        ...
        const herr_t status = H5Dread(dataset_id, nativeType, mem_space,
                                      file_space, H5P_DEFAULT, dst);
        ...
        if (status < 0) {
            eprintf("ERROR: could not read HDF5 restart particle selection "
                    "from %s", datasetPath.c_str());
        }
    }

    void readDoubleSelection(hid_t file_id,
                             const std::string& datasetPath,
                             long long offset,
                             size_t count,
                             double* dst)
    {
        readSelection(file_id, datasetPath, H5T_NATIVE_DOUBLE,
                      offset, count, dst);
    }
...
struct RestartReadContext {
    RestartReadContext(const std::string& restartDir_,
        ...
        : checkpoint(std::move(checkpoint_)),
          ...
          restartDir(restartDir_),
          backend(backend_),
          lastCycle(lastCycle_),
          rank(vct->getCartesian_rank()),
          xlen(vct->getXLEN()),
          ylen(vct->getYLEN()),
          zlen(vct->getZLEN()),
          nranks(vct->getNprocs())
    {}

    bool matches(const std::string& restartDir_,
                 const std::string& backend_,
                 int lastCycle_,
                 const VCtopology3D* vct) const
    {
        return restartDir == restartDir_ &&
               backend == backend_ &&
               lastCycle == lastCycle_ &&
               rank == vct->getCartesian_rank() &&
               xlen == vct->getXLEN() &&
                                          ...
                                          const Grid* grid)
{
    ...
    if (restartReadContext &&
        ...
        if (grid) restartReadContext->validateGridOnce(grid);
    ...
    std::unique_ptr<RestartReadContext> newContext(
        new RestartReadContext(restartDir, backend, lastCycle,
                               vct, std::move(checkpoint)));
    if (grid) newContext->validateGridOnce(grid);
    ...
    const Grid* grid)
{
    ...
    for (const RestartNodeCopy& copy : plan.nodeCopies()) {
        const std::string name_file =
            restartRankFilePath(context.restartDir, context.backend,
                                copy.sourceRank);

        adios2::ADIOS adios;
        adios2::IO ioField = adios.DeclareIO(
            "FieldRestartRemap" + std::to_string(copy.sourceRank));
    ...
    std::vector<ParticleReadSpan>& spans)
{
    ...
    for (const RestartCellCopy& copy : plan.cellCopies()) {
        ...
        adios2::ADIOS adios;
        adios2::IO ioParticle = adios.DeclareIO(
            "ParticleRestartMetadata" + std::to_string(copy.sourceRank));
...
void readAdiosParticlesRemapped(
    RestartReadContext& context,
    int species,
    vector_double& u, vector_double& v, vector_double& w,
    vector_double& q,
    ...
    vector_double& t)
{
    ...
    while (spanIndex < spans.size()) {
        ...
        adios2::ADIOS adios;
        adios2::IO ioParticle = adios.DeclareIO(
            "ParticleRestartRead" + std::to_string(sourceRank));
        ioParticle.SetEngine("BP5");
        adios2::Engine engineParticle =
            ioParticle.Open(name_file, adios2::Mode::Read);
    ...
    const Grid* grid)
{
    ...
    const std::string cycle_str =
        "cycle_" + std::to_string(context.lastCycle);
    ...
    for (const RestartNodeCopy& copy : plan.nodeCopies()) {
        ...
        hid_t file_id = H5Fopen(name_file.c_str(), H5F_ACC_RDONLY,
                                H5P_DEFAULT);
        if (file_id < 0) {
        ...
        context.hdf5FieldReader.readInto(file_id, "/fields/Bx/" + cycle_str,
                                         copy, plan.sourceMesh(), Bxn);
        context.hdf5FieldReader.readInto(file_id, "/fields/By/" + cycle_str,
                                         copy, plan.sourceMesh(), Byn);
        context.hdf5FieldReader.readInto(file_id, "/fields/Bz/" + cycle_str,
                                         copy, plan.sourceMesh(), Bzn);
        context.hdf5FieldReader.readInto(file_id, "/fields/Ex/" + cycle_str,
                                         copy, plan.sourceMesh(), Ex);
        context.hdf5FieldReader.readInto(file_id, "/fields/Ey/" + cycle_str,
                                         copy, plan.sourceMesh(), Ey);
        context.hdf5FieldReader.readInto(file_id, "/fields/Ez/" + cycle_str,
                                         copy, plan.sourceMesh(), Ez);
        ...
        for (int i = 0; i < ns; i++) {
            arr3_double rhoSpecies(rhons->fetch_arr4()[i],
                                   ...
                                   grid->getNYN(),
                                   grid->getNZN());
            const string dsPath = "/moments/species_" + std::to_string(i)
                                + "/rho/" + cycle_str;
            context.hdf5FieldReader.readInto(file_id, dsPath, copy,
...
void readHdf5ParticlesRemapped(
    RestartReadContext& context,
    int species,
    vector_double& u, vector_double& v, vector_double& w,
    vector_double& q,
    ...
    if (checkpoint.found) {
        if (MPIdata::get_rank() == 0) {
            std::cout << "[*] Restart checkpoint ("
                      << restartBackendLabel(checkpoint.backend)
                      << ") = restart_"
                      << checkpoint.slot
    ...
    const std::string& restartDir, int last_cycle)
{
    ...
    if (vct->getCartesian_rank() == 0) {
        printRestartReadSignature("Fields", context.backend,
                                  "active-node", last_cycle);
    }

    readAdiosFieldsRemapped(context, Bxn, Byn, Bzn, Ex, Ey, Ez,
                            rhons_, ns, grid);
    ...
    readHdf5FieldsRemapped(context, Bxn, Byn, Bzn, Ex, Ey, Ez,
                           rhons_, ns, grid);
    ...
    const std::string& restartDir, int last_cycle)
{
    ...
    if (vct->getCartesian_rank() == 0 && species_number == 0) {
        printRestartReadSignature("Particle", context.backend,
                                  "active-cell", last_cycle);
    }
