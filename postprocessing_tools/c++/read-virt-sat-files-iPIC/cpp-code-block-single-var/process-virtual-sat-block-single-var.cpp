...
////////////////////////////////////////////////////////////////////////
// Function 'find_virtual_sat_block' finds the best satellite match with closest
// position to user given (x,y,z) spatial coordinates. The result is used by
// funciton 'read_virtual_sat_block'.
////////////////////////////////////////////////////////////////////////

void find_virtual_sat_block(float x_satellite_user, float y_satellite_user,
  ...
  int satellite_number_best_match[nproc]; // later used in read-virtual-sat.cpp
                                          // to extract the corresponding data
  string
      VirtualSatelliteFilenameFoundString[nproc]; // this file contains the best
  ...
  vector<float> closest_sat_coordinates(
      number_of_coords - 1,
      1.0E10); // define 3D vector - closest satellite position to what we want
  vector<float> current_vector_difference(number_of_coords - 1, 0);
  vector<float> closest_vector_difference(
      number_of_coords - 1,
      1.0E10); // this will end up being closest to
               // the desired satellite position
  ...
  // now we have found the closest coordinates to the desired ones and search
  // through all satellites for that coordinate trace in space
  for (int iproc = 0; iproc < nproc; iproc++) { // loop over all files
    stringstream ss;
    ss << iproc;
    temp_string = "VirtualSatelliteTraces" + ss.str() + ".txt";
    ...
    // scan the file again and this time when coordinate match is found, record
    // the satellite number for later data extraction
    if (virt_sat_file) { // if the file exists
      for (i = 0; i < number_of_satellites;
           i++) { // loop over rows (number of satellites in file)
        for (j = 0; j < number_of_coords;
        ...
        if (mod_current_vector_difference <= mod_closest_vector_difference) {
          satellite_number_best_match[iSatCount] =
  ...
  // write out best satellite info in a file
  // 'BestVirtualSatelliteInfoFoundBlock.txt' to be used by read-virt-sat.cpp
  for (i = 0; i < iSatCount; i++) {
    // count the numeber of lines in 'VirtualSatelliteFilenameFoundString[i]'
    // and take only the number of data rows in it
    FILE* file_to_count_lines =
        fopen(VirtualSatelliteFilenameFoundString[i].c_str(), "r");
    int line_count = 0;
    ...
    cout << endl;
    cout << "---> Found satellite No " << satellite_number_best_match[i]
         << " in file "
         << "'" << VirtualSatelliteFilenameFoundString[i] << "'" << endl;
    cout << "---> Desired " << XYZ_block << " Sat. Coordinates "
         << original_desired_sat_coordinates0 << " "
         << original_desired_sat_coordinates1 << endl;
    cout << "---> Closest " << XYZ_block << " Sat. Coordinates "
         << closest_sat_coordinates[0] << " " << closest_sat_coordinates[1]
         << endl;
    cout << "---> Closest " << XYZ_block << " Vector Difference "
         << closest_vector_difference[0] << " " << closest_vector_difference[1]
         << endl;
    ...
    FILE* best_satellite_match_info_found =
        fopen("BestVirtualSatelliteInfoFoundBlock.txt", "a");
    fprintf(best_satellite_match_info_found, "%s %i %i",
            VirtualSatelliteFilenameFoundString[i].c_str(),
            satellite_number_best_match[i], line_count);
...
////////////////////////////////////////////////////////////////////////
// Function'read_virtual_sat_block' uses output from function
// 'find_virtual_sat_block' to extract the corresponding satellite data
////////////////////////////////////////////////////////////////////////
...
void read_virtual_sat_block(float dt) {
  ...
  FILE* file_to_count_lines =
      fopen("BestVirtualSatelliteInfoFoundBlock.txt", "r");
  int number_of_satellites_to_extract = 0;
  while (!fscanf(file_to_count_lines, "%*[^\n]%*c"))
    number_of_satellites_to_extract++;
  fseek(file_to_count_lines, 0, SEEK_SET);
  fclose(file_to_count_lines);
  ...
  // loop over all satellites from 'BestVirtualSatelliteInfoFoundBlock.txt' and
  // extract the data for each of them
  for (int isatloop = 0; isatloop < number_of_satellites_to_extract;
       isatloop++) {
    ...
    const int satellite_number_in_file =
        satellite_number_best_match; // satellite number for which we extract
    ...
    for (i = 0; i < number_of_data_rows; i++) {           // loop row
      for (j = 0; j < number_of_elements_per_row; j++) {  // loop column
        ...
        //            temp_array[i][j]=temp_array_element; // store it in
        //            temp_array
        temp_array[number_of_elements_per_row * (i - 0) + j] =
            temp_array_element;
        if ((k = fgetc(virt_sat_file)) ==
            '\n') { // if new line then goto next row
          break;
    ...
    for (i = 0; i < number_of_data_rows; i++) { // loop time records
      ...
      ielement = ielement + 1;
      rho_tot_e[i] =
          temp_array[number_of_elements_per_row * (i - 0) + ielement];
      ielement = ielement + 1;
      rho_tot_i[i] =
          temp_array[number_of_elements_per_row * (i - 0) + ielement];
      ...
      // write out - each block of data corresponds to a variable from satellite
      // 'VirtualSatelliteFilenameFound' each row of data corresponds to a
      // satellite and the columns are the cycles
...
#include "user_inputs_block_single_var.h"
  // find the trace of virtual satellites which is closest to our specified
  // (XYZ_block) line
  find_virtual_sat_block(x_satellite_user, y_satellite_user, z_satellite_user,
                         nproc, XYZ_block);
