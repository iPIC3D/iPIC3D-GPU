...
ParticleSoAHost::ParticleSoAHost(int speciesNum, CollectiveIO* col,
    ...
    : speciesNumber_(speciesNum), col_(col), vct_(vct), grid_(grid),
  ...
  if (trackParticleID_) {
    particleIDGenerator_.initialize(vct_->getCartesian_rank(),
                                    vct_->getNprocs(), speciesNumber_,
...
void ParticleSoAHost::maxwellian(Field* EMf) {
  ...
  const auto baseSequence =
      trackParticleID_ ? particleIDGenerator_.reserveHostSequenceBlock(nop)
                       : ParticleIDGenerator::counter_type{0};
  const double chargeFactor = (chargeOverMass_ / fabs(chargeOverMass_)) *
                              grid_->getVOL() / numParticlesPerCell_;
...
#pragma omp parallel
  {
    ...
    std::mt19937_64 rng(vct_->getCartesian_rank() * 31 + speciesNumber_ * 127 +
                        tid * 1049);
      ...
      for (int j = 1; j < numCellsY_ - 1; ++j)
        for (int k = 1; k < numCellsZ_ - 1; ++k) {
          ...
          const double chargePerParticle =
              chargeFactor * EMf->getRHOcs(i, j, k, speciesNumber_);

          for (int ii = 0; ii < numPclPerCellX_; ++ii)
            for (int jj = 0; jj < numPclPerCellY_; ++jj)
              for (int kk = 0; kk < numPclPerCellZ_; ++kk) {
                const int subIdx =
                    (ii * numPclPerCellY_ + jj) * numPclPerCellZ_ + kk;
                const int idx = idxBase + subIdx;

                double velX, velY, velZ;
                sample_maxwellian(velX, velY, velZ, thermalVelocityX_,
                                  thermalVelocityY_, thermalVelocityZ_,
                                  driftVelocityX_, driftVelocityY_,
                                  driftVelocityZ_, rng);
                ...
                x[idx] = (ii + .5) * (gridSpacingX_ / numPclPerCellX_) +
                         grid_->getXN(i, j, k);
                y[idx] = (jj + .5) * (gridSpacingY_ / numPclPerCellY_) +
                         grid_->getYN(i, j, k);
                z[idx] = (kk + .5) * (gridSpacingZ_ / numPclPerCellZ_) +
                         grid_->getZN(i, j, k);
                if (trackParticleID_)
                  id[idx] =
                      particleIDGenerator_.idFromSequence(baseSequence + idx);
              }
        }
  }
}
...
/**
 * @brief Populate the null-points/Taylor-Green cases with local current-driven
 * drift.
 *
...
void ParticleSoAHost::maxwellianNullPoints(Field* EMf) {
  ...
  const double chargeFactor =
      chargeSign * grid_->getVOL() / numParticlesPerCell_;
    ...
    for (int i = 1; i < numCellsX_ - 1; i++)
      for (int j = 1; j < numCellsY_ - 1; j++)
        for (int k = 1; k < numCellsZ_ - 1; k++) {
          ...
          const double signedRho =
              chargeSign * EMf->getRHOns(i, j, k, speciesNumber_);
 ...
 * Matches the ECsim reference-state initialization (at id=0 this gives global
 * uth algebraically, but preserves the mechanism for discrete numerical effects
 * and future extension to non-trivial pressure profiles).
 *
...
void ParticleSoAHost::maxwellianAmpereVaryingThermal(Field* EMf) {
  ...
  const auto baseSequence =
      trackParticleID_ ? particleIDGenerator_.reserveHostSequenceBlock(nop)
                       : ParticleIDGenerator::counter_type{0};
  const double chargeSign =
      chargeOverMass_ / fabs(chargeOverMass_); // +1 ions, -1 electrons
  const double chargeFactor =
      chargeSign * grid_->getVOL() / numParticlesPerCell_;
    ...
    for (int i = 1; i < numCellsX_ - 1; i++)
      for (int j = 1; j < numCellsY_ - 1; j++)
        for (int k = 1; k < numCellsZ_ - 1; k++) {
          ...
          const double localDriftX =
              EMf->getJxs(i, j, k, speciesNumber_) / signedRho;
          const double localDriftY =
              EMf->getJys(i, j, k, speciesNumber_) / signedRho;
          const double localDriftZ =
              EMf->getJzs(i, j, k, speciesNumber_) / signedRho;
          ...
          const double localVthX =
              (vthX2 > 0.0) ? sqrt(vthX2) : thermalVelocityX_;
          const double localVthY =
              (vthY2 > 0.0) ? sqrt(vthY2) : thermalVelocityY_;
          const double localVthZ =
              (vthZ2 > 0.0) ? sqrt(vthZ2) : thermalVelocityZ_;
          ...
          for (int ii = 0; ii < numPclPerCellX_; ++ii)
            for (int jj = 0; jj < numPclPerCellY_; ++jj)
              for (int kk = 0; kk < numPclPerCellZ_; ++kk) {
                ...
                double velX, velY, velZ;
                sample_maxwellian(velX, velY, velZ, localVthX, localVthY,
                                  localVthZ, localDriftX, localDriftY,
                                  localDriftZ, rng);
          ...
          for (int ii = 0; ii < numPclPerCellX_; ++ii)
            for (int jj = 0; jj < numPclPerCellY_; ++jj)
              for (int kk = 0; kk < numPclPerCellZ_; ++kk) {
                ...
                const double posX =
                    (ii + .5) * (gridSpacingX_ / numPclPerCellX_) +
                    grid_->getXN(i, j, k);
                const double posY =
                    (jj + .5) * (gridSpacingY_ / numPclPerCellY_) +
                    grid_->getYN(i, j, k);
                const double posZ =
                    (kk + .5) * (gridSpacingZ_ / numPclPerCellZ_) +
                    grid_->getZN(i, j, k);
...
#pragma omp for collapse(3) schedule(static)
    for (int i = 1; i < numCellsX_ - 1; i++)
      for (int j = 1; j < numCellsY_ - 1; j++)
        for (int k = 1; k < numCellsZ_ - 1; k++) {
          ...
          const double chargePerParticle =
              chargeFactor * EMf->getRHOcs(i, j, k, 0);
          ...
          for (int ii = 0; ii < numPclPerCellX_; ii++)
            for (int jj = 0; jj < numPclPerCellY_; jj++)
              for (int kk = 0; kk < numPclPerCellZ_; kk++) {
                ...
                const double totalMomentum =
                    sqrt((energy_ + 1) * (energy_ + 1) - 1);
                const double parallelVelocity =
                    totalMomentum * cos(pitchAngle_);
  ...
  if (vct_->getCartesian_rank() == 0) {
    ...
    cout << "Initialize Test Particle " << speciesNumber_
         << " with pitch angle " << pitchAngle_ << ", energy " << energy_
         << ", qom " << chargeOverMass_ << ", npcel " << nop << endl;
    cout << "------------------------------------------" << endl;
  }
}
...
#pragma omp parallel for reduction(max : localMaxVel)
  for (int idx = 0; idx < numParticles; idx++) {
    ...
    localMaxVel =
        std::max(localMaxVel, sqrt(velX * velX + velY * velY + velZ * velZ));
  }
 ...
 */
long long* ParticleSoAHost::getVelocityDistribution(int numBins,
...
#pragma omp critical
  ...
  MPI_Allreduce(MPI_IN_PLACE, histogram, numBins, MPI_LONG_LONG, MPI_SUM,
                mpiComm_);
  return histogram;
}
...
void ParticleSoAHost::sort_particles_serial() {
  ...
  Larray<double> uSorted(numParticles), vSorted(numParticles),
      wSorted(numParticles), qSorted(numParticles);
  Larray<double> xSorted(numParticles), ySorted(numParticles),
      zSorted(numParticles);
  Larray<cudaPclType_ID> idSorted;
  uSorted.resize(numParticles);
  vSorted.resize(numParticles);
  wSorted.resize(numParticles);
  qSorted.resize(numParticles);
  xSorted.resize(numParticles);
  ySorted.resize(numParticles);
  zSorted.resize(numParticles);
  if (trackParticleID_)
    idSorted.resize(numParticles);
  ...
  for (int pidx = 0; pidx < numParticles; pidx++) {
    ...
    grid_->get_safe_cell_coordinates(cellX, cellY, cellZ, x[pidx], y[pidx],
                                     z[pidx]);
  ...
  for (int pidx = 0; pidx < numParticles; pidx++) {
    ...
    uSorted[destIndex] = u[pidx];
    vSorted[destIndex] = v[pidx];
    wSorted[destIndex] = w[pidx];
    qSorted[destIndex] = q[pidx];
    xSorted[destIndex] = x[pidx];
    ySorted[destIndex] = y[pidx];
    zSorted[destIndex] = z[pidx];
    if (trackParticleID_)
      idSorted[destIndex] = id[pidx];
  }
...
void ParticleSoAHost::sort_particles_parallel(int* cellCount, int* cellOffset) {
  ...
  std::vector<std::vector<int>> threadLocalCounts(
      numThreads, std::vector<int>(totalCells, 0));
  std::vector<std::vector<int>> threadLocalOffsets(
      numThreads, std::vector<int>(totalCells, 0));
...
#pragma omp parallel
  {
    ...
    const int rangeStart =
        threadID * blockSize + (threadID < remainder ? threadID : remainder);
    const int rangeEnd =
        rangeStart + blockSize + (threadID < remainder ? 1 : 0);
    ...
    for (int pidx = rangeStart; pidx < rangeEnd; pidx++) {
      ...
      grid_->get_safe_cell_coordinates(cellX, cellY, cellZ, x[pidx], y[pidx],
                                       z[pidx]);
      int cellIndex =
          cellX * (numCellsY_ * numCellsZ_) + cellY * numCellsZ_ + cellZ;
...
void ParticleSoAHost::repopulate_particlesInfo(
    bool* doRepopulateInjection, bool* doRepopulateInjectionSide,
...
void ParticleSoAHost::openbc_particles_outflowInfo(
    bool* doOpenBC, bool* applyOpenBC, cudaCommonType* deleteBoundary,
  ...
  *doOpenBC = false;
  ...
  const bool openXleft = !vct_->getPERIODICX_P() && vct_->noXleftNeighbor_P() &&
                         bcPfaceXleft_ == OPENBCOut;
  const bool openYleft = !vct_->getPERIODICY_P() && vct_->noYleftNeighbor_P() &&
                         bcPfaceYleft_ == OPENBCOut;
  const bool openZleft = !vct_->getPERIODICZ_P() && vct_->noZleftNeighbor_P() &&
                         bcPfaceZleft_ == OPENBCOut;
  const bool openXright = !vct_->getPERIODICX_P() &&
                          vct_->noXrghtNeighbor_P() &&
                          bcPfaceXright_ == OPENBCOut;
  const bool openYright = !vct_->getPERIODICY_P() &&
                          vct_->noYrghtNeighbor_P() &&
                          bcPfaceYright_ == OPENBCOut;
  const bool openZright = !vct_->getPERIODICZ_P() &&
                          vct_->noZrghtNeighbor_P() &&
                          bcPfaceZright_ == OPENBCOut;

  if (!(openXleft || openYleft || openZleft || openXright || openYright ||
        openZright))
    return;
  ...
  *doOpenBC = true;
  ...
  deleteBoundary[0] = 0;
  deleteBoundary[1] = domainLengthX_;
  deleteBoundary[2] = 0;
  deleteBoundary[3] = domainLengthY_;
  deleteBoundary[4] = 0;
  deleteBoundary[5] = domainLengthZ_;
  openBoundary[0] = xLow;
  openBoundary[1] = xHgh;
  openBoundary[2] = yLow;
  openBoundary[3] = yHgh;
...
void ParticleSoAHost::fillExitBCFlags(bool* isExitBC) const {
  using namespace BCparticles;
  isExitBC[0] = (!vct_->getPERIODICX_P() && vct_->noXleftNeighbor_P() &&
                 bcPfaceXleft_ == EXIT);
  isExitBC[1] = (!vct_->getPERIODICX_P() && vct_->noXrghtNeighbor_P() &&
                 bcPfaceXright_ == EXIT);
  isExitBC[2] = (!vct_->getPERIODICY_P() && vct_->noYleftNeighbor_P() &&
                 bcPfaceYleft_ == EXIT);
  isExitBC[3] = (!vct_->getPERIODICY_P() && vct_->noYrghtNeighbor_P() &&
                 bcPfaceYright_ == EXIT);
  isExitBC[4] = (!vct_->getPERIODICZ_P() && vct_->noZleftNeighbor_P() &&
                 bcPfaceZleft_ == EXIT);
  isExitBC[5] = (!vct_->getPERIODICZ_P() && vct_->noZrghtNeighbor_P() &&
                 bcPfaceZright_ == EXIT);
}
...
void ParticleSoAHost::appendFromAoS(const SpeciesParticle* buffer, int count) {
  ...
  for (int idx = 0; idx < count; idx++) {
    ...
    if (trackParticleID_)
      id.push_back(buffer[idx].get_id());
  }
}
