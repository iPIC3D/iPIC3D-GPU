    ...
    for (int dim1 = 0; dim1 < dim1Size; dim1 += tile)
      for (int dim2 = 0; dim2 < dim2Size; dim2 += tile) {
        ...
        for (int i = pidx; i < nop; i += gridSize) {
          ...
          data[2] = d3[i];
          const auto sIndex =
              histogramCUDAPtr[0].getIndexTiled(data, dim, dimSize);
          if (sIndex < 0)
            continue; // out of tile range
        ...
        for (int i = threadIdx.x; i < tileSize; i += blockDim.x) {
          atomicAdd(&gHistogram[dim0 + i % tile +
                                (dim1 + i / tile % tile) * dim0Size +
                                (dim2 + i / tile / tile) * dim0Size * dim1Size],
                    sHistogram[i]);
        }
      }
}
 ...
 * @details this kernel is launched once for each histogram bin for all 3
 * histograms
