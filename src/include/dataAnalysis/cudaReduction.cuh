#ifndef CUDA_REDUCTION_CUH
#define CUDA_REDUCTION_CUH
...
__device__ void warpReduceMax(volatile T* sdata, unsigned int tid) {
  ...
  if constexpr (blockSize >= 128 && WARP_SIZE == 64)
...
/**
 * @brief Reduce the input array to the maximum value, the blockSize should be
 * 2^n and < 1024
 * @details The input array is reduced to the maximum value. But each block will
 * have the one maximum value of its area, postprocess is needed.
 *
 * @param g_idata input array, n is the size of the array
 * @param g_odata output array, the size of the array is GridSize, which means
...
/**
 * @brief Reduce the input array to one maximum value, the blockSize should be
 * small enough for 32 threads
...
__global__ void reduceMin(T* g_idata, T* g_odata, unsigned int n) {
  ...
  if (tid < WARP_SIZE)
    warpReduceMin<T, blockSize>(sdata, tid);
...
/**
 * @brief Reduce the input array to the sum, the blockSize should be 2^n and <
 * 1024
 * @details The input array is reduced to the sum. But each block will have one
 * sum value of its area, postprocess is needed.
 *
 * @param g_idata input array, n is the size of the array
 * @param g_odata output array, the size of the array is GridSize, which means
 * the number of blocks
...
__global__ void reduceSum(T* g_idata, T* g_odata, unsigned int n) {
  ...
  if (tid < WARP_SIZE)
    warpReduceSum<T, blockSize>(sdata, tid);
...
/**
 * @brief Reduce the input array to one sum value, the blockSize should be small
 * enough for 32 threads
 * @details can only be luanched by one block, and 32 threads
 *
 * @param g_idata input array, n is the size of the array
 * @param g_odata output value, the size is 1
...
enum class PostProcessType { logAdd, divide, divideEXP };

// pre and post process reduction, it can be more elegent and flexible with cuda
// extended lambda
 ...
 * @param preProcOprand Auxiliary operand consumed by the selected preprocessing
 * rule.
 * @param weight Optional per-element weights, used only when `ifWeight` is
 * true.
 */
template <typename T, unsigned int blockSize, PreProcessType preProc,
          typename U, typename V = int, bool ifWeight = false>
__global__ void reduceSumPreProcess(T* g_idata, T* g_odata, unsigned int n,
                                    U* preProcOprand, V* weight = nullptr) {
  extern __shared__ T sdata[];
  unsigned int tid = threadIdx.x;
  unsigned int i = blockIdx.x * (blockSize * 2) + tid;
  unsigned int gridSize = blockSize * 2 * gridDim.x;
  ...
  while (i < n) {
    if constexpr (ifWeight) { // weighted
      sdata[tid] +=
          preProcess<T, preProc, U>(g_idata[i], preProcOprand, i) * weight[i];
      if (i + blockSize < n)
        sdata[tid] += preProcess<T, preProc, U>(g_idata[i + blockSize],
                                                preProcOprand, i + blockSize) *
                      weight[i + blockSize];
    } else { // not weighted
...
template <typename T, PostProcessType postProc, typename U>
__global__ void reduceSumWarpPostProcess(T* g_idata, T* g_odata, unsigned int n,
                                         U* postProcOprand) {
  ...
  if (tid == 0)
    g_odata[0] = postProcess<T, postProc, U>(sumValue, postProcOprand);
}

} // namespace cudaReduction

#endif // CUDA_REDUCTION_CUH
