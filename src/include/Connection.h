...
// tag=2 for upward communication, and all ways use particle communicator for
// the group.
//
namespace Direction {
enum Enum {
...
public:           // init
  ...
  static Connection null2self(int rank_, int tag_, int self_tag,
                              MPI_Comm comm_) {
  ...
  Connection() : _rank(0), _tag(0), _comm(MPI_COMM_NULL) {}
...
public: // accessors
  ...
  const char* tag_name() const { return tag_name(_tag); }
};
