...
#endif
inline bool is_aligned(void* p, int N) { return (unsigned long)p % N == 0; }
...
//
template <class type>
inline type**** newArray4(type* in, size_t sz1, size_t sz2, size_t sz3,
                          size_t sz4) {
...
//
template <class type> inline void delArray1(type* arr) { AlignedFree(arr); }
...
//
template <class type> inline void delArr1(type* arr) { delArray1(arr); }
...
template <class type> class base_arr {
private:
  size_t size;
...
template <class type> class array_fetch1 {
...
template <class type> class array_fetch2 {
  type* const __restrict__ arr;
  const size_t shift;
  const size_t S2, S1;
...
template <class type> class array_fetch3 {
  type* const __restrict__ arr;
  const size_t shift;
  const size_t S3, S2, S1;

public:
  inline array_fetch3(type* const arr_, size_t k, size_t s3, size_t s2,
                      size_t s1)
...
template <class type> class const_array_get1 {
...
template <class type> class const_array_get2 {
  type const* const __restrict__ arr;
  const size_t shift;
  const size_t S2, S1;

public:
  inline const_array_get2(type const* const arr_, size_t k, size_t s2,
                          size_t s1)
      : arr(arr_), shift(k), S2(s2), S1(s1) {}
...
template <class type> class const_array_get3 {
  type const* const __restrict__ arr;
  const size_t shift;
  const size_t S3, S2, S1;

public:
  const_array_get3(type const* const arr_, size_t k, size_t s3, size_t s2,
                   size_t s1)
      : arr(arr_), shift(k), S3(s3), S2(s2), S1(s1) {}
...
template <class type> class array_ref1 {
private: // data
  const size_t S1;
  type* const __restrict__ arr;
...
public:
  ~array_ref1() {}
  void free() { AlignedFree(arr); }
  array_ref1(size_t s1) : S1(s1), arr(AlignedAlloc(type, s1)) {}
  array_ref1(type* in, size_t s1) : S1(s1), arr(in) {}
  inline type& operator[](size_t n1) {
    check_bounds(n1, S1);
    ALIGNED(arr);
    return arr[n1];
...
template <class type> class const_array_ref2 : public base_arr<type> {
public:
...
public:
  ...
  const_array_ref2(size_t s2, size_t s1)
      : size(s2 * s1), base_arr<type>(s2 * s1), S2(s2), S1(s1),
        arr2(newArray2<type>(arr, s2, s1)) {}
  const_array_ref2(type* const* in, size_t s2, size_t s1)
      : size(s2 * s1), // arr(**in),
        base_arr<type>(*in, s2 * s1), S2(s2), S1(s1), arr2(in) {}
...
template <class type> class array_ref2 : public const_array_ref2<type> {
  // using base_arr<type>::arr;
...
public:
  ~array_ref2() {}
  array_ref2(size_t s2, size_t s1) : const_array_ref2<type>(s2, s1) {}
  array_ref2(type* const* in, size_t s2, size_t s1)
...
template <class type> class const_array_ref3 : public base_arr<type> {
public:
  using base_arr<type>::arr;
  using base_arr<type>::get_arr;
...
public:
  ...
  const_array_ref3(size_t s3, size_t s2, size_t s1)
      : size(s3 * s2 * s1), // arr(AlignedAlloc(type, size)),
        base_arr<type>(s3 * s2 * s1), S3(s3), S2(s2), S1(s1),
        arr3(newArray3<type>(arr, s3, s2, s1)) {}
  const_array_ref3(type* const* const* in, size_t s3, size_t s2, size_t s1)
      : size(s3 * s2 * s1), // arr(**in),
        base_arr<type>(**in, s3 * s2 * s1), S3(s3), S2(s2), S1(s1), arr3(in) {}
  int get_size() const { return size; }
...
template <class type> class array_ref3 : public const_array_ref3<type> {
  // using base_arr<type>::arr;
...
public:
  ...
  array_ref3(type* const* const* in, size_t s3, size_t s2, size_t s1)
...
template <class type> class const_array_ref4 : public base_arr<type> {
public:
  using base_arr<type>::arr;
  using base_arr<type>::get_arr;
...
public:
  ...
  const_array_ref4(size_t s4, size_t s3, size_t s2, size_t s1)
      : size(s4 * s3 * s2 * s1), // arr(AlignedAlloc(type, size)),
        base_arr<type>(s4 * s3 * s2 * s1), S4(s4), S3(s3), S2(s2), S1(s1),
        arr4(newArray4<type>((type*)get_arr(), s4, s3, s2, s1)) {}
  const_array_ref4(type* const* const* const* in, size_t s4, size_t s3,
                   size_t s2,
                   size_t s1)
      : size(s4 * s3 * s2 * s1), // arr(***in),
        base_arr<type>(***in, s4 * s3 * s2 * s1), S4(s4), S3(s3), S2(s2),
        S1(s1), arr4(in) {}
  int get_size() const { return size; }
...
template <class type> class array_ref4 : public const_array_ref4<type> {
...
public:
  ...
  array_ref4(type* const* const* const* in, size_t s4, size_t s3, size_t s2,
             size_t s1)
...
template <class type> inline type**** fetch_arr4(array_ref4<type>& in) {
...
template <class type> inline type*** fetch_arr3(array_ref3<type>& in) {
  return in.fetch_arr3();
}

template <class type> inline const type** get_arr2(const_array_ref2<type>& in) {
  return in.get_arr2();
}
template <class type> inline type** fetch_arr2(array_ref2<type>& in) {
  return in.fetch_arr2();
}

template <class type> inline type* fetch_arr(array_ref1<type>& in) {
  return in.get_arr();
}
template <class type> inline type* fetch_arr(array_fetch1<type>& in) {
  return in.fetch_arr();
}
} // namespace iPic3D
...
#define newArr4(type, sz1, sz2, sz3, sz4)                                      \
  newArray4<type>((sz1), (sz2), (sz3), (sz4))
