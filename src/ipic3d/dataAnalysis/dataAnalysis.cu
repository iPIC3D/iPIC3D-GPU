...
private:
  ...
  std::vector<std::vector<cudaGMMWeight::GMMResult<GMMType, DATA_DIM_GMM>>>
      gmmResults;
...
public:
  dataAnalysisPipelineImpl(c_Solver& KCode) {
    ...
    if constexpr (VELOCITY_HISTOGRAM_ENABLE) { // velocity histogram
      ...
      HistogramSubDomainOutputPath = HISTOGRAM_OUTPUT_DIR + "subDomain" +
                                     std::to_string(KCode.myrank) + "_";
      velocityHistogram =
          new velocityHistogram::velocityHistogram3D(VELOCITY_HISTOGRAM3D_SIZE);

      if constexpr (GMM_ENABLE) { // GMM
        GMMSubDomainOutputPath =
            GMM_OUTPUT_DIR + "subDomain" + std::to_string(KCode.myrank) + "_";
    ...
    if constexpr (MACROCELL_SPECTRA_ENABLE) {
      if (KCode.col->getVelocitySpectra()) {
        ...
        if (Cx > 0 && Cy > 0 && Cz > 0 && anySpecies > 0) {
          // Interior cell extents per subdomain (exclude one ghost layer per
          // side).
          ...
          if (!part.build(Nx_int, Ny_int, Nz_int, Cx, Cy, Cz, gOffX, gOffY,
                          gOffZ)) {
            std::cerr << "[macrocellSpectra] invalid macrocell sizes (" << Cx
                      << "," << Cy << "," << Cz << ") vs interior (" << Nx_int
                      << "," << Ny_int << "," << Nz_int
                      << "); feature disabled.\n";
          } else {
            macrocellHist =
                std::make_unique<macrocellSpectra::macrocellSpectra2D>(part);
            ...
            if constexpr (MACROCELL_SPECTRA_OUTPUT) {
              ...
              for (int s = 0; s < ns; ++s) {
                if (macrocellSpeciesMask[s]) {
                  macrocellHist->initSpeciesFile(macrocellSubdomainDir, s,
  ...
  ~dataAnalysisPipelineImpl() {
    ...
    if (DAthreadPool != nullptr)
      delete DAthreadPool;
    if (velocitySoACUDA != nullptr)
      delete velocitySoACUDA;
    if (velocityHistogram != nullptr)
      delete velocityHistogram;
    if (gmmArray != nullptr)
      delete[] gmmArray;
  }
...
private:
  ...
  int GMMAnalysisSpecies(const int cycle, const int species,
                         const std::string outputPath);
};

int dataAnalysisPipelineImpl::GMMAnalysisSpecies(const int cycle,
                                                 const int species,
                                                 const std::string outputPath) {
  ...
  auto GMMLambda = [=]() mutable {
    ...
    // set the random number generator to sample velocity from circle of radius
    // max velocity
    ...
    const GMMType maxVelocity = (species == 0 || species == 2)
                                    ? MAX_VELOCITY_HIST_E
                                    : MAX_VELOCITY_HIST_I;
    // it is assumed that DATA_DIM_GMM == 3 and that the velocity range is
    // homogenues in all dimensions
    const GMMType maxVelocityArray[DATA_DIM_GMM] = {maxVelocity, maxVelocity,
                                                    maxVelocity};
    // right now it is not used (fixed to zero) --> mean is not subtracted to
    // data, but it might be useful for future developments
    ...
    if constexpr (START_WITH_LAST_PARAMETERS_GMM) // start GMM with output GMM
                                                  // parameters from last cycle
                                                  // as initial parameters
    {
      ...
      if (gmmResults[species].size() == 0) {
        for (int j = 0; j < NUM_COMPONENT_GMM; j++) {
          ...
          meanVector[j * DATA_DIM_GMM] =
              maxVelocity * std::cbrt(u) * n1 / llsqrt / normalization;
          meanVector[j * DATA_DIM_GMM + 1] =
              maxVelocity * std::cbrt(u) * n2 / llsqrt / normalization;
          meanVector[j * DATA_DIM_GMM + 2] =
              maxVelocity * std::cbrt(u) * n3 / llsqrt / normalization;

          for (int k = 0; k < DATA_DIM_GMM * DATA_DIM_GMM; k++) {
            const int i = k / DATA_DIM_GMM; // row index
            const int l = k % DATA_DIM_GMM; // column index
            coVarianceMatrix[j * DATA_DIM_GMM * DATA_DIM_GMM + k] =
                varArray[i] * varArray[l] / (normalization * normalization);
          }
        }
      } else // Initialize GMM with previous output parameters
      {
        ...
        // safety checks on components weights and mean since after pruning some
        // components have zero weight check if meanVector is NaN or component
        // weight is too small if meanVector is NaN sample new mean vector
        for (int j = 0; j < NUM_COMPONENT_GMM; j++) {
          if (std::isnan(lastResult.mean[j * DATA_DIM_GMM]) ||
              std::isinf(lastResult.mean[j * DATA_DIM_GMM]) ||
              std::isnan(lastResult.mean[j * DATA_DIM_GMM + 1]) ||
              std::isinf(lastResult.mean[j * DATA_DIM_GMM + 1]) ||
              std::isnan(lastResult.mean[j * DATA_DIM_GMM + 2]) ||
              std::isinf(lastResult.mean[j * DATA_DIM_GMM + 2])) {
            reset = true;
            const GMMType n1 = norm_dist(gen);
            const GMMType n2 = norm_dist(gen);
            const GMMType n3 = norm_dist(gen);
            ...
            meanVector[j * DATA_DIM_GMM] =
                lastResult.mean[j * DATA_DIM_GMM] / normalization;
            meanVector[j * DATA_DIM_GMM + 1] =
                lastResult.mean[j * DATA_DIM_GMM + 1] / normalization;
            meanVector[j * DATA_DIM_GMM + 2] =
                lastResult.mean[j * DATA_DIM_GMM + 2] / normalization;
          }

          if (lastResult.weight[j] < PRUNE_THRESHOLD_GMM * 10)
            reset = true;
        }
        ...
        for (int j = 0; j < NUM_COMPONENT_GMM; j++) {
          ...
          for (int k = 0; k < DATA_DIM_GMM * DATA_DIM_GMM; k++) {
            ...
            if (i == l) {
              coVarianceMatrix[j * DATA_DIM_GMM * DATA_DIM_GMM + k] =
                  lastResult.coVariance[j * DATA_DIM_GMM * DATA_DIM_GMM + k] >
                          (10 * EPS_COVMATRIX_GMM *
                           (normalization * normalization))
                      ? lastResult
                            .coVariance[j * DATA_DIM_GMM * DATA_DIM_GMM + k]
                      : varArray[i] * varArray[l];
              coVarianceMatrix[j * DATA_DIM_GMM * DATA_DIM_GMM + k] /=
                  (normalization * normalization);
            } else {
              coVarianceMatrix[j * DATA_DIM_GMM * DATA_DIM_GMM + k] =
    ...
    } else // start GMM with fixed initial parameters at any cycle
    {
      for (int j = 0; j < NUM_COMPONENT_GMM; j++) {
        ...
        // for (int k = 0; k < DATA_DIM_GMM * DATA_DIM_GMM; k++){
        //     coVarianceMatrix[j * DATA_DIM_GMM * DATA_DIM_GMM + k] = (k %
        //     (DATA_DIM_GMM + 1)) == 0 ? 0.0001 : 0.0;
        // }
      }
    }

    GMMParam_t<GMMType> GMMParam = {.numComponents = NUM_COMPONENT_GMM,
    ...
    GMMDataMultiDim<GMMType, DATA_DIM_GMM, weightType> GMMData(
        VELOCITY_HISTOGRAM3D_SIZE, velocityHistogram->getHistogramScaleMark(),
        velocityHistogram->getVelocityHistogramCUDAArray());

    cudaErrChk(
        cudaHostRegister(&GMMData, sizeof(GMMData), cudaHostRegisterDefault));
  ...
  if constexpr (GMM_OUTPUT) {
    ...
    if (!std::filesystem::exists(outputPath)) {
      ...
      if (output.is_open()) {
        ...
        throw std::runtime_error(
            "[!]Error: Can not open output file for velocity GMM species");
      }
    }

    if (std::isnan(gmmResults[species].back().getlogLikelihoodFinal()))
      std::cerr << "[!]GMM: LogLikelihood is NaN!" << " path " << outputPath
                << " time step " << gmmResults[species].back().simulationStep
                << std::endl;
    ...
    if (output.is_open()) {
      ...
      buffer << "\"" << std::to_string(cycle)
             << "\": " << gmmResults[species].back().outputString() << "\n";
 ...
 * @details procesures in this function should be executed in sequence, the
 * order of the analysis should be defined here But the procedures can launch
 * other threads to do the analysis Also this function is a friend function of
 * c_Solver, resources in the c_Slover should be dispatched here
  ...
  for (int i = 0; i < ns; i++) {
    if constexpr (VELOCITY_HISTOGRAM_ENABLE) {
      ...
      auto histogramSpeciesOutputPath =
          HistogramSubDomainOutputPath + "species" + std::to_string(i) + "_";
      velocityHistogram->init(velocitySoACUDA, i, streams[i]);
      if constexpr (HISTOGRAM_OUTPUT)
        velocityHistogram->writeToFile(histogramSpeciesOutputPath, cycle,
                                       streams[i]);
      else
      ...
      if constexpr (GMM_ENABLE) { // GMM
        auto GMMSpeciesOutputPath =
            GMMSubDomainOutputPath + "species" + std::to_string(i) + ".json";
    ...
    if constexpr (MACROCELL_SPECTRA_ENABLE) {
      if (macrocellEnabled && i < (int)macrocellSpeciesMask.size() &&
        ...
        macrocellHist->launch(
            pclsArrayHostPtr[i], macrocellFieldForPcls, macrocellGridDevicePtr,
            macrocellCellSorters[i].getCellStartOffsets(),
            macrocellCellSorters[i].getCellCounts(), i, streams[i]);
        if constexpr (MACROCELL_SPECTRA_OUTPUT) {
...
void dataAnalysisPipelineImpl::startAnalysis(int cycle) {
  ...
  if (DATA_ANALYSIS_EVERY_CYCLE == 0 ||
      (cycle % DATA_ANALYSIS_EVERY_CYCLE != 0)) {
    ...
    if (MPIdata::get_rank() == 0) {
      ...
      if (histOn || gmmOn || mcOn) {
        printf("  [Cycle %d] Data analysis: ON [%s%s%s]\n", cycle,
               histOn ? "histogram " : "", gmmOn ? "GMM " : "",
               mcOn ? "macrocellSpectra" : "");
      }
    }
    analysisFuture = DAthreadPool->enqueue(
        &dataAnalysisPipelineImpl::analysisEntre, this, cycle);
...
int dataAnalysisPipelineImpl::checkAnalysis() {
  ...
  if (analysisFuture.wait_for(std::chrono::seconds(0)) ==
...
/**
 * @brief create output directory for the data analysis, controlled by
 * dataAnalysisConfig.cuh
 ...
 * @param velocitySpectraEnabled runtime flag from VelocitySpectra input
 * parameter.
 */
void dataAnalysisPipeline::createOutputDirectory(int myrank, int ns,
  ...
  auto writeVctMapping = [&](const std::string& filePath) {
    ...
    if (vctMapping.is_open()) {
      ...
      vctMapping << "Neighbor X left: " << vct->getXleft_neighbor()
                 << std::endl;
      vctMapping << "Neighbor X right: " << vct->getXright_neighbor()
                 << std::endl;
      vctMapping << "Neighbor Y left: " << vct->getYleft_neighbor()
                 << std::endl;
      vctMapping << "Neighbor Y right: " << vct->getYright_neighbor()
                 << std::endl;
      vctMapping << "Neighbor Z left: " << vct->getZleft_neighbor()
                 << std::endl;
      vctMapping << "Neighbor Z right: " << vct->getZright_neighbor()
                 << std::endl;
  ...
  if constexpr (VELOCITY_HISTOGRAM_ENABLE && HISTOGRAM_OUTPUT) {
    ...
    if (myrank == 0) {
      prepareOutputFolder(
          histogramSubDomainOutputPath,
          "[!]Error: Can not create output folder for velocity histogram");
    }

    MPI_Barrier(MPIdata::get_PicGlobalComm());

    writeVctMapping(histogramSubDomainOutputPath + "vctMapping_subDomain" +
                    std::to_string(myrank) + ".txt");
  }
  ...
  if constexpr (GMM_ENABLE && GMM_OUTPUT) {
    ...
    writeVctMapping(GMMSubDomainOutputPath + "vctMapping_subDomain" +
                    std::to_string(myrank) + ".txt");
  }
  ...
  if constexpr (MACROCELL_SPECTRA_ENABLE && MACROCELL_SPECTRA_OUTPUT) {
    if (velocitySpectraEnabled) {
      ...
      const auto subDir =
          macrocellRoot + "subDomain_" + std::to_string(myrank) + "/";
      prepareOutputFolder(
          subDir,
