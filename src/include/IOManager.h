...
public:
  ...
  /**
   * @brief Initialise backends based on compile-time options and runtime
   * config.
   *
   * Must be called once, after Collective/Grid/EMfields/Particles are ready.
   * The pointers are stored but not owned; the caller (`c_Solver`) keeps
   * ownership.
   *
   ...
   * @param first_cycle First simulation cycle to consider for output
   * scheduling.
   */
  void init(Collective* col, VCtopology3D* vct, Grid3DCU* grid, EMfields3D* EMf,
            ParticleSoAHost** outputPart, int ns, ParticleSoAHost** testpart,
            int nstestpart, int first_cycle);
...
private:
  ...
  /**
   * @brief Complete any in-flight NBCVTK split-collective writes and close
   * their files.
   *
