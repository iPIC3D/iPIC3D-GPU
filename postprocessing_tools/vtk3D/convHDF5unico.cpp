...
int main(int argc, char** argv) {
  ...
  for (int n_cycle = beg_cycle; n_cycle <= end_cycle; n_cycle += stp_cycle) {
    ...
    my_fileB << "DIMENSIONS " << nxn * XLEN << " " << nyn * YLEN << " "
             << nzn * ZLEN << endl;
    ...
    my_fileScalars << "DIMENSIONS " << nxn * XLEN << " " << nyn * YLEN << " "
                   << nzn * ZLEN << endl;
    ...
    for (int kk = 0; kk < nzn * ZLEN; kk++)
      for (int jj = 0; jj < nyn * YLEN; jj++)
        for (int ii = 0; ii < nxn * XLEN; ii++) {
          my_fileB << BX[ii][jj][kk] << " " << BY[ii][jj][kk] << " "
                   << BZ[ii][jj][kk] << endl;
          // my_file << temp_storageX[node] << " " << temp_storageY[node] << " "
          // << temp_storageZ[node] << endl;
        }
