...
void test_spawn(char** argv) {
  ...
  if (1) {
    ...
    MPI_Irecv(recvbuff, count, MPI_CHAR, lower_neighbor, 0, comm,
              &recv_request);
    // send a message to upper
    MPI_Request send_request;
    MPI_Isend(sendbuff, sendcount, MPI_CHAR, upper_neighbor, 0, comm,
              &send_request);
  ...
  if (1) {
    ...
    if (!was_spawned) {
      ...
      MPI_Comm_spawn("./testspawn", &argv[1], nprocs, MPI_INFO_NULL, 0,
