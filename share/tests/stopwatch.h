...
static inline struct timeval add_tv(const struct timeval a,
                                    const struct timeval b) {
  const struct timeval res = {a.tv_sec + b.tv_sec, a.tv_usec + b.tv_usec};
  return res;
}

static inline struct timeval diff_tv(const struct timeval start,
                                     const struct timeval stop) {
  const struct timeval diff = {stop.tv_sec - start.tv_sec,
...
static inline struct timeval stopwatch_mt(stopwatch_t* const sw,
                                          sw_action_t action) {
  typedef struct timeval stopwatch_func_t(stopwatch_t * sw);

  static stopwatch_func_t* const stopwatch_transitions[3][4] = {
      {sw_start, 0, 0, 0}, {0, sw_stop, sw_lap, 0}, {sw_start, 0, 0, sw_reset}};
  assert(sw != 0);
