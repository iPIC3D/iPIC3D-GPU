...
void HDF5OutputAdaptor::get_dataset_context(const std::string& name,
  ...
  if (ncompx == 0) {
    // throw PSK::OutputException("HDF5OutputAdaptor::get_dataset_context()>>
    // zero name components");
    eprintf(">> zero name components");
  } else if (ncompx == 1) {
    hid_array[0] = _hdf5_file_id;
    dataset_name = name_components[0];
  } else {
    /* HDF5 error handling code from
    ...
    for (int i = 0; i < ncompx - 1; ++i) {
      ...
      hid_array[i + 1] =
          H5Gopen2(hid_array[i], name_components[i].c_str(), H5P_DEFAULT);
      ...
      if (hid_array[i + 1] < 0) {
        ...
        hid_array[i + 1] =
            H5Gcreate2(hid_array[i], name_components[i].c_str(), H5P_DEFAULT,
                       H5P_DEFAULT, H5P_DEFAULT); // HDF 1.8

        if (hid_array[i + 1] < 0) {

          // std::cout << "group create failed \n" ;

          // throw PSK::OutputException("Failed to open/create group for <" +
          // name + "> at element <" + name_components[i] + ">",
          // "HDF5OutputAdaptor::get_dataset_context()");
          eprintf("Failed to open/create group for <%s> at element <%s>",
                  name.c_str(), name_components[i].c_str());
        }
      }
    }
...
std::string HDF5OutputAdaptor::purify_object_name(const std::string& objname) {
  if (objname.length() == 0)
    // throw PSK::OutputException("Zero length tag name",
    // "HDF5OutputAdaptor::purify_object_name()");
...
void HDF5OutputAdaptor::open_append(const std::string& name) {
  ...
  _hdf5_file_id =
      H5Fcreate(name.c_str(), H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
  ...
  if (_hdf5_file_id < 0) {
    ...
    // PSK::OutputException e("H5Fopen fails",
    // "HDF5OutputAdaptor::open_append()");
...
void HDF5OutputAdaptor::write(const std::string& tag, int i_value) {
  ...
  herr_t hdf5err =
      H5LTfind_dataset(hid_array[hid_array.size() - 1], dataset_name.c_str());
  if (hdf5err < 1) {
    herr_t hdf5err =
        H5LTmake_dataset_int(hid_array[hid_array.size() - 1],
                             dataset_name.c_str(), 1, hdf5dims, &i_value);
  } else {
    hid_t dataset_id =
        H5Dopen2(hid_array[hid_array.size() - 1], dataset_name.c_str(),
                 H5P_DEFAULT); // HDF 1.8
    herr_t hdf5err = H5Dwrite(dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
                              H5P_DEFAULT, &i_value);
  ...
  if (hdf5err < 0) {
    ...
    // PSK::OutputException e("make_dataset fails for " + tag,
    // "HDF5OutputAdaptor::write(int)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, long i_value) {
  ...
  if (hdf5err < 1) {
    herr_t hdf5err =
        H5LTmake_dataset_long(hid_array[hid_array.size() - 1],
                              dataset_name.c_str(), 1, hdf5dims, &i_value);
  } else {
    hid_t dataset_id =
        H5Dopen2(hid_array[hid_array.size() - 1], dataset_name.c_str(),
  ...
  if (hdf5err < 0) {
    eprintf("make_dataset fails for %s", tag.c_str());
    // PSK::OutputException e("make_dataset fails for " + tag,
    // "HDF5OutputAdaptor::write(long)"); throw e;
  }
...
//
void HDF5OutputAdaptor::write(const std::string& tag, const Dimens dimens,
                              const int* i_array) {
  // try {
  if (dimens.size() == 0) {
    eprintf("Zero Dimens size");
    // PSK::OutputException e("Zero Dimens size", "HDF5OutputAdaptor::write(int*
    // array)"); throw e;
  }
  ...
  if (hdf5err < 1) {
    herr_t hdf5err = H5LTmake_dataset_int(hid_array[hid_array.size() - 1],
                                          dataset_name.c_str(), dimens.size(),
                                          hdf5dims, i_array);
  } else {
    hid_t dataset_id =
        H5Dopen2(hid_array[hid_array.size() - 1], dataset_name.c_str(),
  ...
  if (hdf5err < 0) {
    eprintf("make_dataset fails for %s", tag.c_str());
    // PSK::OutputException e("make_dataset fails for " + tag,
    // "HDF5OutputAdaptor::write(int* array)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, const Dimens dimens,
  ...
  if (dimens.size() == 0) {
    ...
    // "HDF5OutputAdaptor::write(long* array)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, const Dimens dimens,
  ...
  herr_t hdf5err =
      H5LTmake_dataset(hid_array[hid_array.size() - 1], dataset_name.c_str(),
                       dimens.size(), hdf5dims, H5T_NATIVE_UINT64, i_array);
...
void HDF5OutputAdaptor::write(const std::string& objname, const Dimens dimens,
                              const int*** i_array) {
  if (dimens.size() != 3) {
    eprintf("Dimens size not 3 for object %s", objname.c_str());
    // PSK::OutputException e("Dimens size not 3 for object " + objname,
    // "HDF5OutputAdaptor::write(int*** array)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, float f_value) {
  ...
  herr_t hdf5err =
      H5LTmake_dataset_float(hid_array[hid_array.size() - 1],
                             dataset_name.c_str(), 1, hdf5dims, &f_value);

  if (hdf5err < 0) {
    eprintf("make_dataset fails for %s", tag.c_str());
    // PSK::OutputException e("make_dataset fails for " + tag,
    // "HDF5OutputAdaptor::write(float)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, const Dimens dimens,
  ...
  if (dimens.size() == 0) {
    eprintf("Zero Dimens size");
    // PSK::OutputException e("Zero Dimens size",
    // "HDF5OutputAdaptor::write(float* array)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, double d_value) {
  ...
  if (hdf5err < 1) {
    herr_t hdf5err =
        H5LTmake_dataset_double(hid_array[hid_array.size() - 1],
                                dataset_name.c_str(), 1, hdf5dims, &d_value);
  } else {
    hid_t dataset_id =
        H5Dopen2(hid_array[hid_array.size() - 1], dataset_name.c_str(),
                 H5P_DEFAULT); // HDF 1.8
    herr_t hdf5err = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL,
                              H5P_DEFAULT, &d_value);
    hdf5err = H5Dclose(dataset_id);
  }
  ...
  if (hdf5err < 0) {
    ...
    // PSK::OutputException e("make_dataset fails for " + tag,
    // "HDF5OutputAdaptor::write(double)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& tag, const Dimens dimens,
  ...
  if (dimens.size() == 0) {
    eprintf("Zero Dimens size");
    // PSK::OutputException e("Zero Dimens size",
    // "HDF5OutputAdaptor::write(double* array)"); throw e;
  }
  ...
  if (hdf5err < 1) {
    ...
    hid_t dataset_id =
        H5Dopen2(hid_array[hid_array.size() - 1], dataset_name.c_str(),
                 H5P_DEFAULT); // HDF 1.8
    hdf5err = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL,
                       H5P_DEFAULT, d_array);
    hdf5err = H5Dclose(dataset_id);
  }
...
void HDF5OutputAdaptor::write(const std::string& objname, const Dimens dimens,
                              const_arr3_double d_array, double scale) {
  if (dimens.size() != 3) {
    eprintf("Dimens size not 3 for object %s", objname.c_str());
    // PSK::OutputException e("Dimens size not 3 for object " + objname,
    // "HDF5OutputAdaptor::write(const_arr3_double array)"); throw e;
  }
  ...
  for (int i = 0; i < di; ++i)
    for (int j = 0; j < dj; ++j)
      for (int k = 0; k < dk; ++k) {
        ...
        if (dk != 1)
          d_array_p[i * djk + j * dk + k] =
              d_array[i + 1][j + 1][k + 1] * scale;
        else if (dj != 1)
...
void HDF5OutputAdaptor::write(const std::string& objname, const Dimens dimens,
                              const int ns, const_arr4_double d_array,
                              double scale) {
  if (dimens.size() != 3) {
    eprintf("Dimens size not 3 for object %s", objname.c_str());
    // PSK::OutputException e("Dimens size not 3 for object " + objname,
    // "HDF5OutputAdaptor::write(const_arr4_double array)"); throw e;
  }
...
void HDF5OutputAdaptor::write(const std::string& objname, const Dimens dimens,
  ...
  if (dimens.size() != 2) {
    ...
    // PSK::OutputException e("Dimens size not 2 for object " + objname,
    // "HDF5OutputAdaptor::write(double** array)"); throw e;
  }
  ...
  for (int i = 0; i < di; ++i)
    for (int j = 0; j < dj; ++j)
      d_array_p[i * dj + j] =
          d_array[i + 1][j + 1]; // I am not writing ghost cells
  write(objname, dimens, d_array_p);
  delete[] d_array_p;
  //} catch(PSK::Exception & e) {
  //  e.push("In HDF5OutputAdaptor::write(double** array)");
...
void HDF5OutputAdaptor::write(const std::string& objname, const Dimens dimens,
                              const int ns, const_arr3_double d_array) {
  if (dimens.size() != 2) {
    eprintf("Dimens size not 2 for object %s", objname.c_str());
    // PSK::OutputException e("Dimens size not 2 for object " + objname,
    // "HDF5OutputAdaptor::write(const_arr3_double array)"); throw e;
  ...
  for (int i = 0; i < di; ++i)
    for (int j = 0; j < dj; ++j)
      d_array_p[i * dj + j] =
          d_array[i + 1][j + 1][ns]; // I am not writing ghost cells
  write(objname, dimens, d_array_p);
  delete[] d_array_p;
}
#endif
