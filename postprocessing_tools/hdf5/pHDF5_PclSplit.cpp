...
int main(int argc, char** argv) {
  ...
  cout << " Successful in reading settings.hdf, XLEN=" << XLEN
       << ", YLEN=" << YLEN << ", ZLEN=" << ZLEN << endl;
  ...
  for (int icycle = start_cycle; icycle <= end_cycle;
       icycle = icycle + step_cycle)
    for (int xi = 0; xi < XLEN; xi++)
      for (int yi = 0; yi < YLEN; yi++)
        for (int zi = 0; zi < ZLEN; zi++) {
          ...
          dataset_id = H5Dopen2(proc_file_id, "/topology/cartesian_coord",
          ...
          dataset_id = H5Dopen2(proc_file_id, ss.str().c_str(),
                                H5P_DEFAULT); // HDF 1.8.8
          ...
          cout << ss.str() << " at [" << cartesian_cor[0] << "]["
               << cartesian_cor[1] << "][" << cartesian_cor[2]
               << "], species_0: " << nop0 << ", species_1: " << nop1;
