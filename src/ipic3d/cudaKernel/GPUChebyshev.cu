...
// =========================================================================
template<typename T>
__global__ void k_chebyshevStep1(T* __restrict__ d_y,
                                 T* __restrict__ d_z,
                                 const T* __restrict__ d_b,
                                 const T* __restrict__ d_Ab,
                                 T invTheta,
                                 T coeff_b,
                                 T coeff_Ab,
                                 int n)
                       ...
                       int n, cudaStream_t stream)
{
    ...
    cudaSolverType coeff_Ab = -2.0 * rho / (delta * theta);  // 2*rho/delta * (-1/theta)
...
// =========================================================================
template<typename T>
__global__ void k_chebyshevStepN(T* __restrict__ d_w,
                                 const T* __restrict__ d_y,
                                 const T* __restrict__ d_z,
                                 const T* __restrict__ d_b,
                                 const T* __restrict__ d_Ay,
                                 T fY, T fB, T fAy, T fZ,
                                 int n)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
