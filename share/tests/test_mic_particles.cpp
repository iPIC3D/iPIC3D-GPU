...
#define printarr(var, numel)                                                   \
  std::cout << "line " << __LINE__ << ": " << #var << " = (";                  \
  for (int i = 0; i < (numel - 1); i++)                                        \
    std::cout << var[i] << ", ";                                               \
  std::cout << var[numel - 1] << ")" << std::endl;

inline void get_field_components_for_cell(arr1_double_get field_components[8],
...
void init_pcls() {
  for (int i = 0; i < nop; i++) {
    ...
    double z = 3;
    double t = 0;
    pcls[i].set(u + .1 * i, v + .1 * i, w + .1 * i, q + .1 * i, x + .1 * i,
                y + .1 * i, z + .1 * i, t + .1 * i);
    pclsB[i] = pclsA[i];
  }
}
...
void test_get_field_components_for_cell() {
  ...
  get_field_components_for_cell(field_components0, field_components1,
                                fieldForPcls, cx);
  std::cout << "cx=" << cx << endl;
  for (int i = 0; i < 8; i++) {
    std::cout << "field_components0[" << i << "]=" << field_components0[i]
              << endl;
  }
  for (int i = 0; i < 8; i++) {
    std::cout << "field_components1[" << i << "]=" << field_components1[i]
              << endl;
  }
}
...
Grid3DCU::Grid3DCU(int nxc_, int nyc_, int nzc_, double dx_, double dy_,
                   double dz_, double xStart_, double yStart_, double zStart_)
    : nxc(nxc_), nyc(nyc_), nzc(nzc_), dx(dx_), dy(dy_), dz(dz_),
