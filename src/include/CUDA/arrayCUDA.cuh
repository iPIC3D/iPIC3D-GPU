#ifndef ARRAY_CUDA_CUH
#define ARRAY_CUDA_CUH
...
template <typename T, int sizeUnit = 64> class arrayCUDA {
...
private:
  ...
  /**
   * @brief Round a requested element count up to the internal allocation
   * quantum.
...
public: // con-de-structor
   ...
   */
  __host__ arrayCUDA(commonInt requiredSize)
      : onHeap(false), numberOfElement(0) {
   ...
   */
  __host__ arrayCUDA(const T* hostArrayPtr, commonInt size)
      : onHeap(false), numberOfElement(size) {
    arraySize = roundUpToSizeUnit(size);
    cudaErrChk(cudaMalloc(&arrayPtr, arraySize * sizeof(T)));
    cudaErrChk(cudaMemcpyAsync(arrayPtr, hostArrayPtr,
                               numberOfElement * sizeof(T), cudaMemcpyDefault,
                               stream));
  ...
  __host__ ~arrayCUDA() {
    if (arrayPtr != nullptr) {
      if (onHeap == false)
        cudaErrChk(cudaFree(arrayPtr));
      else {
        std::cerr << "arrayCUDA: the array is allocated on device heap!"
                  << std::endl;
        assert(0);
      }
    }
...
public: // utilities
  ...
  __host__ __device__ T* getElement(commonInt index) {
    return arrayPtr + index;
  }

  __host__ commonInt appendElement(T* p) {
    if (numberOfElement >= (arraySize - 1))
      assert(0);
  ...
  /**
   * @brief Expand the device allocation if `targetedSize` exceeds current
   * capacity.
  ...
  __host__ commonInt expand(commonInt targetedSize) {
    if (targetedSize <= arraySize)
      return arraySize;

    arraySize = roundUpToSizeUnit(targetedSize);
    auto oldArray = arrayPtr;
    cudaErrChk(cudaMalloc(&arrayPtr, arraySize * sizeof(T)));
    cudaErrChk(cudaMemcpyAsync(arrayPtr, oldArray, numberOfElement * sizeof(T),
  ...
  /**
   * @brief Resize the device allocation while preserving the current element
   * count.
   * @param targetedSize Requested capacity in elements.
   * @return Final allocated capacity.
   */
  __host__ commonInt resize(commonInt targetedSize) {
    if (targetedSize <= numberOfElement)
      return arraySize;
  ...
  __host__ virtual arrayCUDA* copyToDevice() {
    ...
    cudaErrChk(cudaMemcpyAsync(ptr, this, sizeof(arrayCUDA), cudaMemcpyDefault,
                               stream));
