...
std::vector<dataType> generateDataset(size_t size) {
  ...
  for (size_t i = 0; i < size; ++i) {
    data[i] = static_cast<dataType>(rand()) / RAND_MAX *
              100.0f; // Random floats in range [0, 100)
  }
  return data;
}
...
static int runTest() {
  ...
  dataType *deviceInput, *deviceOutput;
  dataType* reductionTemp;
  cudaErrChk(cudaMalloc(&deviceInput, size * sizeof(dataType)));
  cudaErrChk(cudaMalloc(&deviceOutput, sizeof(dataType)));
  cudaErrChk(cudaMemcpy(deviceInput, data.data(), size * sizeof(dataType),
                        cudaMemcpyHostToDevice));

  cudaErrChk(cudaMalloc(&reductionTemp, blockNum * sizeof(dataType)));

  // max
  cudaReduction::reduceMax<dataType, blockSize>
      <<<blockNum, blockSize, blockSize * sizeof(dataType)>>>(
          deviceInput, reductionTemp, size);
  cudaReduction::reduceMaxWarp<dataType>
      <<<1, WARP_SIZE>>>(reductionTemp, deviceOutput, blockNum);
  cudaErrChk(cudaMemcpy(&result, deviceOutput, sizeof(dataType),
  ...
  // min
  cudaReduction::reduceMin<dataType, blockSize>
      <<<blockNum, blockSize, blockSize * sizeof(dataType)>>>(
          deviceInput, reductionTemp, size);
  cudaReduction::reduceMinWarp<dataType>
      <<<1, WARP_SIZE>>>(reductionTemp, deviceOutput, blockNum);
  cudaErrChk(cudaMemcpy(&result, deviceOutput, sizeof(dataType),
                        cudaMemcpyDeviceToHost));
