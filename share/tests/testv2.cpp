...
//
void sample_field(dfloat fields[2 * D], dfloat weights[8],
...
public:
  const void* get_data_memory() const { return &data[0][0]; }
...
void copy_AoS_particles_to_another_array_rev() {
  uint64_t* p1 = (uint64_t*)pcls;
  ASSUME_ALIGNED(p1);
  uint64_t* p2 = (uint64_t*)pcls2;
  ASSUME_ALIGNED(p2);
#pragma omp parallel
  {
...
#pragma simd
    ...
    dfloat uavg[D] ALLOC_ALIGNED;
    {
      ...
      {
        uavg[0] =
            (ut[0] + (ut[1] * Om[2] - ut[2] * Om[1] + udotOm * Om[0])) * denom;
        uavg[1] =
            (ut[1] + (ut[2] * Om[0] - ut[0] * Om[2] + udotOm * Om[1])) * denom;
        uavg[2] =
            (ut[2] + (ut[0] * Om[1] - ut[1] * Om[0] + udotOm * Om[2])) * denom;
      }
...
#pragma simd
    for (int pi = 0; pi < NUMPCLS; pi += 1) {
      ...
      dfloat uavg[D];
      {
        ...
        // const dfloat omsq_p1 = 1.0+(Om[0] * Om[0] + Om[1] * Om[1] + Om[2] *
        // Om[2]); const dfloat denom = 1/rfloat(omsq_p1);
        {
          dfloat omsq_p1 = 1. + Om[0] * Om[0] + Om[1] * Om[1] + Om[2] * Om[2];
          denom = 1 / rfloat(omsq_p1);
          // denom = 1.0/omsq_p1;
        ...
        // #pragma simd -- how do I tell it to recognize the swizzle?
        {
          uavg[0] = (ut[0] + (ut[1] * Om[2] - ut[2] * Om[1] + udotOm * Om[0])) *
                    denom;
          uavg[1] = (ut[1] + (ut[2] * Om[0] - ut[0] * Om[2] + udotOm * Om[1])) *
                    denom;
          uavg[2] = (ut[2] + (ut[0] * Om[1] - ut[1] * Om[0] + udotOm * Om[2])) *
                    denom;
        }
        // update average position
        for (int i = 0; i < D; i++) {
...
static inline pfloat get_truncation_ratio(
    // int&direction,
    pfloat dXwant, pfloat dXfull, pfloat dYwant, pfloat dYfull, pfloat dZwant,
    pfloat dZfull) {
...
#pragma simd // vectorlength(16) // why doesn't this help?
    for (int pidx = 0; pidx < NUMPCLS; pidx++) {
      ...
      for (int niter = 0; niter < NiterMover; niter++) {
        ...
        //
        const pfloat ratio =
            get_truncation_ratio(dXwall, dXmag, dYwall, dYmag, dZwall, dZmag);
        //
...
#pragma simd
      for (int pidx = 0; pidx < 8; pidx++) {
        ...
        for (int niter = 0; niter < NiterMover; niter++) {
          ...
          // solve the velocity equation
          const pfloat uavg =
              (ut + (vt * Omz - wt * Omy + udotOm * Omx)) * denom;
          const pfloat vavg =
              (vt + (wt * Omx - ut * Omz + udotOm * Omy)) * denom;
          const pfloat wavg =
              (wt + (ut * Omy - vt * Omx + udotOm * Omz)) * denom;
...
int main() {
  ...
  printf("#\n");
  printf("#\n");
  printf("# demonstrating that the time required to query the time is "
         "negligible:\n");
  printf("#\n");
