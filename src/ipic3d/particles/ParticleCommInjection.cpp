...
ParticleCommInjection::ParticleCommInjection(ParticleSoAHost& hostParticles)
    : hostParticles_(hostParticles), col_(hostParticles.getCollective()),
      vct_(hostParticles.getVirtualTopology()), grid_(hostParticles.getGrid()) {
  ...
  sendXleft_.init(
      Connection::null2self(vct_->getXleft_neighbor_P(), XDN, XDN, mpiComm_));
  sendXrght_.init(
      Connection::null2self(vct_->getXright_neighbor_P(), XUP, XUP, mpiComm_));
  recvXleft_.init(
      Connection::null2self(vct_->getXleft_neighbor_P(), XUP, XDN, mpiComm_));
  recvXrght_.init(
      Connection::null2self(vct_->getXright_neighbor_P(), XDN, XUP, mpiComm_));

  sendYleft_.init(
      Connection::null2self(vct_->getYleft_neighbor_P(), YDN, YDN, mpiComm_));
  sendYrght_.init(
      Connection::null2self(vct_->getYright_neighbor_P(), YUP, YUP, mpiComm_));
  recvYleft_.init(
      Connection::null2self(vct_->getYleft_neighbor_P(), YUP, YDN, mpiComm_));
  recvYrght_.init(
      Connection::null2self(vct_->getYright_neighbor_P(), YDN, YUP, mpiComm_));

  sendZleft_.init(
      Connection::null2self(vct_->getZleft_neighbor_P(), ZDN, ZDN, mpiComm_));
  sendZrght_.init(
      Connection::null2self(vct_->getZright_neighbor_P(), ZUP, ZUP, mpiComm_));
  recvZleft_.init(
      Connection::null2self(vct_->getZleft_neighbor_P(), ZUP, ZDN, mpiComm_));
  recvZrght_.init(
      Connection::null2self(vct_->getZright_neighbor_P(), ZDN, ZUP, mpiComm_));
  ...
  // Compute injection count once (depends only on grid, BC, topology — all
  // constant).
  cachedInjectionCount_ = computeInjectionCountImpl();
}

ParticleCommInjection::~ParticleCommInjection() { MPI_Comm_free(&mpiComm_); }
...
inline bool
ParticleCommInjection::sendParticleToAppropriateBuffer(SpeciesParticle& pcl,
                                                       int count[6]) {
  bool wasSent = true;
  if (pcl.get_x() < subdomainXstart_) {
    sendXleft_.send(pcl);
    count[0]++;
  } else if (pcl.get_x() > subdomainXend_) {
    sendXrght_.send(pcl);
    count[1]++;
  } else if (pcl.get_y() < subdomainYstart_) {
    sendYleft_.send(pcl);
    count[2]++;
  } else if (pcl.get_y() > subdomainYend_) {
    sendYrght_.send(pcl);
    count[3]++;
  } else if (pcl.get_z() < subdomainZstart_) {
    sendZleft_.send(pcl);
    count[4]++;
  } else if (pcl.get_z() > subdomainZend_) {
    sendZrght_.send(pcl);
    count[5]++;
  } else
    wasSent = false;
...
bool ParticleCommInjection::testOutsideDomain(
    const SpeciesParticle& pcl) const {
  return pcl.get_x() < 0. || pcl.get_y() < 0. || pcl.get_z() < 0. ||
         pcl.get_x() > domainLengthX_ || pcl.get_y() > domainLengthY_ ||
         pcl.get_z() > domainLengthZ_;
}

bool ParticleCommInjection::testOutsideNonperiodicDomain(
    const SpeciesParticle& pcl) const {
  return (!vct_->getPERIODICX_P() &&
          (pcl.get_x() < 0. || pcl.get_x() > domainLengthX_)) ||
         (!vct_->getPERIODICY_P() &&
          (pcl.get_y() < 0. || pcl.get_y() > domainLengthY_)) ||
         (!vct_->getPERIODICZ_P() &&
          (pcl.get_z() < 0. || pcl.get_z() > domainLengthZ_));
}

bool ParticleCommInjection::testPclsAreInDomain(
    const vector_SpeciesParticle& pcls) const {
  for (int pidx = 0; pidx < (int)pcls.size(); pidx++) {
    if (__builtin_expect(testOutsideDomain(pcls[pidx]), false))
      return false;
  }
  return true;
}

bool ParticleCommInjection::testPclsAreInNonperiodicDomain(
    const vector_SpeciesParticle& pcls) const {
  for (int pidx = 0; pidx < (int)pcls.size(); pidx++) {
    if (__builtin_expect(testOutsideNonperiodicDomain(pcls[pidx]), false))
      return false;
  }
  return true;
}
...
void ParticleCommInjection::applyBCsGlobally(vector_SpeciesParticle& pclList) {
  ...
  for (int iteration = 0; pstart < (int)pclList.size(); iteration++) {
    ...
    if (iteration >= 100) {
      dprintf("WARNING: applyBCsGlobally removing %d unrecoverable particles "
              "(species %d)",
...
void ParticleCommInjection::applyBCsLocally(vector_SpeciesParticle& pclList,
                                            int direction, bool applyShift,
                                            bool doApplyBCs) {
  using namespace Direction;
  const double Lxinv = 1.0 / domainLengthX_;
  const double Lyinv = 1.0 / domainLengthY_;
  ...
  if (applyShift) {
    ...
    default:
      invalid_value_error(direction);
    case XDN:
      assert(vct_->noXleftNeighbor_P());
      apply_Xleft_BC(pclList);
      break;
    case XUP:
      assert(vct_->noXrghtNeighbor_P());
      apply_Xrght_BC(pclList);
      break;
    case YDN:
      assert(vct_->noYleftNeighbor_P());
      apply_Yleft_BC(pclList);
      break;
    case YUP:
      assert(vct_->noYrghtNeighbor_P());
      apply_Yrght_BC(pclList);
      break;
    case ZDN:
      assert(vct_->noZleftNeighbor_P());
      apply_Zleft_BC(pclList);
      break;
    case ZUP:
      assert(vct_->noZrghtNeighbor_P());
      apply_Zrght_BC(pclList);
      break;
    }
  }
}
...
void ParticleCommInjection::apply_Xleft_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceXleft_) {
  default:
    unsupported_value_error(bcPfaceXleft_);
  ...
  case BCparticles::REEMISSION:
    for (int pidx = start; pidx < size; pidx++) {
      ...
      pcl.fetch_x() *= -1;
      double vel[3];
      sample_maxwellian(vel[0], vel[1], vel[2], thermalVelocityX_,
                        thermalVelocityY_, thermalVelocityZ_, bcRng_);
      vel[0] = fabs(vel[0]);
      pcl.set_u(vel);
    }
    break;
...
void ParticleCommInjection::apply_Yleft_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceYleft_) {
  default:
    unsupported_value_error(bcPfaceYleft_);
  case BCparticles::PERFECT_MIRROR:
    for (int pidx = start; pidx < size; pidx++) {
      pcls[pidx].fetch_y() *= -1;
      pcls[pidx].fetch_v() *= -1;
...
void ParticleCommInjection::apply_Zleft_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceZleft_) {
  default:
    unsupported_value_error(bcPfaceZleft_);
  case BCparticles::PERFECT_MIRROR:
    for (int pidx = start; pidx < size; pidx++) {
      pcls[pidx].fetch_z() *= -1;
      pcls[pidx].fetch_w() *= -1;
...
void ParticleCommInjection::apply_Xrght_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceXright_) {
  default:
    unsupported_value_error(bcPfaceXright_);
  case BCparticles::PERFECT_MIRROR:
    for (int pidx = start; pidx < size; pidx++) {
...
void ParticleCommInjection::apply_Yrght_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceYright_) {
  default:
    unsupported_value_error(bcPfaceYright_);
  case BCparticles::PERFECT_MIRROR:
    for (int pidx = start; pidx < size; pidx++) {
...
void ParticleCommInjection::apply_Zrght_BC(vector_SpeciesParticle& pcls,
                                           int start) {
  const int size = pcls.size();
  assert_le(0, start);
  switch (bcPfaceZright_) {
  default:
    unsupported_value_error(bcPfaceZright_);
  case BCparticles::PERFECT_MIRROR:
    for (int pidx = start; pidx < size; pidx++) {
...
/**
 * @brief Send exiting particles from the AoS communication buffer to
 * neighbours.
 *
...
int ParticleCommInjection::separateAndSendParticles() {
  // activate receiving
  recvXleft_.recv_start();
  recvXrght_.recv_start();
  recvYleft_.recv_start();
  recvYrght_.recv_start();
  recvZleft_.recv_start();
  recvZrght_.recv_start();

  // prepare send blocks
  sendXleft_.send_start();
  sendXrght_.send_start();
  sendYleft_.send_start();
  sendYrght_.send_start();
  sendZleft_.send_start();
  sendZrght_.send_start();
  ...
  if (print_pcl_comm_counts) {
    dprintf("spec %d send_count: %d+%d+%d+%d+%d+%d=%d", speciesNumber_,
            sendCount[0], sendCount[1], sendCount[2], sendCount[3],
            sendCount[4], sendCount[5], numPclsSent);
  }
  return numPclsSent;
}
...
/**
 * @brief Receive incoming particle blocks, apply boundary conditions, and keep
 * survivors.
 *
...
int ParticleCommInjection::handleReceivedParticles(int pclCommMode) {
  ...
  BlockCommunicator<SpeciesParticle>* recvBuffArr[numRecvBuffers] = {
      &recvXleft_, &recvXrght_, &recvYleft_,
      &recvYrght_, &recvZleft_, &recvZrght_};
  ...
  const int directionMap[numRecvBuffers] = {Direction::XDN, Direction::XUP,
  ...
  while (!(recvXleft_.comm_finished() && recvXrght_.comm_finished() &&
    ...
    Block<SpeciesParticle>& recvBlock =
        recvBuff->fetch_received_block(recvStatus);
    vector_SpeciesParticle& pclList = recvBlock.fetch_block();

    if (pclCommMode & do_apply_BCs_globally) {
      applyBCsGlobally(pclList);
    } else {
      applyBCsLocally(pclList, directionMap[recvIndex], applyShift[recvIndex],
                      doApplyBCs[recvIndex]);
    }
  ...
  if (print_pcl_comm_counts) {
    dprintf("spec %d recved_count: %d+%d+%d+%d+%d+%d=%d", speciesNumber_,
            recvCount[0], recvCount[1], recvCount[2], recvCount[3],
            recvCount[4], recvCount[5], numPclsRecved);
    dprintf("spec %d resent_count: %d+%d+%d+%d+%d+%d=%d", speciesNumber_,
            sendCount[0], sendCount[1], sendCount[2], sendCount[3],
            sendCount[4], sendCount[5], numPclsResent);
  }
 ...
 *
 * @param minNumIterations Minimum number of exchange iterations before
 * convergence checks.
 */
void ParticleCommInjection::recommunicateParticlesUntilDone(
  ...
  MPI_Allreduce(&numPclsSent, &totalNumPclsSent, 1, MPI_LONG_LONG, MPI_SUM,
                mpiComm_);
 ...
 */
void ParticleCommInjection::appendFromAoS(const SpeciesParticle* buffer,
                                          int count) {
  if (count <= 0)
    return;
...
int ParticleCommInjection::computeInjectionCountImpl() const {
  ...
  if (!vct_->isBoundaryProcess_P())
    return 0;

  const bool repopXleft =
      (vct_->noXleftNeighbor_P() && bcPfaceXleft_ == REEMISSION);
  const bool repopYleft =
      (vct_->noYleftNeighbor_P() && bcPfaceYleft_ == REEMISSION);
  const bool repopZleft =
      (vct_->noZleftNeighbor_P() && bcPfaceZleft_ == REEMISSION);
  const bool repopXrght =
      (vct_->noXrghtNeighbor_P() && bcPfaceXright_ == REEMISSION);
  const bool repopYrght =
      (vct_->noYrghtNeighbor_P() && bcPfaceYright_ == REEMISSION);
  const bool repopZrght =
      (vct_->noZrghtNeighbor_P() && bcPfaceZright_ == REEMISSION);

  if (!(repopXleft || repopYleft || repopZleft || repopXrght || repopYrght ||
        repopZrght))
    return 0;
  ...
  // Xleft
  if (repopXleft) {
    total += faceCells(1, numLayers, ybeg, yend, zbeg, zend);
    xbeg += numLayers;
  }
  // Xrght
  if (repopXrght) {
    total += faceCells(upXstart, xend, ybeg, yend, zbeg, zend);
    xend -= numLayers;
  }
  // Yleft
  if (repopYleft) {
    total += faceCells(xbeg, xend, 1, numLayers, zbeg, zend);
    ybeg += numLayers;
  }
  // Yrght
  if (repopYrght) {
    total += faceCells(xbeg, xend, upYstart, yend, zbeg, zend);
    yend -= numLayers;
  }
  // Zleft
  if (repopZleft)
    total += faceCells(xbeg, xend, ybeg, yend, 1, numLayers);
  // Zrght
  if (repopZrght)
    total += faceCells(xbeg, xend, ybeg, yend, upZstart, zend);
...
void ParticleCommInjection::fillInjectionParameter(
    injectionParameter* param) const {
  using namespace BCparticles;
  ...
  auto setFace = [&](int faceIdx, int ixB, int ixE, int iyB, int iyE, int izB,
                     int izE, bool active) {
    param->faces[faceIdx].ixBeg = ixB;
    param->faces[faceIdx].ixEnd = ixE;
    param->faces[faceIdx].iyBeg = iyB;
    param->faces[faceIdx].iyEnd = iyE;
    param->faces[faceIdx].izBeg = izB;
    param->faces[faceIdx].izEnd = izE;
    param->faces[faceIdx].nY = iyE - iyB + 1;
    param->faces[faceIdx].nZ = izE - izB + 1;
    param->faces[faceIdx].nCells = active ? (ixE - ixB + 1) *
                                                param->faces[faceIdx].nY *
                                                param->faces[faceIdx].nZ
                                          : 0;
