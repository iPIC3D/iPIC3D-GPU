...
bool compareDataArrays(vtkDataArray* array1, vtkDataArray* array2,
                       double tolerance) {
...
bool compareVTKFiles(const char* file1, const char* file2, double tolerance) {
  ...
  vtkSmartPointer<vtkStructuredPointsReader> reader1 =
      vtkSmartPointer<vtkStructuredPointsReader>::New();
  reader1->SetFileName(file1);
  reader1->Update();

  vtkSmartPointer<vtkStructuredPointsReader> reader2 =
      vtkSmartPointer<vtkStructuredPointsReader>::New();
  ...
  for (int i = 0; i < pointData1->GetNumberOfArrays(); ++i) {
    ...
    if (!compareDataArrays(array1, array2, tolerance)) {
      std::cout << "Data arrays " << array1->GetName() << " and "
                << array2->GetName() << " differ." << std::endl;
      return false;
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 4) {
    std::cerr << "Usage: " << argv[0] << " file1.vtk file2.vtk tolerance"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "The files are "
            << (areEqual ? "similar within the tolerance"
                         : "[not] similar within the tolerance")
            << std::endl;
