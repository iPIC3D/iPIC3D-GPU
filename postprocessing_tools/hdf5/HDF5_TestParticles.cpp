...
int main(int argc, char** argv) {
  if (argc == 1) {
    cout << "Provide initial cycle, step cycle, and last cycle in the command "
            "line"
         << endl;
  ...
  for (int si = 0; si < NsTestPart; si++) {
    ...
    pcl[si] = pclx * pcly * pclz * nxc * nyc *
              nzc; // cout << "pcl[" << si << "]" <<pcl[si] <<endl;
  ...
  cout << " Successful in opening and closing all the proc*.hdf the first time"
       << endl;
  ...
  for (int i_cycle = start_cycle; i_cycle < (end_cycle + 1);
       i_cycle += step_cycle) {
    ...
    for (int si = 0; si < NsTestPart; si++) {
      ...
      cc.str("");
      cc << "testparticle" << (ns + si) << "_cycle" << i_cycle << ".vtk";
      cout << "Start writing " << cc.str() << endl;
      ...
      for (int i = 0; i < nproc; i++) {
        ...
        cc.str("");
        cc << "/testparticles/species_" << (si + ns) << "/x/cycle_"
           << i_cycle; // cout << "reading data"<< cc.str() << endl;
        dataset_id =
            H5Dopen2(file_id, cc.str().c_str(), H5P_DEFAULT); // HDF 1.8.8
        datatype = H5Dget_type(dataset_id);
        dataspace = H5Dget_space(dataset_id);
        status = H5Sget_simple_extent_dims(dataspace, dims_out, NULL);
        nop = dims_out[0];
        totwritepcl += nop; // cout << "nop =" << nop <<endl;
        status =
            H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, X);
        status = H5Dclose(dataset_id);

        cc.str("");
        cc << "/testparticles/species_" << (si + ns) << "/y/cycle_" << i_cycle;
        dataset_id = H5Dopen2(file_id, cc.str().c_str(), H5P_DEFAULT);
        status =
            H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, Y);
        status = H5Dclose(dataset_id);

        cc.str("");
        cc << "/testparticles/species_" << (si + ns) << "/z/cycle_" << i_cycle;
        dataset_id = H5Dopen2(file_id, cc.str().c_str(), H5P_DEFAULT);
        status =
            H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, Z);
        status = H5Dclose(dataset_id);

        //						cc.str("");cc <<
        //"/testparticles/species_" << (si+ns) << "/u/cycle_" << i_cycle;
        //						dataset_id =
        // H5Dopen2(file_id,cc.str().c_str(), H5P_DEFAULT);
        // status = H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        // U); 						status =
        // H5Dclose(dataset_id);
        //
        //						cc.str("");cc <<
        //"/testparticles/species_" << (si+ns) << "/v/cycle_" << i_cycle;
        //						dataset_id =
        // H5Dopen2(file_id,cc.str().c_str(), H5P_DEFAULT);
        // status = H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        // V); 						status =
        ...
        //
        //						cc.str("");cc <<
        //"/testparticles/species_" << (si+ns) << "/w/cycle_" << i_cycle;
        //						dataset_id =
        // H5Dopen2(file_id,cc.str().c_str(), H5P_DEFAULT);
        // status = H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        // W); 						status =
        ...
        //
        //						cc.str("");cc <<
        //"/testparticles/species_" << (si+ns) << "/q/cycle_" << i_cycle;
        //						dataset_id =
        // H5Dopen2(file_id,cc.str().c_str(), H5P_DEFAULT);
        // status = H5Dread(dataset_id, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        // Q);
