...
int c_Solver::Init(int argc, char **argv) {
  ...
  if (col->getCase() == "GEMnoPert")
    EMf->initGEMnoPert();
  else if (col->getCase() == "ForceFree")
    EMf->initForceFree();
  else if (col->getCase() == "GEM")
    EMf->initGEM();
  else if (col->getCase() == "GEMDoubleHarris")
    ...
    EMf->initDipole2D();
  else if (col->getCase() == "NullPoints")
    EMf->initNullPoints();
  else if (col->getCase() == "TaylorGreen")
    EMf->initTaylorGreen();
  ...
  if (restart_status == 0) {
    for (int i = 0; i < ns; i++) {
      if (col->getCase() == "ForceFree")
        part[i].force_free(EMf);
#ifdef BATSRUS
      else if (col->getCase() == "BATSRUS")
        part[i].MaxwellianFromFluid(EMf, col, i);
#endif
      else if (col->getCase() == "NullPoints")
        part[i].maxwellianNullPoints(EMf);
      else if (col->getCase() == "TaylorGreen")
        part[i].maxwellianNullPoints(
            EMf); // Flow is initiated from the current prescribed on the grid.
      else if (col->getCase() == "GEMDoubleHarris")
        part[i].maxwellianDoubleHarris(EMf);
      else
        part[i].maxwellian(EMf);
...
#ifndef NO_HDF5
    if (col->getWriteMethod() == "shdf5" || col->getCallFinalize() ||
        restart_cycle > 0 ||
    ...
    if (!col->field_output_is_off()) {
      if (col->getWriteMethod() == "pvtk") {
        if (!(col->getFieldOutputTag()).empty())
          fieldwritebuffer = newArr4(float, (grid->getNZN() - 3),
                                     grid->getNYN() - 3, grid->getNXN() - 3, 3);
        if (!(col->getMomentsOutputTag()).empty())
          momentwritebuffer = newArr3(float, (grid->getNZN() - 3),
                                      grid->getNYN() - 3, grid->getNXN() - 3);
      } else if (col->getWriteMethod() == "nbcvtk") {
        momentreqcounter = 0;
        fieldreqcounter = 0;
        if (!(col->getFieldOutputTag()).empty())
          fieldwritebuffer = newArr4(float, (grid->getNZN() - 3) * 4,
                                     grid->getNYN() - 3, grid->getNXN() - 3, 3);
        if (!(col->getMomentsOutputTag()).empty())
          momentwritebuffer = newArr3(float, (grid->getNZN() - 3) * 14,
                                      grid->getNYN() - 3, grid->getNXN() - 3);
      }
    }
  }
...
int c_Solver::initCUDA() {
  ...
  { // init arrays on device, pointers are device pointer, copied
    ...
    for (int i = 0; i < ns; i++) {
      ...
      pclsArrayHostPtr[i] =
          newHostPinnedObject<particleArrayCUDA>(part + i, streams[i]);
      pclsArrayCUDAPtr[i] = pclsArrayHostPtr[i]->copyToDevice();

      // register the host pointer to pinned memory
      cudaErrChk(cudaHostRegister(part[i].get_pcl_array().getList(),
                                  part[i].get_pcl_array().capacity() *
  ...
  for (int i = 0; i < ns; i++) {
    moverParamHostPtr[i] = newHostPinnedObject<moverParameter>(
        part + i, pclsArrayCUDAPtr[i], departureArrayCUDAPtr[i],
  ...
  for (int i = 0; i < ns; i++)
    cudaMallocAsync(&(momentsCUDAPtr[i]),
                    gridSize * 10 * sizeof(cudaCommonType), streams[i]);

  { // register the 10 densities to host pinned memory
    for (int i = 0; i < ns; i++) {
      cudaErrChk(cudaHostRegister((void *)&(EMf->getRHOns().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getJxs().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getJys().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getJzs().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpXXsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpXYsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpXZsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpYYsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpYZsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
      cudaErrChk(cudaHostRegister((void *)&(EMf->getpZZsn().get(i, 0, 0, 0)),
                                  gridSize * sizeof(cudaCommonType),
                                  cudaHostRegisterDefault));
    }
  }
  ...
  cudaMallocAsync(&fieldForPclCUDAPtr, fieldSize * 24 * sizeof(cudaCommonType),
                  0);

  cudaErrChk(cudaHostAlloc((void **)&fieldForPclHostPtr,
                           fieldSize * 24 * sizeof(cudaCommonType), 0));
  ...
  { // output path for data analysis
    auto GMMSubDomainOutputPath =
        "./velocityGMM/subDomain" + std::to_string(myrank) + "/";
    for (int i = 0; i < ns; i++) {
      auto GMMSpeciesOutputPath =
          GMMSubDomainOutputPath + "species" + std::to_string(i) + "/";
      if (0 != checkOutputFolder(GMMSpeciesOutputPath)) {
        throw std::runtime_error(
            "[!]Error: Can not create output folder for velocity GMM species");
      }
    }

    auto histogramSubDomainOutputPath =
        "./velocityHistogram/subDomain" + std::to_string(myrank) + "/";
    for (int i = 0; i < ns; i++) {
      auto histogramSpeciesOutputPath =
          histogramSubDomainOutputPath + "species"s + std::to_string(i);
      if (0 != checkOutputFolder(histogramSpeciesOutputPath)) {
        throw std::runtime_error("[!]Error: Can not create output folder for "
                                 "velocity histogram species");
      }
    }
  }
...
int c_Solver::deInitCUDA() {
  ...
  { // unregister the pinned mem
    for (int i = 0; i < ns; i++) {
      ...
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getRHOns().get(i, 0, 0, 0))));
      ...
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpXXsn().get(i, 0, 0, 0))));
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpXYsn().get(i, 0, 0, 0))));
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpXZsn().get(i, 0, 0, 0))));
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpYYsn().get(i, 0, 0, 0))));
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpYZsn().get(i, 0, 0, 0))));
      cudaErrChk(
          cudaHostUnregister((void *)&(EMf->getpZZsn().get(i, 0, 0, 0))));
    }
  }
...
void c_Solver::CalculateMoments(bool isInit) {
  ...
  if (Parameters::get_VECTORIZE_MOMENTS()) {
    ...
    case Parameters::AoS:
      ...
      EMf->sumMoments_AoS(
          part); // sum up the 10 densities of each particles of each species
  ...
  if (isInit) {
    ...
    for (int i = 0; i < ns; i++) {
      cudaErrChk(cudaMemsetAsync(momentsCUDAPtr[i], 0,
                                 gridSize * 10 * sizeof(cudaCommonType),
                                 streams[i])); // set moments to 0
      // copy the particles to device---- already there...by initliazation or
      // Mover launch the moment kernel
      momentKernelNew<<<(pclsArrayHostPtr[i]->getNOP() / 256 + 1), 256, 0,
                        streams[i]>>>(momentParamCUDAPtr[i], grid3DCUDACUDAPtr,
                                      momentsCUDAPtr[i], 0);
      // copy moments back to 10 densities
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getRHOns().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 0 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getJxs().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 1 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getJys().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 2 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getJzs().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 3 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpXXsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 4 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpXYsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 5 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpXZsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 6 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpYYsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 7 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpYZsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 8 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
      cudaErrChk(cudaMemcpyAsync((void *)&(EMf->getpZZsn().get(i, 0, 0, 0)),
                                 momentsCUDAPtr[i] + 9 * gridSize,
                                 gridSize * sizeof(cudaCommonType),
                                 cudaMemcpyDefault, streams[i]));
    }
  }
  ...
  if (col->getCase() == "Dipole") {
    EMf->ConstantChargePlanet(col->getL_square(), col->getx_center(),
                              col->gety_center(), col->getz_center());
  } else if (col->getCase() == "Dipole2D") {
    EMf->ConstantChargePlanet2DPlaneXZ(col->getL_square(), col->getx_center(),
                                       col->getz_center());
  }
...
int c_Solver::cudaLauncherAsync(const int species) {
  ...
  moverKernel<<<getGridSize((int)pclsArrayHostPtr[species]->getNOP(), 256), 256,
                0, streams[species]>>>(moverParamCUDAPtr[species],
                                       fieldForPclCUDAPtr, grid3DCUDACUDAPtr);
  cudaErrChk(cudaEventRecord(event1, streams[species]));
  // Moment stayed
  cudaErrChk(cudaMemsetAsync(momentsCUDAPtr[species], 0,
                             gridSize * 10 * sizeof(cudaCommonType),
                             streams[species])); // set moments to 0
  momentKernelStayed<<<getGridSize((int)pclsArrayHostPtr[species]->getNOP(),
                                   256),
                       256, 0, streams[species]>>>(
  ...
  if (x > part[species].get_pcl_list().capacity()) {
    ...
    // register the list
    cudaErrChk(cudaHostRegister(pclArray->getList(),
                                pclArray->capacity() * sizeof(SpeciesParticle),
                                cudaHostRegisterDefault));
  }
  exitingKernel<<<getGridSize((int)pclsArrayHostPtr[species]->getNOP(), 256),
                  256, 0, streams[species + ns]>>>(
  ...
  cudaErrChk(cudaMemcpyAsync(part[species].get_pcl_array().getList(),
                             exitingArrayHostPtr[species]->getArray(),
                             x * sizeof(SpeciesParticle), cudaMemcpyDefault,
                             streams[species + ns]));
  ...
  sortingKernel1<<<getGridSize(x, 128), 128, 0, streams[species]>>>(
      pclsArrayCUDAPtr[species], departureArrayCUDAPtr[species],
      fillerBufferArrayCUDAPtr[species], hashedSumArrayCUDAPtr[species] + 7, x);
  sortingKernel2<<<getGridSize((int)(pclsArrayHostPtr[species]->getNOP() - x),
                               256),
                   256, 0, streams[species]>>>(
      pclsArrayCUDAPtr[species], departureArrayCUDAPtr[species],
      fillerBufferArrayCUDAPtr[species], hashedSumArrayCUDAPtr[species] + 6,
      pclsArrayHostPtr[species]->getNOP() - x);

  cudaErrChk(cudaEventDestroy(event1));
  cudaErrChk(cudaEventDestroy(event2));
  cudaErrChk(
      cudaStreamSynchronize(streams[species + ns])); // exiting particle copied
  return x;
}
...
bool c_Solver::ParticlesMover() {
  ...
  // move all species of particles
  {
    ...
    // cudaErrChk(cudaMemcpyAsync(fieldForPclCUDAPtr,
    // (void*)&(EMf->get_fieldForPcls().get(0,0,0,0)),
    // gridSize*8*sizeof(cudaCommonType), cudaMemcpyDefault, streams[0]));
    cudaErrChk(cudaMemcpyAsync(
        fieldForPclCUDAPtr, fieldForPclHostPtr,
        (grid->getNZN() * (grid->getNYN() - 1) * (grid->getNXN() - 1)) * 24 *
    ...
    CALI_MARK_END("recommunicate_particles");
  }
  ...
  // for (int i=0; i < ns; i++)
  //   Qremoved[i] =
  //   part[i].deleteParticlesInsideSphere2DPlaneXZ(col->getL_square(),col->getx_center(),col->getz_center());
  // }
  ...
  for (int i = 0; i < ns; i++) {
    ...
    if ((part[i].getNOP() + stayedParticle[i]) >=
        pclsArrayHostPtr[i]
            ->getSize()) { // not enough size, expand the device array size
      pclsArrayHostPtr[i]->expand((part[i].getNOP() + stayedParticle[i]) * 1.5,
                                  streams[i]);
    ...
    cudaErrChk(
        cudaMemcpyAsync(pclsArrayHostPtr[i]->getpcls() + stayedParticle[i],
    ...
    // reset the hashedSum, no need for departureArray it will be cleared in
    // Mover
    for (int j = 0; j < 8; j++)
      hashedSumArrayHostPtr[i][j].resetBucket();
    cudaErrChk(cudaMemcpyAsync(hashedSumArrayCUDAPtr[i],
                               hashedSumArrayHostPtr[i], 8 * sizeof(hashedSum),
                               cudaMemcpyDefault, streams[i]));
  }
...
void c_Solver::WriteOutput(int cycle) {
  ...
  if (col->getWriteMethod() == "nbcvtk") { // Non-blocking collective MPI-IO
      ...
      if (!(col->getFieldOutputTag()).empty()) {
        ...
        if (fieldreqcounter > 0) {
          ...
          for (int si = 0; si < fieldreqcounter; si++) {
            int error_code = MPI_File_write_all_end(
                fieldfhArr[si], &fieldwritebuffer[si][0][0][0],
                &fieldstsArr[si]); // fieldstsArr[si].MPI_ERROR;
        ...
        fieldreqcounter =
            WriteFieldsVTKNonblk(grid, EMf, col, vct, cycle, fieldwritebuffer,
                                 fieldreqArr, fieldfhArr);
      }
      ...
      if (!(col->getMomentsOutputTag()).empty()) {
        ...
        if (momentreqcounter > 0) {
          ...
          for (int si = 0; si < momentreqcounter; si++) {
            int error_code = MPI_File_write_all_end(
                momentfhArr[si], &momentwritebuffer[si][0][0],
                &momentstsArr[si]); // momentstsArr[si].MPI_ERROR;
            if (error_code != MPI_SUCCESS) {
              char error_string[100];
              int length_of_error_string, error_class;
              MPI_Error_class(error_code, &error_class);
        ...
        momentreqcounter =
            WriteMomentsVTKNonblk(grid, EMf, col, vct, cycle, momentwritebuffer,
                                  momentreqArr, momentfhArr);
      }
    }
    ...
    if (!col->field_output_is_off() &&
        (cycle % (col->getFieldOutputCycle()) == 0 || cycle == first_cycle)) {
      if (!(col->getFieldOutputTag()).empty()) {
        // WriteFieldsVTK(grid, EMf, col, vct, col->getFieldOutputTag()
        // ,cycle);//B + E + Je + Ji + rho
        WriteFieldsVTK(grid, EMf, col, vct, col->getFieldOutputTag(), cycle,
                       fieldwritebuffer); // B + E + Je + Ji + rho
      }
      if (!(col->getMomentsOutputTag()).empty()) {
        WriteMomentsVTK(grid, EMf, col, vct, col->getMomentsOutputTag(), cycle,
                        momentwritebuffer);
      }
    }
...
void c_Solver::WriteRestart(int cycle) {
  ...
  if (restart_cycle > 0 && cycle % restart_cycle == 0) {
    ...
    { // copy the particles back to host, if restart output
      for (int i = 0; i < ns; i++) {
        if (part[i].get_pcl_array().capacity() <
            pclsArrayHostPtr[i]->getNOP()) {
          // unregister the list
          cudaErrChk(cudaHostUnregister(part[i].get_pcl_array().getList()));
          // expand the host array
          ...
          // register the list
          cudaErrChk(
              cudaHostRegister(pclArray->getList(),
                               pclArray->capacity() * sizeof(SpeciesParticle),
                               cudaHostRegisterDefault));
        }
        cudaErrChk(cudaMemcpyAsync(
            part[i].get_pcl_array().getList(), pclsArrayHostPtr[i]->getpcls(),
            pclsArrayHostPtr[i]->getNOP() * sizeof(SpeciesParticle),
            cudaMemcpyDefault, streams[i]));
        part[i].get_pcl_array().setSize(
            pclsArrayHostPtr[i]
                ->getNOP()); // this will be corrected in the next mover
      }
      cudaErrChk(cudaDeviceSynchronize());
    }
    ...
    if (myrank == (nprocs - 1)) {
      ...
      if (cycle == 0)
        my_file << "\t"
                ...
                << "Kenergy(species)"
                << "\t"
                << "BulkEnergy(species)" << endl;
      my_file << cycle << "\t"
              << "\t" << (Eenergy + Benergy + TOTenergy) << "\t" << TOTmomentum
              << "\t" << Eenergy << "\t" << Benergy << "\t" << TOTenergy;
      for (int is = 0; is < ns; is++)
        my_file << "\t" << Ke[is];
      for (int is = 0; is < ns; is++)
        my_file << "\t" << BulkEnergy[is];
      my_file << endl;
      my_file.close();
    }
  }
...
/* write the conserved quantities
void c_Solver::WriteConserved(int cycle) {
  if(col->getDiagnosticsOutputCycle() > 0 && cycle %
col->getDiagnosticsOutputCycle() == 0)
  {
    ...
    if(buf_counter==OUTPUT_BUFSIZE || cycle==(LastCycle()-1)){
      ...
      //my_file  << "Cycle" << "\t" << "Total_Energy"
<< "\t" << "Momentum" << "\t" << "Eenergy" <<"\t" << "Benergy" << "\t" <<
"Kenergy" << endl; for(int bufid=0;bufid<OUTPUT_BUFSIZE;bufid++) ss <<
outputcycle[bufid] << "\t" << (Eenergy[bufid]+Benergy[bufid]+Kenergy[bufid])<<
"\t" << TOTmomentum[bufid] << "\t" << Eenergy[bufid] << "\t" << Benergy[bufid]
<< "\t" << Kenergy[bufid] << endl;
  ...
  // if(cycle % col->getVelocityDistributionOutputCycle() == 0)
  {
    for (int is = 0; is < ns; is++) {
      ...
      long long *VelocityDist =
          part[is].getVelocityDistribution(nDistributionBins, maxVel);
      if (myrank == 0) {
...
void c_Solver::WriteFields(int cycle) {
  ...
  if (cycle % (col->getFieldOutputCycle()) == 0 || cycle == first_cycle) {
    if (!(col->getFieldOutputTag()).empty())
      fetch_outputWrapperFPP().append_output((col->getFieldOutputTag()).c_str(),
                                             cycle); // E+B+Js
    if (!(col->getMomentsOutputTag()).empty())
      fetch_outputWrapperFPP().append_output(
          (col->getMomentsOutputTag()).c_str(), cycle); // rhos+pressure
  }
#endif
}
...
void c_Solver::WriteParticles(int cycle) {
  ...
  fetch_outputWrapperFPP().append_output((col->getPclOutputTag()).c_str(),
                                         cycle, 0); //"position + velocity + q "
#endif
}
...
void c_Solver::WriteTestParticles(int cycle) {
  ...
  fetch_outputWrapperFPP().append_output(
      "testpartpos + testpartvel+ testparttag", cycle, 0); // + testpartcharge
#endif
}
